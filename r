...
function Library:CreateLibrary(opts)
    ...
    -- Create Config system for this library instance (robust + safe JSON + atomic-ish writes)
    local Config = {}
    do
        local Base = sanitize(libraryName)
        -- Configs directory (no per-game subfolder, matches user's original approach)
        local DIR = tostring(opts.ConfigFolder or Library.ConfigFolder or "Dendrite.cc")

        -- Backing store: memory fallback when file APIs are unavailable
        Library._memConfig = Library._memConfig or { files = {}, meta = {} }
        local function memEnsure()
            local key = DIR
            if not Library._memConfig.files[key] then Library._memConfig.files[key] = {} end
            if not Library._memConfig.meta[key] then Library._memConfig.meta[key] = { autoload = {}, lastSaved = {} } end
            return key
        end
        ...
        local function ensureFolder()
            if type(_isfolder) == "function" and type(_makefolder) == "function" then
                if not _isfolder(DIR) then pcall(_makefolder, DIR) end
            end
        end
        ...
        local function fsAvailable()
            ...
            return (CAN_WRITE_DIR or CAN_WRITE_ROOT)
        end

        local MEM_MODE = nil -- set on first Ensure()
        function Config.Ensure()
            if MEM_MODE == nil then
                MEM_MODE = (not fsAvailable())
            end
            if MEM_MODE then
                memEnsure()
            else
        ...
        local function atomicWrite(path, content)
            local tmp = path .. ".tmp"
            _writefile(tmp, content)
            -- Best-effort replace without rename support
            pcall(_delfile, path)
            local ok, tmpContent = pcall(_readfile, tmp)
            if ok and type(tmpContent) == "string" then
                _writefile(path, tmpContent)
            else
        ...
        local function rootPathFor(name)
            local s = sanitize(name or "")
            return (sanitize(Base) .. "_" .. s .. ".json")
        end
        function Config.Path(name)
            ...
            if CAN_WRITE_DIR then
                return DIR .. "/" .. s .. ".json"
            elseif CAN_WRITE_ROOT then
                return rootPathFor(s)
            else
                -- Should not happen when MEM_MODE == false, but return folder path by default
                return DIR .. "/" .. s .. ".json"
            end
        end
    ...
    local IndexAdd, IndexRemove, SaveIndex, LoadIndex
        ...
        function Config.Save(name, data)
            ...
            -- Preserve any existing metadata in the file
            local existingMeta = nil
            if not MEM_MODE then
                local okRaw, raw = pcall(_readfile, p)
                if okRaw and type(raw) == "string" and #raw > 0 then
                    local okD, dec = pcall(function() return HttpService:JSONDecode(raw) end)
                    if okD and type(dec) == "table" and type(dec.metadata) == "table" then
                        existingMeta = dec.metadata
                    end
                end
            end
            local values = serialize(data or {})
            if type(values) ~= "table" then values = {} end
            -- Merge metadata structure similar to user's original system
            local meta = existingMeta or { autoLoad = false, createdAt = os.time(), lastModified = os.time(), name = sanitize(name) }
            meta.lastModified = os.time()
            local payload = { metadata = meta, values = values }
            if MEM_MODE then
                local key = memEnsure()
                local sname = sanitize(name)
                Library._memConfig.files[key][sname] = payload
                sessionAdd(sname)
                return true
            end
            local ok, json = pcall(function() return HttpService:JSONEncode(payload) end)
            if not ok then return false end
            -- Try preferred path first, then root fallback (if different)
            local ok2, err = pcall(function() atomicWrite(p, json) end)
            if not ok2 and CAN_WRITE_ROOT and CAN_WRITE_DIR and p ~= rootPathFor(name) then
                local alt = rootPathFor(name)
                ok2, err = pcall(function() atomicWrite(alt, json) end)
                if ok2 then p = alt end
            end
        ...
        function Config.Load(name)
            ...
            -- Try preferred path then root fallback
            local tryPaths = { p }
            local alt = rootPathFor(name)
            if alt ~= p then table.insert(tryPaths, alt) end
            for _, path in ipairs(tryPaths) do
                local okRaw, raw = pcall(_readfile, path)
                if okRaw and type(raw) == "string" and #raw > 0 then
                    local ok, decoded = pcall(function() return HttpService:JSONDecode(raw) end)
                    if ok and type(decoded) == "table" then
                        -- Expect storage shape { metadata = {}, values = table }
                        local vs = type(decoded.values) == "table" and decoded.values or decoded
                        local obj = deserialize(vs)
                        if type(obj) == "table" then
                            -- strip any reserved fields
                            obj.__meta = nil
                        end
                        return obj
                    else
                        pcall(function() _writefile(path .. ".bak", raw) end)
                    end
                end
            end
            return {}
        ...
        function Config.List()
            ...
            if MEM_MODE then
                local key = memEnsure()
                for name, _ in pairs(Library._memConfig.files[key]) do
                    if name and name ~= "__meta" then table.insert(r, name) end
                end
            else
                -- Directory scan if available; otherwise, return in-session list
                local files = type(_listfiles) == "function" and (_listfiles(DIR) or {}) or nil
                if files then
                    for _, f in ipairs(files) do
                        local n = f:match("([^/\\]+)$") or f
                        n = n:gsub("%.json$", "")
                        if n ~= "" and n:sub(1,2) ~= "__" then table.insert(r, n) end
                    end
                else
                    r = sessionList()
                end
        ...
        function Config.Delete(name)
            ...
            if MEM_MODE then
                ...
                pcall(_delfile, p)
                -- Also try deleting root-fallback path
                local alt = rootPathFor(name)
                if alt ~= p then pcall(_delfile, alt) end
                sessionRemove(sanitize(name or ""))
            end
        end

        -- Autoload flags embedded per-config inside the JSON under metadata
        ...
        function Config.GetAutoLoad(name)
            ...
            for _, path in ipairs(tryPaths) do
                ...
                if okRaw then
                    local ok, dec = pcall(function() return HttpService:JSONDecode(raw) end)
                    if ok and type(dec) == "table" and type(dec.metadata) == "table" then
                        return dec.metadata.autoLoad == true
                    end
                end
            end
            return false
        ...
        function Config.RecordSave(name)
            ...
            if MEM_MODE then
                local key = memEnsure()
                local t = Library._memConfig.files[key][p]
                if type(t) == "table" and type(t.metadata) == "table" and t.metadata.autoLoad then
            ...
            for _, path in ipairs(tryPaths) do
                ...
                if okRaw then
                    ...
                    if ok and type(dec) == "table" then
                        dec.metadata = dec.metadata or {}
                        if dec.metadata.autoLoad then
                            dec.metadata.lastModified = os.time()
        ...
        function Config.ClearMeta(name)
            ...
            if MEM_MODE then
                ...
                if type(t) == "table" then
                    if t.metadata then t.metadata.autoLoad = nil; t.metadata.lastModified = nil end
                end
                return
            end
            local tryPaths = { p }
            ...
            for _, path in ipairs(tryPaths) do
                ...
                if okRaw then
                    local ok, dec = pcall(function() return HttpService:JSONDecode(raw) end)
                    if ok and type(dec) == "table" then
                        if type(dec.metadata) == "table" then
                            dec.metadata.autoLoad = nil
                            dec.metadata.lastModified = nil
                        end
                        local okJ, json = pcall(function() return HttpService:JSONEncode(dec) end)
                        if okJ then pcall(function() atomicWrite(path, json) end) end
                        break
        ...
        function Config.MostRecentAutoLoad()
            ...
            local files = type(_listfiles) == "function" and (_listfiles(DIR) or {}) or {}
            for _, f in ipairs(files) do
                local n = f:match("([^/\\]+)$") or f
                local okRaw, raw = pcall(_readfile, f)
                if okRaw and type(raw) == "string" and #raw > 0 then
                    local ok, dec = pcall(function() return HttpService:JSONDecode(raw) end)
                    if ok and type(dec) == "table" and type(dec.metadata) == "table" and dec.metadata.autoLoad then
