...
local Library = { _windows = {}, _controls = {}, _theme = Theme, _fonts = Fonts, _version = "3.1.0", _searchEntries = {}, Flags = {}, _3dPreviews = {} }
...
Library._roundToggles = Library._roundToggles ~= false
Library._toggleStyleListeners = Library._toggleStyleListeners or {}

function Library:GetRoundToggles()
    return self._roundToggles == true
end

function Library:_notifyToggleStyleListeners(immediate)
    local list = self._toggleStyleListeners or {}
    local isRound = self:GetRoundToggles()
    for i = 1, #list do
        local fn = list[i]
        if type(fn) == "function" then
            pcall(fn, isRound, immediate == true)
        end
    end
end

function Library:_registerToggleStyleListener(fn)
    if type(fn) ~= "function" then return nil end
    self._toggleStyleListeners = self._toggleStyleListeners or {}
    table.insert(self._toggleStyleListeners, fn)
    pcall(fn, self:GetRoundToggles(), true)
    return fn
end

function Library:SetRoundToggles(enabled, silent)
    enabled = (enabled == true)
    local changed = self._roundToggles ~= enabled
    self._roundToggles = enabled
    if changed or not silent then
        self:_notifyToggleStyleListeners(true)
        if changed then
            task.delay(0.28, function()
                if Library and Library._roundToggles == enabled then
                    Library:_notifyToggleStyleListeners(true)
                end
            end)
        end
    end
...
local _luvC3 = Color3.new
local _luvBlack = _luvC3(0, 0, 0)
local _luvClamp = math.clamp

local function _rgbToLuv(c)
...
local function _luvToRgb(l, u, v)
    if l < 0.0197955 then return _luvBlack end
    local u2 = u / l + 0.19783
    local v2 = v / l + 0.46832
    local yy = (l + 16) / 116
    yy = yy > 0.206896551724137931 and yy * yy * yy or 0.12841854934601665 * yy - 0.01771290335807126
    local xx = yy * u2 / v2
    local zz = yy * ((3 - 0.75 * u2) / v2 - 5)
    local r = 7.2914074 * xx - 1.5372080 * yy - 0.4986286 * zz
    local g = -2.1800940 * xx + 1.8757561 * yy + 0.0415175 * zz
    local b = 0.1253477 * xx - 0.2040211 * yy + 1.0569959 * zz
    ...
    return _luvC3(
        _luvClamp(r < 3.1306684425e-3 and 12.92 * r or 1.055 * r ^ (1 / 2.4) - 0.055, 0, 1),
        _luvClamp(g < 3.1306684425e-3 and 12.92 * g or 1.055 * g ^ (1 / 2.4) - 0.055, 0, 1),
        _luvClamp(b < 3.1306684425e-3 and 12.92 * b or 1.055 * b ^ (1 / 2.4) - 0.055, 0, 1)
    )
end

local function luvLerp(c0, c1, t)
    local l0, u0, v0 = _rgbToLuv(c0)
    local l1, u1, v1 = _rgbToLuv(c1)
    return _luvToRgb((1 - t) * l0 + t * l1, (1 - t) * u0 + t * u1, (1 - t) * v0 + t * v1)
end

local _rainbowKeypoints = {
...
local function luvRainbow(t, s, v)
    t = t % 1
    local segCount = #_rainbowKeypoints
    local pos = t * segCount
    local idx = math.floor(pos)
    local frac = pos - idx
    local c0 = _rainbowKeypoints[(idx % segCount) + 1]
    local c1 = _rainbowKeypoints[((idx + 1) % segCount) + 1]
    local base = luvLerp(c0, c1, frac)
    if s < 1 or v < 1 then
        local h2, _, _ = Color3.toHSV(base)
        return Color3.fromHSV(h2, s, v)
    end
    return base
end

local function luvPulse(t, s, baseV, pulseT)
    local full = luvRainbow(t, s, baseV)
    local dim = luvRainbow(t, s, math.max(baseV * 0.15, 0.05))
    return luvLerp(dim, full, pulseT)
end
...
function Library:Unload()
    ...
    pcall(function()
        for _, p in ipairs(self._3dPreviews or {}) do
            if type(p) == "table" and type(p.Destroy) == "function" then
    ...
    pcall(function()
        if self._radar and self._radar.Parent then self._radar:Destroy() end
    end)
    pcall(function()
    ...
    self._radar = nil
    self._radarFrame = nil
    self._radarViewport = nil
    self._radarVisible = false
    self._radarDots = {}
    self._keybinds = {}
    self._controls = {}
    self.Flags = {}
...
do
    ...
    function Library:_setupKeybindListDragging(frame, strokeElement, positionField)
        local isDragging = false
        local startPos = Vector2.new(0, 0)
        local frameStartPos = Vector2.new(0, 0)
        local dragOffset = Vector2.new(0, 0)
        local parentAbsAtStart = Vector2.new(0, 0)
        positionField = positionField or "_keybindListPosition"
...
do
    Library._radar = nil
    Library._radarFrame = nil
    Library._radarViewport = nil
    Library._radarVisible = false
    Library._radarDots = Library._radarDots or {}
    Library._radarPosition = Library._radarPosition or {
        X = 0,
        XOffset = 16,
        Y = 0,
        YOffset = 56
    }
    Library._radarSize = Library._radarSize or UDim2.fromOffset(210, 228)
    Library._radarRange = Library._radarRange or 180

    function Library:_hideRadarDots()
        for _, dot in pairs(self._radarDots or {}) do
            if dot then
                dot.Visible = false
            end
        end
    ...
    function Library:_cleanupRadarDots(activePlayers)
        local dots = self._radarDots or {}
    ...
    function Library:_ensureRadarDot(container, player)
        self._radarDots = self._radarDots or {}
        local dot = self._radarDots[player]
        if dot and dot.Parent then
            return dot
        end

        dot = Create("Frame", {
            Name = "Dot_" .. tostring(player and (player.UserId or player.Name) or "Unknown"),
            Size = UDim2.fromOffset(6, 6),
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = Theme.Accent,
            BorderSizePixel = 0,
            Visible = false,
            ZIndex = (container.ZIndex or 1) + 4,
            Parent = container
        })
        Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = dot})
        self._radarDots[player] = dot
        return dot
    end

    function Library:_getRadarLibraryGui()
        for _, gui in pairs(CoreGui:GetChildren()) do
            if gui:IsA("ScreenGui") and gui.DisplayOrder == 999999 then
                return gui
            end
    ...
    function Library:_getRadarLocalRoot()
        ...
        return localPlayer, root
    end

    function Library:_getRadarRelativePosition(worldPosition, originRoot)
        ...
        if camera then
            ...
            if flatCameraLook.Magnitude > 0.001 then
                look = flatCameraLook.Unit
            end
        end

        if not look then
            local rootLook = originRoot.CFrame.LookVector
            local flatRootLook = Vector3.new(rootLook.X, 0, rootLook.Z)
            if flatRootLook.Magnitude > 0.001 then
                look = flatRootLook.Unit
            else
                look = Vector3.new(0, 0, -1)
            end
        end

        local basis = CFrame.lookAt(originRoot.Position, originRoot.Position + look)
        local relative = basis:PointToObjectSpace(worldPosition)
        return Vector2.new(relative.X, relative.Z)
    end

    function Library:_getRadarDotColor(player, humanoid)
        local players = SRV("Players")
        local localPlayer = players and players.LocalPlayer
        local sameTeam = false
        pcall(function()
            sameTeam = localPlayer and localPlayer.Team ~= nil and player and player.Team == localPlayer.Team
        end)
        ...
        if humanoid and humanoid.MaxHealth and humanoid.MaxHealth > 0 then
            alpha = math.clamp(humanoid.Health / humanoid.MaxHealth, 0, 1)
        end
        return Theme.Bad:Lerp(Theme.Warn, alpha)
    end

    function Library:_updateRadarLayout()
        local frame = self._radarFrame
        local body = self._radarBody
        local viewport = self._radarViewport
        local footer = self._radarFooterLabel
        if not (frame and body and viewport and footer) then
            return
        end

        local availableWidth = math.max(120, body.AbsoluteSize.X)
        local availableHeight = math.max(120, body.AbsoluteSize.Y - 18)
        local diameter = math.max(112, math.min(availableWidth, availableHeight))
        local topOffset = math.max(2, math.floor((availableHeight - diameter) * 0.5))

        viewport.Size = UDim2.fromOffset(diameter, diameter)
        viewport.Position = UDim2.new(0.5, -math.floor(diameter * 0.5), 0, topOffset)
        footer.Position = UDim2.new(0, 0, 1, -14)
        footer.Size = UDim2.new(1, 0, 0, 14)

        if self._radarRangeRing then
            local ringSize = math.max(24, math.floor(diameter * 0.52))
            self._radarRangeRing.Size = UDim2.fromOffset(ringSize, ringSize)
            self._radarRangeRing.Position = UDim2.new(0.5, 0, 0.5, 0)
        end
    end

    function Library:_updateRadarDisplay()
        local gui = self._radar
        local frame = self._radarFrame
        local viewport = self._radarViewport
        if not (gui and gui.Parent and frame and viewport) then
            return
        end

        self:_updateRadarLayout()

        if self._radarOuterStroke then self._radarOuterStroke.Color = Theme.Stroke end
        if self._radarAccentStroke then self._radarAccentStroke.Color = Theme.Accent end
        if self._radarTitleStroke then self._radarTitleStroke.Color = Theme.Stroke end
        if self._radarViewportStroke then self._radarViewportStroke.Color = Theme.Stroke end
        if self._radarTitleBar then self._radarTitleBar.BackgroundColor3 = Theme.Panel end
        if self._radarBody then self._radarBody.BackgroundColor3 = Theme.Bg end
        if self._radarViewport then self._radarViewport.BackgroundColor3 = Theme.Panel end
        if self._radarCenterDot then self._radarCenterDot.BackgroundColor3 = Theme.Text end
        if self._radarHeadingLine then self._radarHeadingLine.BackgroundColor3 = Theme.Text end
        if self._radarCrossX then self._radarCrossX.BackgroundColor3 = Theme.Stroke end
        if self._radarCrossY then self._radarCrossY.BackgroundColor3 = Theme.Stroke end
        if self._radarRangeRingStroke then self._radarRangeRingStroke.Color = Theme.Accent end
        if self._radarTitleLabel then self._radarTitleLabel.TextColor3 = Theme.Text end
        if self._radarFooterLabel then self._radarFooterLabel.TextColor3 = Theme.SubText end

        local localPlayer, localRoot = self:_getRadarLocalRoot()
        if not (localPlayer and localRoot) then
            self:_hideRadarDots()
            if self._radarTitleLabel then self._radarTitleLabel.Text = "Radar" end
            if self._radarFooterLabel then self._radarFooterLabel.Text = "Waiting for character" end
            return
        end
        ...
        local radius = math.max(12, math.min(viewportSize.X, viewportSize.Y) * 0.5 - 8)
        local range = math.max(60, tonumber(self._radarRange) or 180)
        ...
        for _, player in ipairs(players:GetPlayers()) do
            if player ~= localPlayer then
                ...
                local dot = self:_ensureRadarDot(viewport, player)
                local character = player.Character
                local humanoid = character and character:FindFirstChildOfClass("Humanoid")
                local root = character and (character.PrimaryPart or character:FindFirstChild("HumanoidRootPart"))

                if humanoid and humanoid.Health > 0 and root then
                    local relative = self:_getRadarRelativePosition(root.Position, localRoot)
                    if relative then
                        local offset = Vector2.new(relative.X * scale, relative.Y * scale)
                        local limit = math.max(0, radius - 6)
                        local dotSize = 6
                        if offset.Magnitude > limit and offset.Magnitude > 0 then
                            offset = offset.Unit * limit
                            dotSize = 4
                        end

                        dot.Size = UDim2.fromOffset(dotSize, dotSize)
                        dot.Position = UDim2.fromOffset(math.floor(center.X + offset.X + 0.5), math.floor(center.Y + offset.Y + 0.5))
                        dot.BackgroundColor3 = self:_getRadarDotColor(player, humanoid)
                        dot.Visible = true
        ...
        self:_cleanupRadarDots(activePlayers)

        if self._radarTitleLabel then self._radarTitleLabel.Text = "Radar" end
        if self._radarFooterLabel then self._radarFooterLabel.Text = tostring(range) .. " studs  |  " .. tostring(visibleCount) .. " visible" end
    end

    function Library:_createRadar()
        if self._radar then
        ...
        local viewport = Create("Frame", {
            ...
            Position = UDim2.new(0.5, -77, 0, 4),
            BackgroundColor3 = Theme.Panel,
            BorderSizePixel = 0,
            ClipsDescendants = true,
            Parent = body
        })
        Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = viewport})
        local viewportStroke = Create("UIStroke", {
            Color = Theme.Stroke,
            Thickness = 1,
            Transparency = 0.15,
            Parent = viewport
        ...
        Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = rangeRing})
        local rangeRingStroke = Create("UIStroke", {
            Color = Theme.Accent,
            Thickness = 1,
            Transparency = 0.75,
            Parent = rangeRing
        ...
        Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = centerDot})

        local headingLine = Create("Frame", {
            Name = "HeadingLine",
        ...
        Create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = headingLine})
        ...
        pcall(function() footerLabel:SetAttribute("DendriteNoTranslate", true) end)

        pcall(function()
            local rp = Library and Library._radarPosition
            if rp and type(rp) == "table" then
                mainFrame.Position = UDim2.new(rp.X or 0, rp.XOffset or 0, rp.Y or 0, rp.YOffset or 0)
            end
            local rs = Library and Library._radarSize
            if rs then
                if typeof and typeof(rs) == "UDim2" then
                    mainFrame.Size = rs
                elseif type(rs) == "table" then
                    if rs.__t == "ud2" and type(rs.x) == "table" and type(rs.y) == "table" then
                        mainFrame.Size = UDim2.new(rs.x[1] or 0, rs.x[2] or 0, rs.y[1] or 0, rs.y[2] or 0)
                    elseif rs.X and rs.Y then
                        mainFrame.Size = UDim2.new(rs.X or 0, rs.XOffset or 0, rs.Y or 0, rs.YOffset or 0)
                    end
                end
            end
        end)

        self:_setupKeybindListDragging(mainFrame, outerStroke, "_radarPosition")
        ...
        do
            ...
            self:_connect(UserInputService.InputChanged, function(input)
                ...
                local newH = math.max(minH, math.floor(startSize.Y + dy))
                mainFrame.Size = UDim2.fromOffset(newW, newH)
                Library._radarSize = mainFrame.Size
                self:_updateRadarLayout()
            end)
        ...
        self._radarFrame = mainFrame
        self._radarBody = body
        self._radarViewport = viewport
        self._radarOuterStroke = outerStroke
        self._radarAccentStroke = accentStroke
        self._radarTitleBar = titleBar
        self._radarTitleStroke = titleStroke
        self._radarTitleLabel = titleLabel
        self._radarViewportStroke = viewportStroke
        self._radarRangeRing = rangeRing
        self._radarRangeRingStroke = rangeRingStroke
        self._radarCrossX = crossX
        self._radarCrossY = crossY
        self._radarCenterDot = centerDot
        self._radarHeadingLine = headingLine
        self._radarFooterLabel = footerLabel

        self:_updateRadarDisplay()
        self._radar.Enabled = self._radarVisible

        task.spawn(function()
            while self._radar and self._radar.Parent do
                task.wait(0.03)
                if self._radarVisible and self._radar.Enabled then
                    self:_updateRadarDisplay()
                else
                    self:_hideRadarDots()
                end
    ...
    function Library:SetRadarVisibility(visible)
        self._radarVisible = (visible == true)

        if self._radarVisible then
            if not self._radar or not self._radar.Parent then
                self:_createRadar()
            elseif self._radar then
                self._radar.Enabled = true
            end
        else
            if self._radar and self._radar.Parent then
                self._radar.Enabled = false
            end
            self:_hideRadarDots()
        end
    end

    function Library:GetRadarVisibility()
        return self._radarVisible
    end

    function Library:ToggleRadar()
        self:SetRadarVisibility(not self._radarVisible)
    end
end
...
function Library:CreateLibrary(opts)
    ...
    local function filePath(name) return configsFolder() .. "/" .. sanitize(name) .. ".json" end
        ...
        function Config.Save(name, data)
            ...
            pcall(function()
                ...
                do
                    local rf = nil
                    if Library and Library._radar then rf = Library._radar:FindFirstChild("MainFrame") end
                    if rf then
                        local rpos = rf.Position
                        local rsiz = rf.Size
                        pcall(function()
                            libraryInfo.radar = libraryInfo.radar or {}
                            libraryInfo.radar.position = { X = rpos.X.Scale or 0, XOffset = rpos.X.Offset or 0, Y = rpos.Y.Scale or 0, YOffset = rpos.Y.Offset or 0 }
                            libraryInfo.radar.size = serialize(rsiz)
                        end)
                    elseif Library and Library._radarPosition then
                        local r = Library._radarPosition
                        pcall(function()
                            libraryInfo.radar = { position = { X = r.X or 0, XOffset = r.XOffset or 0, Y = r.Y or 0, YOffset = r.YOffset or 0 } }
                            if Library._radarSize then libraryInfo.radar.size = serialize(Library._radarSize) end
                        end)
                    end
                end
                ...
                layout.radar = clone(libraryInfo.radar)
    ...
    function Category:AddSection(p)
            ...
            function Page:AddPage(g)
                ...
                if g.AutoRotateToggle == true then
                    ...
                    local shellCorner = Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = shell})
                    ...
                    local recessCorner = Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = recess})
                    ...
                    local accentCorner = Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = accentFill})
                    pcall(function() accentFill:SetAttribute("DendriteNoBgImage", true) end)
                    ...
                    local knobCorner = Create("UICorner", {CornerRadius = UDim.new(1,0), Parent = knob})
                    local shellStroke = Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.45, Parent = shell})
                    local shellHighlight = Create("Frame", {BackgroundColor3 = Color3.fromRGB(255,255,255), BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1, -6, 0, 1), Position = UDim2.fromOffset(3, 2), Parent = shell})
                    local highlightCorner = Create("UICorner", {CornerRadius = UDim.new(0, 4), Parent = shellHighlight})

                    local function autoToggleMetrics()
                        if Library and Library.GetRoundToggles and Library:GetRoundToggles() then
                            return { corner = UDim.new(1, 0), knobSize = UDim2.fromOffset(10, 10), offX = 3, onX = -13 }
                        end
                        return { corner = UDim.new(0, 4), knobSize = UDim2.fromOffset(8, 8), offX = 3, onX = -11 }
                    end

                    local function isRoundToggleStyle()
                        return Library and Library.GetRoundToggles and Library:GetRoundToggles()
                    end

                    local function autoTogglePalette(isOn)
                        if isOn then
                            return Color3.fromRGB(50,50,50), Theme.Bg, Theme.Accent, Theme.Text, Theme.Stroke, 1, 0
                        end
                        return isRoundToggleStyle() and Theme.Bg or Color3.fromRGB(50,50,50), Theme.Bg, Theme.Accent, Theme.Scrollbar, Theme.Stroke, 1, 1
                    end

                    local function applyAutoToggleStyle(immediate)
                        local m = autoToggleMetrics()
                        shellCorner.CornerRadius = m.corner
                        recessCorner.CornerRadius = m.corner
                        accentCorner.CornerRadius = m.corner
                        highlightCorner.CornerRadius = m.corner
                        knobCorner.CornerRadius = UDim.new(1,0)
                        local targetPos = arVal and UDim2.new(1, m.onX, 0.5, 0) or UDim2.new(0, m.offX, 0.5, 0)
                        local track, recessColor, accentColor, knobColor, strokeColor, highlightT, accentT = autoTogglePalette(arVal)
                        local recessT = isRoundToggleStyle() and 1 or 0
                        if immediate then
                            knob.Size = m.knobSize
                            knob.Position = targetPos
                            shell.BackgroundColor3 = track
                            recess.BackgroundColor3 = recessColor
                            recess.BackgroundTransparency = recessT
                            accentFill.BackgroundColor3 = accentColor
                            accentFill.BackgroundTransparency = accentT
                            knob.BackgroundColor3 = knobColor
                            shellStroke.Color = strokeColor
                            shellHighlight.BackgroundTransparency = highlightT
                        else
                            T(knob, 0.16, {Size = m.knobSize, Position = targetPos, BackgroundColor3 = knobColor}):Play()
                            T(shell, 0.16, {BackgroundColor3 = track}):Play()
                            T(recess, 0.16, {BackgroundColor3 = recessColor, BackgroundTransparency = recessT}):Play()
                            T(accentFill, 0.22, {BackgroundColor3 = accentColor, BackgroundTransparency = accentT}):Play()
                            T(shellStroke, 0.2, {Color = strokeColor}):Play()
                            T(shellHighlight, 0.18, {BackgroundTransparency = highlightT}):Play()
                        end
                    end
                    ...
                    if Library and Library._registerToggleStyleListener then
                        Library:_registerToggleStyleListener(function(_, immediate)
                            applyAutoToggleStyle(immediate)
                        end)
                    else
                        applyAutoToggleStyle(true)
                    end
                    setAuto(false, true)
                ...
                function Group:AddToggle(o)
                    ...
                    local knobCorner = Create("UICorner", {CornerRadius = UDim.new(1,0), Parent = knob})
                    local shellStroke = Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.45, Parent = shell})
                    local shellHighlight = Create("Frame", {BackgroundColor3 = Color3.fromRGB(255,255,255), BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1, -8, 0, 1), Position = UDim2.fromOffset(4, 3), Parent = shell})
                    local highlightCorner = Create("UICorner", {CornerRadius = UDim.new(0,4), Parent = shellHighlight})

                    local function toggleMetrics()
                        if Library and Library.GetRoundToggles and Library:GetRoundToggles() then
                            return { corner = UDim.new(1, 0), knobSize = UDim2.fromOffset(14, 14), offX = 3, onX = -17 }
                        end
                        return { corner = UDim.new(0, 4), knobSize = UDim2.fromOffset(12, 12), offX = 4, onX = -16 }
                    end
                    ...
                    local function togglePalette(isOn)
                        if isOn then
                            return Color3.fromRGB(50,50,50), Theme.Bg, Theme.Accent, Theme.Text, Theme.Stroke, 1, 0
                        end
                        return isRoundToggleStyle() and Theme.Bg or Color3.fromRGB(50,50,50), Theme.Bg, Theme.Accent, Theme.Scrollbar, Theme.Stroke, 1, 1
                    ...
                    local Toggle
                    local function applyToggleStyle(immediate)
                        local m = toggleMetrics()
                        shellCorner.CornerRadius = m.corner
                        recessCorner.CornerRadius = m.corner
                        accentCorner.CornerRadius = m.corner
                        highlightCorner.CornerRadius = m.corner
                        knobCorner.CornerRadius = UDim.new(1,0)
                        local cur = (Toggle and Toggle._value == true) and true or false
                        local targetPos = cur and UDim2.new(1, m.onX, 0.5, 0) or UDim2.new(0, m.offX, 0.5, 0)
                        local track, recessColor, accentColor, knobColor, strokeColor, highlightT, accentT = togglePalette(cur)
                        local recessT = isRoundToggleStyle() and 1 or 0
                        if immediate then
                            knob.Size = m.knobSize
                            knob.Position = targetPos
                    ...
                    Toggle = { id = id, _value = default, _signal = Signal(), _row = row }
                    ...
                    container.MouseEnter:Connect(function()
                        local hoverColor = isRoundToggleStyle() and Theme.Bg:Lerp(Color3.fromRGB(255,255,255), 0.06) or Color3.fromRGB(58,58,58)
                        T(shell,0.15,{BackgroundColor3 = hoverColor}):Play()
                    end)
                ...
                function Group:AddDropdown(...)
                    ...
                    local listItemGap = 4
                    local textService = SRV("TextService")
                    local optionTextSize = isMobile and 11 or 12
                    local maxVisible = 7
                    local open = false
                    local anim = false
                    local conns = {}
                    local activeIndex = 1
                    local searchUpdateVersion = 0
                    local lastAppliedSearch = nil
                    local Dropdown = { id = id, _value = default, _values = {}, _options = {}, _filtered = {}, _itemMap = {}, _onOpen = nil, _onClose = nil, _multi = isMulti, _clickGate = false, _emptyLabel = nil }
                    ...
                    local function normalizedSearchTerm()
                        if not (enableSearch and searchBox) then return "" end
                    ...
                    local function source()
                        if normalizedSearchTerm() ~= "" then
                            return Dropdown._filtered
                        end
                        return Dropdown._options
                    end
                    local function clamp(v, lo, hi) if v < lo then return lo elseif v > hi then return hi else return v end end
                    local function getOptionTextWidth()
                        local triggerWidth = trigger.AbsoluteSize.X
                        if triggerWidth <= 0 then
                            local rootWidth = root.AbsoluteSize.X
                            triggerWidth = math.max(120, math.floor((rootWidth - 24) * (1 - labelWidth)) - 4)
                        end
                        return math.max(72, triggerWidth - (innerPad * 2) - 40)
                    end
                    local function measureOptionTextHeight(textValue)
                        if not isMobile then return itemHeight end
                        local bounds = textService:GetTextSize(tostring(textValue or ""), optionTextSize, Fonts.Medium, Vector2.new(getOptionTextWidth(), math.huge))
                        return math.max(itemHeight, bounds.Y + 8)
                    end
                    local function layoutOptionText(item, textLabel, displayText)
                        if not (item and textLabel) then return itemHeight end
                        local optionHeight = measureOptionTextHeight(displayText)
                        item.Size = UDim2.new(1,0,0,optionHeight)
                        if isMobile then
                            local textHeight = math.max(optionHeight - 8, textService:GetTextSize(tostring(displayText or ""), optionTextSize, Fonts.Medium, Vector2.new(getOptionTextWidth(), math.huge)).Y)
                            textLabel.Size = UDim2.new(1,-24,0,textHeight)
                            textLabel.Position = UDim2.fromOffset(22, math.max(0, math.floor((optionHeight - textHeight) * 0.5)))
                    ...
                    local function getRenderedListHeight()
                        return Dropdown._visibleListHeight or itemHeight
                    end
                    local function getTargetPanelHeight()
                        local searchH = enableSearch and (24 + innerPad) or 0
                        local listH = getRenderedListHeight()
                        return math.max(searchH + listH + innerPad, (innerPad * 2 + itemHeight))
                    end
                    ...
                    local function buildPanel()
                        ...
                        listFrame = Create("ScrollingFrame", {BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.fromOffset(innerPad, y), Size = UDim2.new(1, -innerPad*2, 1, -y - innerPad), CanvasSize = UDim2.fromOffset(0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollingDirection = Enum.ScrollingDirection.Y, ScrollBarThickness = 2, ScrollBarImageColor3 = Theme.Scrollbar, ZIndex = panel.ZIndex + 1, Parent = panel}, {
                            Create("UIListLayout", {Padding = UDim.new(0,listItemGap), SortOrder = Enum.SortOrder.LayoutOrder})
                        })
                    end
                    ...
                    local function renderItems()
                        local lang = Library and Library._translationLang
                        local activeQuery = normalizedSearchTerm()
                        local doTranslate = (lang and lang ~= "" and lang ~= "en" and activeQuery == "")
                        local cache = Library._translationCache or {}
                        Library._translationCache = cache
                        ...
                        if Dropdown._emptyLabel and Dropdown._emptyLabel.Parent then
                            Dropdown._emptyLabel:Destroy()
                        end
                        Dropdown._emptyLabel = nil
                        ...
                        Dropdown._itemMap = {}
                        local src = source()
                        local visibleCount = math.min(#src, maxVisible)
                        ...
                        for _, val in ipairs(src) do
                            ...
                            local itemRowHeight = layoutOptionText(item, text, shownText)
                            if visibleCount > 0 and _ <= visibleCount then
                                if visibleHeight > 0 then visibleHeight = visibleHeight + listItemGap end
                                visibleHeight = visibleHeight + itemRowHeight
                            end
                            if doTranslate then
                                local k = origText .. "|" .. tostring(lang)
                                if not cache[k] then
                                    refs[origText] = refs[origText] or {}
                                    table.insert(refs[origText], {label = text, item = item})
                                end
                            end
                        ...
                        Dropdown._visibleListHeight = (visibleHeight > 0 and visibleHeight) or itemHeight
                                ...
                                for orig, out in pairs(results or {}) do
                                    ...
                                    if list then
                                        for _, ref in ipairs(list) do
                                            local lbl = ref and ref.label
                                            local item = ref and ref.item
                                            if lbl and lbl.Parent then
                                                pcall(function() lbl.Text = v end)
                                                pcall(function() layoutOptionText(item, lbl, v) end)
                                            end
                                        end
                                    end
                                end
                                task.defer(function()
                                    if open and panel and panel.Parent and panel.Visible then
                                        local targetH = getTargetPanelHeight()
                        ...
                        if #src == 0 then
                            ...
                            Dropdown._visibleListHeight = itemHeight
                        end
                    ...
                    local function openPanel()
                        ...
                        if enableSearch and searchBox then
                            searchUpdateVersion = 0
                            lastAppliedSearch = nil
                            table.insert(conns, searchBox:GetPropertyChangedSignal("Text"):Connect(function()
                                searchUpdateVersion = searchUpdateVersion + 1
                                local myVersion = searchUpdateVersion
                                task.delay(0.09, function()
                                    if myVersion ~= searchUpdateVersion then return end
                                    if not (open and panel and panel.Visible and searchBox and searchBox.Parent) then return end
                                    local q = normalizedSearchTerm()
                                    if q == lastAppliedSearch then return end
                                    lastAppliedSearch = q
                                    Dropdown._filtered = {}
                                    if q ~= "" then
                                        for _, v in ipairs(Dropdown._options) do
                                            if tostring(v):lower():find(q,1,true) then
                                                table.insert(Dropdown._filtered, v)
                                            end
                                        end
                                    end
                                    activeIndex = 1
                        ...
                        table.insert(conns, UserInputService.InputBegan:Connect(function(input)
                            ...
                            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                                ...
                                if input.UserInputType == Enum.UserInputType.Touch then
                                    local ok, guiAtPos = pcall(function()
                                        return SRV("GuiService"):GetGuiObjectsAtPosition(pos.X, pos.Y)
                                    end)
                                    if ok and type(guiAtPos) == "table" then
                                        for _, guiObj in ipairs(guiAtPos) do
                                            if guiObj and panel and guiObj:IsDescendantOf(panel) then insideP = true end
                                            if guiObj and trigger and guiObj:IsDescendantOf(trigger) then insideB = true end
                                            if guiObj and ctxMenu and ctxMenu.Parent and guiObj:IsDescendantOf(ctxMenu) then insideC = true end
                                        end
                                    end
                                end
                                ...
                                if not insideP then
                                    ...
                                    insideP = pos.X >= p.X and pos.X <= p.X+ps.X and pos.Y >= p.Y and pos.Y <= p.Y+ps.Y
                                end
                                if not insideB then
                                    local b = trigger.AbsolutePosition; local bs = trigger.AbsoluteSize
                                    insideB = pos.X >= b.X and pos.X <= b.X+bs.X and pos.Y >= b.Y and pos.Y <= b.Y+bs.Y
                                end
                                if not insideC and ctxMenu and ctxMenu.Parent then
                ...
                function Group:AddTextbox(o)
                    ...
                    local box = Create("TextBox", {BackgroundColor3 = Theme.Button, Size = UDim2.new(0.58,-4,1,0), Position = UDim2.new(0.42,4,0,0), ClearTextOnFocus = false, PlaceholderText = placeholder, Text = default, Font = Fonts.Regular, TextSize = 13, TextColor3 = Theme.Text, PlaceholderColor3 = Theme.SubText, TextTruncate = Enum.TextTruncate.AtEnd, ClipsDescendants = true, Parent = row}, {
                ...
                function Group:AddDivider(o)
                    o = o or {}
                    local text = o.Text
                    if text == nil or tostring(text) == "" then text = nil end
                    local rowHeight = 28
                    local row = Create("Frame", {
                        BackgroundTransparency = 1,
                        Size = UDim2.new(1, -2, 0, rowHeight),
                        Position = UDim2.fromOffset(0, nextY(rowHeight)),
                        Parent = gFrame
                    })
                    if not text then
                        ...
                        Create("UICorner", {CornerRadius = UDim.new(1, 1), Parent = line})
                    else
                        local textFont = Fonts.Medium
                        local textSize = 13
                        local padding = 12
                        local textLabel = Create("TextLabel", {
                            BackgroundTransparency = 1,
                        ...
                        local ts = SRV("TextService")
                        local textWidth = ts:GetTextSize(tostring(text), textSize, textFont, Vector2.new(1000, 18)).X
                        textLabel.Size = UDim2.new(0, textWidth + padding*2, 0, 18)
                        local leftLine = Create("Frame", {
                            BackgroundColor3 = Theme.Accent,
                            BackgroundTransparency = 0.25,
                            BorderSizePixel = 0,
                            Size = UDim2.new(0.5, -(textWidth/2 + padding), 0, 2),
                            Position = UDim2.new(0, 0, 0.5, 0),
                            AnchorPoint = Vector2.new(0, 0.5),
                            Parent = row
                        })
                        Create("UICorner", {CornerRadius = UDim.new(1, 1), Parent = leftLine})
                        local rightLine = Create("Frame", {
                            BackgroundColor3 = Theme.Accent,
                            BackgroundTransparency = 0.25,
                            BorderSizePixel = 0,
                            Size = UDim2.new(0.5, -(textWidth/2 + padding), 0, 2),
                            Position = UDim2.new(0.5, (textWidth/2 + padding), 0.5, 0),
                            AnchorPoint = Vector2.new(0, 0.5),
                            Parent = row
                        })
                        Create("UICorner", {CornerRadius = UDim.new(1, 1), Parent = rightLine})
                    end
                    local dividerCount = Group._dividerCount or 0
                    dividerCount = dividerCount + 1
                    Group._dividerCount = dividerCount
                    local id = o.Flag and tostring(o.Flag) or ("%s/%s/%s/%s/DIVIDER_%d"):format(Window.Name, Category.Name, Page.Name, Group.Name, dividerCount)
                    local Divider = { id = id, _row = row, _text = text }
                    function Divider:Get() return self._text end
                    function Divider:Set(v)
                        self._text = v
                    end
                    Library:_registerControl(Divider)
                    table.insert(Group._controls, Divider)
                    return Divider
                end
                function Group:AddColorpicker(o)
                    ...
                    local function enablePulse(i)
                        ...
                        slot.pulseHook = rainbowAdd(function(h)
                            ...
                            local pulseT = 0.35 + 0.65 * (0.5 + 0.5 * math.sin(dt * 4))
                            local c = luvPulse(hh, s, baseV, pulseT)
                            slot.hsv = {hh, s, baseV}
                            slot.color = c
                            slot.fill.BackgroundColor3 = c
                        ...
                        if doSync and Library._globalPulseSync and Library._rainbowBus and Library._rainbowBus.h then
                            ...
                            local dt = (Library._rainbowBus and Library._rainbowBus.dt) or tick()
                            local pulseT = 0.35 + 0.65 * (0.5 + 0.5 * math.sin(dt * 4))
                            local c0 = luvPulse(hh, s, baseV, pulseT)
                            slot.color = c0
                            slot.fill.BackgroundColor3 = c0
                            bus.color = c0
                            if type(cb) == "function" then pcall(cb, c0, i) end
                ...
                function Group:AddKeybindToggle(o)
                    ...
                    local function keybindToggleMetrics()
                        if Library and Library.GetRoundToggles and Library:GetRoundToggles() then
                            return { corner = UDim.new(1, 0), knobSize = UDim2.fromOffset(14, 14), offX = 3, onX = -17 }
                        end
                        return { corner = UDim.new(0, 4), knobSize = UDim2.fromOffset(12, 12), offX = 4, onX = -16 }
                    ...
                    local function keybindTogglePalette(isOn)
                        if isOn then
                            return Color3.fromRGB(50,50,50), Theme.Bg, Theme.Accent, Theme.Text, Theme.Stroke, 1, 0
                        end
                        return isRoundToggleStyle() and Theme.Bg or Color3.fromRGB(50,50,50), Theme.Bg, Theme.Accent, Theme.Scrollbar, Theme.Stroke, 1, 1
                    ...
                    local function applyKeybindToggleStyle(immediate)
                        local m = keybindToggleMetrics()
                        shellCorner.CornerRadius = m.corner
                        recessCorner.CornerRadius = m.corner
                        accentCorner.CornerRadius = m.corner
                        highlightCorner.CornerRadius = m.corner
                        ...
                        local targetPos = visualOn and UDim2.new(1, m.onX, 0.5, 0) or UDim2.new(0, m.offX, 0.5, 0)
                        local track, recessColor, accentColor, knobColor, strokeColor, highlightT, accentT = keybindTogglePalette(visualOn)
                        if immediate then
                            knob.Size = m.knobSize
                            knob.Position = targetPos
                            shell.BackgroundColor3 = track
                            recess.BackgroundColor3 = recessColor
                            recess.BackgroundTransparency = isRoundToggleStyle() and 1 or 0
                            accentFill.BackgroundColor3 = accentColor
                            accentFill.BackgroundTransparency = accentT
                            knob.BackgroundColor3 = knobColor
                            shellStroke.Color = strokeColor
                            ...
                            T(knob, 0.16, {Size = m.knobSize, Position = targetPos, BackgroundColor3 = knobColor}):Play()
                            T(shell, 0.16, {BackgroundColor3 = track}):Play()
                            T(recess, 0.16, {BackgroundColor3 = recessColor, BackgroundTransparency = isRoundToggleStyle() and 1 or 0}):Play()
                            T(accentFill, 0.22, {BackgroundColor3 = accentColor, BackgroundTransparency = accentT}):Play()
                            T(shellStroke, 0.2, {Color = strokeColor}):Play()
                            T(shellHighlight, 0.18, {BackgroundTransparency = highlightT}):Play()
                        end
                    ...
                    if Library and Library._registerToggleStyleListener then
                        Library:_registerToggleStyleListener(function(_, immediate)
                            applyKeybindToggleStyle(immediate)
                        end)
                    else
                        applyKeybindToggleStyle(true)
                    end
                ...
                function Group:Add3DPreviewWidget(o)
                    ...
                    local row = Create("Frame", {
                        ...
                        Create("UIPadding", {PaddingLeft = UDim.new(0, 18), PaddingRight = UDim.new(0, 6), PaddingTop = UDim.new(0, 6), PaddingBottom = UDim.new(0, 6)})
                    })
                    ...
                    local config = {
                        ...
                        ArrowOutlineColor = Color3.fromRGB(18, 20, 19),
                        ArrowRadius = 150,
                        BarPosition = "Left",
                        Color = Theme.Accent
                    }
                    ...
                    if Drawing and Drawing.new then
                        ...
                        for i = 1, 8 do
                            drawings.corners[i] = newDrawing("Line")
                        ...
                        drawings.armor = newDrawing("Square")
                        drawings.armorOutline = newDrawing("Square")
                        -- Arrow pool is created dynamically per-player
                    end

                    local function hideDrawings()
                        if drawings.box then drawings.box.Visible = false end
                        if drawings.boxOutline then drawings.boxOutline.Visible = false end
                        for i = 1, #drawings.corners do if drawings.corners[i] then drawings.corners[i].Visible = false end end
                        for i = 1, #drawings.cornerOutlines do if drawings.cornerOutlines[i] then drawings.cornerOutlines[i].Visible = false end end
                        ...
                        if drawings.armor then drawings.armor.Visible = false end
                        if drawings.armorOutline then drawings.armorOutline.Visible = false end
                        for _, ap in ipairs(drawings._arrowPool) do
                            if ap.fill then ap.fill.Visible = false end
                            if ap.outline then ap.outline.Visible = false end
                        end
                    ...
                    local function countAccessories(m)
                        if not m then return 0 end
                        local n = 0
                    ...
                    local function getCharacterRoot(char)
                        ...
                        -- Use recursive search (true) so nested custom character parts are found.
                        return char:FindFirstChild("HumanoidRootPart", true)
                            or char:FindFirstChild("Root", true)
                            or char:FindFirstChild("RootPart", true)
                            or char:FindFirstChild("Torso", true)
                            or char:FindFirstChild("UpperTorso", true)
                    end

                    local function computeAabbFromParts(parts)
                    ...
                    local function getFramingBounds(m)
                        if not m then return nil end
                        local parts = {}
                        for _, d in ipairs(m:GetDescendants()) do
                            if d:IsA("BasePart") and d.Transparency < 1 then
                                local acc = d.FindFirstAncestorOfClass and d:FindFirstAncestorOfClass("Accessory")
                                if not acc then
                                    parts[#parts + 1] = d
                                end
                            end
                        end
                        -- If no visible non-accessory parts, include ALL BaseParts as fallback
                        if #parts == 0 then
                            for _, d in ipairs(m:GetDescendants()) do
                                if d:IsA("BasePart") then
                                    parts[#parts + 1] = d
                        ...
                        local cf, size = computeAabbFromParts(parts)
                        if cf and size then
                            return cf, size
                        end
                        local bbOk, bbCf, bbSize = pcall(function()
                            return m:GetBoundingBox()
                        end)
                        if bbOk and typeof(bbCf) == "CFrame" and typeof(bbSize) == "Vector3" then
                            return bbCf, bbSize
                        end
                        return nil
                    end
                    ...
                    local function findManualCharacterModel()
                        ...
                        -- Search workspace for any instance with the matching name
                        -- that contains the required parts. Accept Model, Folder, or any Instance.
                        local bestMatch, bestParts = nil, 0
                        for _, inst in ipairs(workspace:GetDescendants()) do
                            if inst.Name == manualCharacterName and inst ~= wm and inst ~= state.model then
                                local hasAll = true
                                for i = 1, #manualCharacterParts do
                                    if not inst:FindFirstChild(manualCharacterParts[i], true) then
                                        hasAll = false
                                        break
                                    end
                                end
                                if hasAll then
                                    -- Prefer the one with the most BaseParts (most complete)
                                    local partCount = countBaseParts(inst)
                                    if partCount > bestParts then
                                        bestMatch = inst
                                        bestParts = partCount
                                    end
                                end
                    ...
                    local function findBestCharacterModel()
                        ...
                        bestScore = -1
                        for _, inst in ipairs(workspace:GetDescendants()) do
                            if inst:IsA("Model") and inst ~= wm and inst ~= state.model then
                                local hum = inst:FindFirstChildOfClass("Humanoid")
                                local head = inst:FindFirstChild("Head")
                                local root = getCharacterRoot(inst)
                                if hum and head and root and root:IsA("BasePart") then
                                    local score = 0
                                    if inst.Name == (LocalPlayer and LocalPlayer.Name) then score = score + 5 end
                                    score = score + math.min(20, countBaseParts(inst))
                                    score = score + math.min(10, countAccessories(inst) * 2)
                    ...
                    local function isCharacterReady(char)
                        ...
                        if isManualEnabled() and char.Name == manualCharacterName then
                            for i = 1, #manualCharacterParts do
                                if not char:FindFirstChild(manualCharacterParts[i], true) then
                                    return false
                                end
                            end
                        ...
                        local root = getCharacterRoot(char) or torso
                        if not (head and torso and root) then
                            return false
                        end
                        if countBaseParts(char) < 6 then
                            return false
                        end
                        return true
                    end
                        ...
                        for _, d in ipairs(clone:GetDescendants()) do
                            ...
                            elseif d:IsA("BasePart") then
                                ...
                                if d.Name == "HumanoidRootPart" then
                                    d.Transparency = 1
                                elseif d.Transparency and d.Transparency >= 1 then
                                    d.Transparency = 1
                            ...
                            elseif d:IsA("Animator") then
                                -- Stop any residual animation state from influencing Motor6D.Transform.
                                pcall(function() d:Destroy() end)
                            elseif d:IsA("Motor6D") then
                                -- Cloning a live rig can carry over non-identity Transform values from animations,
                                -- which makes parts look offset in ViewportFrames. Clear them.
                                pcall(function() d.Transform = CFrame.new() end)
                            end
                        end
                    end

                    local function setAllAnchored(model, anchored)
                    ...
                    local function getPrimaryRigRoot(model)
                        if not model then return nil end
                        -- Prefer the actual torso/root of the rig over HumanoidRootPart.
                        -- Using HRP as the root can produce odd-looking limb offsets on some rigs.
                        -- Use recursive search (true) so nested custom character parts are found.
                        return model:FindFirstChild("LowerTorso", true)
                    ...
                    local function buildJointGraph(model)
                        local motorEdges = {}
                        local weldEdges = {}
                        for _, inst in ipairs(model:GetDescendants()) do
                            if (inst:IsA("Motor6D") or inst:IsA("Weld")) and inst.Part0 and inst.Part1 then
                                local p0 = inst.Part0
                                local p1 = inst.Part1
                                local target = inst:IsA("Motor6D") and motorEdges or weldEdges
                                target[p0] = target[p0] or {}
                                target[p1] = target[p1] or {}
                                table.insert(target[p0], inst)
                                table.insert(target[p1], inst)
                            end
                        end
                        return motorEdges, weldEdges
                    end

                    local function propagateFromEdges(edges, visited, initialQueue)
                        local queue = initialQueue
                        while #queue > 0 do
                            local cur = table.remove(queue, 1)
                            local list = edges[cur]
                            if list then
                                for _, joint in ipairs(list) do
                                    ...
                                    if p0 and p1 and p0:IsA("BasePart") and p1:IsA("BasePart") then
                                        -- Constraint: p0.CFrame * C0 == p1.CFrame * C1
                                        if cur == p0 and not visited[p1] then
                                            visited[p1] = true
                                            pcall(function()
                                                p1.CFrame = p0.CFrame * joint.C0 * joint.C1:Inverse()
                                            end)
                                            table.insert(queue, p1)
                                        elseif cur == p1 and not visited[p0] then
                                            visited[p0] = true
                                            pcall(function()
                    ...
                    local function solvePoseFromJoints(model)
                        local root = getPrimaryRigRoot(model)
                        ...
                        if not hasJoints then
                            -- Just center the model around origin using its current part positions.
                            setAllAnchored(model, true)
                            return true
                        end

                        -- Ensure joints can reposition parts while we solve.
                        setAllAnchored(model, false)

                        -- Start from origin; later framing code will center/offset the entire model.
                        pcall(function() root.CFrame = CFrame.new(0, 0, 0) end)

                        local motorEdges, weldEdges = buildJointGraph(model)
                        local visited = {}
                        visited[root] = true

                        -- 1) Solve the body pose using Motor6Ds only (stable, avoids accessory influence).
                        propagateFromEdges(motorEdges, visited, { root })

                        -- 2) Then solve Weld joints for remaining parts (mostly accessories).
                        local weldQueue = {}
                        for part, _ in pairs(visited) do
                            table.insert(weldQueue, part)
                        end
                        propagateFromEdges(weldEdges, visited, weldQueue)

                        -- Freeze the solved neutral pose.
                        setAllAnchored(model, true)
                        return true
                    end
                    local function partPath(model, inst)
                        if not (model and inst) then return nil end
                        local names = {}
                        local cur = inst
                    ...
                    local function buildLivePartMap(sourceModel, cloneModel)
                        if not (sourceModel and cloneModel) then return nil end
                        local src = {}
                        for _, d in ipairs(sourceModel:GetDescendants()) do
                            if d:IsA("BasePart") then
                                local p = partPath(sourceModel, d)
                                if p then
                                    src[p] = d
                                end
                            end
                        ...
                        for _, d in ipairs(cloneModel:GetDescendants()) do
                            if d:IsA("BasePart") then
                                local p = partPath(cloneModel, d)
                                local s = p and src[p]
                                if s then
                                    map[#map + 1] = { src = s, dst = d, path = p }
                                end
                            end
                        end
                        if #map == 0 then
                    ...
                    local function buildNeutralMotorDefaultsForRig(rigType)
                        if not (LocalPlayer and Players and typeof(rigType) == "EnumItem") then
                            return nil
                        end
                        if state._neutralMotorDefaults
                            and state._neutralMotorDefaultsRigType == rigType
                            and state._neutralMotorDefaultsUserId == (LocalPlayer.UserId)
                        then
                            return state._neutralMotorDefaults
                        end
                        ...
                        local okModel, neutral = pcall(function()
                            return Players:CreateHumanoidModelFromDescription(desc, rigType)
                        end)
                        if not okModel or not neutral then
                            return nil
                        end

                        local defaults = {}
                        for _, inst in ipairs(neutral:GetDescendants()) do
                            if inst:IsA("Motor6D") and inst.Part0 and inst.Part1 then
                                -- Key includes name + connected parts to avoid collisions.
                                local key = tostring(inst.Name)
                                    .. "|" .. tostring(inst.Part0.Name)
                                    .. "|" .. tostring(inst.Part1.Name)
                                defaults[key] = { C0 = inst.C0, C1 = inst.C1 }
                            end
                        end

                        pcall(function() neutral:Destroy() end)

                        state._neutralMotorDefaults = defaults
                        state._neutralMotorDefaultsRigType = rigType
                        state._neutralMotorDefaultsUserId = LocalPlayer.UserId
                        return defaults
                    end

                    local function applyNeutralMotorDefaults(clone, defaults)
                        if not (clone and defaults) then return end
                    ...
                    local function rebuild()
                        ...
                        local clone
                        local wasManualPartClone = false

                        -- Try 1: Clone the whole instance
                        do
                            local old
                            pcall(function() old = char.Archivable end)
                            ...
                            if ok and out then
                                -- If the clone is not a Model, wrap it in one
                                if not out:IsA("Model") then
                                    local wrapper = Instance.new("Model")
                                    wrapper.Name = out.Name
                                    out.Parent = wrapper
                                    clone = wrapper
                                else
                                    clone = out
                        ...
                        -- Try 2 (manual models): clone each descendant part individually
                        if (not clone) and usedManual then
                            local newModel = Instance.new("Model")
                            newModel.Name = char.Name
                            local anyPart = false

                            -- Gather all BaseParts from the source
                            local srcParts = {}
                            ...
                            -- Also check if char itself is a BasePart
                            if char:IsA("BasePart") then
                                srcParts[#srcParts + 1] = char
                            end

                            for _, d in ipairs(srcParts) do
                                local partClone
                                -- Try clone
                                pcall(function() d.Archivable = true end)
                                local ok2, pc = pcall(function() return d:Clone() end)
                                if ok2 and pc then
                                    partClone = pc
                                else
                                    -- Manually reconstruct
                                    local okNew, newP = pcall(function()
                                        local cn = d.ClassName
                                        local np
                                        if cn == "MeshPart" then
                                            np = Instance.new("MeshPart")
                                            pcall(function() np.MeshId = d.MeshId end)
                                            pcall(function() np.TextureID = d.TextureID end)
                                        ...
                                        np.Size = d.Size
                                        np.CFrame = d.CFrame
                                        pcall(function() np.Color = d.Color end)
                                        pcall(function() np.Material = d.Material end)
                                        pcall(function() np.Transparency = d.Transparency end)
                                        np.CanCollide = false
                                        np.Anchored = true
                                        return np
                                    end)
                                    if okNew and newP then
                                        partClone = newP
                                    end
                                end
                                if partClone then
                            ...
                            -- Clone visual children (SpecialMesh, SurfaceAppearance, Decal, Texture)
                            if anyPart then
                                local srcVisuals = {}
                                for _, d in ipairs(char:GetDescendants()) do
                                    if d:IsA("SpecialMesh") or d:IsA("SurfaceAppearance") or d:IsA("Decal") or d:IsA("Texture") then
                                        srcVisuals[#srcVisuals + 1] = d
                                    end
                                end
                                for _, d in ipairs(srcVisuals) do
                                    local parentPart = d.Parent
                                    if parentPart and parentPart:IsA("BasePart") then
                                        local target = newModel:FindFirstChild(parentPart.Name)
                                        if target then
                                            pcall(function()
                                                pcall(function() d.Archivable = true end)
                                                local okC, dc = pcall(function() return d:Clone() end)
                                                if okC and dc then dc.Parent = target end
                                            end)
                                        end
                                    end
                                end
                                clone = newModel
                                wasManualPartClone = true
                            else
                                pcall(function() newModel:Destroy() end)
                            end
                        end

                        -- Try 3 (non-manual): build from HumanoidDescription
                        if (not clone) and (not usedManual) then
                            local desc
                        ...
                        -- Helper: move all parts in the model by an offset CFrame
                        local function translateAllParts(model, offset)
                            for _, d in ipairs(model:GetDescendants()) do
                                if d:IsA("BasePart") then
                                    pcall(function() d.CFrame = d.CFrame + offset end)
                                end
                            end
                            if model:IsA("BasePart") then
                                pcall(function() model.CFrame = model.CFrame + offset end)
                            end
                        end

                        local function rotateAllParts(model, center, rotation)
                            for _, d in ipairs(model:GetDescendants()) do
                                if d:IsA("BasePart") then
                                    pcall(function()
                                        local rel = d.CFrame.Position - center
                                        local newPos = rotation * rel + center
                                        d.CFrame = CFrame.new(newPos) * (rotation * d.CFrame.Rotation)
                                    end)
                                end
                            end
                        end
                        ...
                        do
                            local pp = getCharacterRoot(clone) or clone:FindFirstChild("Head", true)
                            if pp and pp:IsA("BasePart") then
                                pcall(function() clone.PrimaryPart = pp end)
                            end
                        end

                        -- Strip scripts & anchor parts first.
                        cleanupClone(clone)

                        -- Ensure the clone is assembled in a sane neutral pose before we frame it.
                        -- This prevents scattered/offset limbs on some executors/rigs.
                        pcall(function()
                            solvePoseFromJoints(clone)
                        end)

                        -- Anchor the clone so it stays in the viewport world. We'll mirror the live pose
                        -- by copying relative part CFrames every frame.
                        setAllAnchored(clone, true)

                        -- Store live source + mapping for RenderStepped mirroring.
                        state._liveSource = char
                        state._liveMap = buildLivePartMap(char, clone)
                        clone.Parent = wm

                        -- How far below the feet the model is allowed to sit in the viewport world.
                        -- Keep this very small so the character sits centered instead of low.
                        local extraDown = 0.1

                        -- Center the model at origin using actual part positions (never rely on PivotTo)
                        do
                            -- Step 1: Compute average position of ALL BaseParts (including invisible)
                            -- so we always bring the model to origin, even if visible parts are few/none.
                            local sumPos = Vector3.zero
                            local partCount = 0
                            for _, d in ipairs(clone:GetDescendants()) do
                                if d:IsA("BasePart") then
                                    sumPos = sumPos + d.CFrame.Position
                                    partCount = partCount + 1
                                end
                            end
                            if partCount > 0 then
                                translateAllParts(clone, -(sumPos / partCount))
                            end
                        ...
                        pcall(function()
                            local rp = getCharacterRoot(clone) or clone:FindFirstChild("Head", true)
                        ...
                        -- Safety net: if the model AABB center is still far from origin, force re-center.
                        -- This catches edge cases where prior steps failed silently.
                        do
                            local safeCheck = getFramingBounds(clone)
                            if safeCheck and safeCheck.Position.Magnitude > 50 then
                                local sumP = Vector3.zero
                                local cnt = 0
                                for _, d in ipairs(clone:GetDescendants()) do
                                    if d:IsA("BasePart") then
                                        sumP = sumP + d.CFrame.Position
                                        cnt = cnt + 1
                                    end
                                end
                                if cnt > 0 then
                                    translateAllParts(clone, -(sumP / cnt))
                                end
                            end
                        end
                        ...
                        state.rootPart = getCharacterRoot(clone) or clone:FindFirstChild("Head", true)
                        ...
                        local cf, size = getFramingBounds(clone)
                        if cf and size then
                            local radius = math.max(size.X, size.Y, size.Z) * 0.5
                            -- Default zoom: slightly closer than before.
                            state.dist = math.clamp(radius * 2.35, 4.5, 14)
                            state.baseDist = state.dist
                            -- Aim a bit lower so the character sits higher / more centered in-frame.
                            state.focus = Vector3.new(0, size.Y * 0.50, 0)
                        else
                            state.dist = 10
                            state.baseDist = 10
                    ...
                    local function scheduleRebuild()
                        if state._rebuilding then
                            return
                        end
                        state._rebuilding = true
                        state._rebuildToken = (state._rebuildToken or 0) + 1
                        local token = state._rebuildToken

                        task.spawn(function()
                            local attempts = 0
                            while attempts < 30 do
                                if token ~= state._rebuildToken then
                                    break
                                end
                                attempts = attempts + 1
                                    ...
                                    if ok then
                                        if (not state._accessoryRefreshUsed) and (not state._lastBuildUsedManual) then
                                            task.delay(0.8, function()
                                                ...
                                                local live = findBestCharacterModel()
                                                if not isCharacterReady(live) then return end
                                                local liveAcc = countAccessories(live)
                                                local previewAcc = countAccessories(state.model)
                                                if liveAcc > previewAcc then
                                                    state._accessoryRefreshUsed = true
                    ...
                    Library:_connect(UserInputService.InputChanged, function(input)
                        if input.UserInputType == Enum.UserInputType.MouseWheel and isMouseInside() then
                            ...
                            if delta ~= 0 then
                                state.baseDist = math.clamp((state.baseDist or state.dist) - delta * 0.8, 2.5, 22)
                    ...
                    Library:_connect(RunService.RenderStepped, function(dt)
                        ...
                        if vpAbsPos.X + vpAbsSize.X < 0 or vpAbsPos.Y + vpAbsSize.Y < 0 or vpAbsPos.X > screenSize.X or vpAbsPos.Y > screenSize.Y then
                            hideDrawings(); return
                        end
                        ...
                        -- Live pose mirroring: copy the live character's relative part CFrames onto the
                        -- preview clone so crouching/looking/etc is reflected in the viewport.
                        do
                            local srcModel = state._liveSource
                            local map = state._liveMap
                            if srcModel and map and srcModel.Parent and state.model and state.model.Parent then
                                local srcRoot = getModelRootPart(srcModel)
                                local dstRoot = getModelRootPart(state.model)
                                if srcRoot and dstRoot and srcRoot:IsA("BasePart") and dstRoot:IsA("BasePart") then
                                    local ragdolled = false
                                    local maxRelDist = 0
                                    for i = 1, math.min(#map, 5) do
                                        local pair = map[i]
                                        local s = pair and pair.src
                                        if s and s.Parent and s:IsA("BasePart") then
                                            pcall(function()
                                                local rel = srcRoot.CFrame:ToObjectSpace(s.CFrame)
                                                local d = rel.Position.Magnitude
                                                if d > maxRelDist then maxRelDist = d end
                                            end)
                                        end
                                    end
                                    if maxRelDist > 15 then ragdolled = true end
                                    if not ragdolled then
                                        for i = 1, #map do
                                            local pair = map[i]
                                            local s = pair and pair.src
                                            local d = pair and pair.dst
                                            if s and d and s.Parent and d.Parent and d ~= dstRoot then
                                                local ok = pcall(function()
                                                    local rel = srcRoot.CFrame:ToObjectSpace(s.CFrame)
                                                    d.CFrame = dstRoot.CFrame * rel
                                                end)
                                                if not ok then
                                                end
                                            end
                        ...
                        -- Compute the visible draw region (viewport intersected with root frame)
                        local vpLeft = drawBase.X
                        local vpTop = drawBase.Y
                        local vpRight = drawBase.X + absSize.X
                        local vpBottom = drawBase.Y + absSize.Y
                        pcall(function()
                            if Library._rootFrame then
                                local rfPos = Library._rootFrame.AbsolutePosition
                                local rfSize = Library._rootFrame.AbsoluteSize
                                local rfLeft = rfPos.X
                                local rfTop = rfPos.Y + insetY
                                local rfRight = rfPos.X + rfSize.X
                                local rfBottom = rfPos.Y + rfSize.Y + insetY
                                vpLeft = math.max(vpLeft, rfLeft)
                                vpTop = math.max(vpTop, rfTop)
                                vpRight = math.min(vpRight, rfRight)
                                vpBottom = math.min(vpBottom, rfBottom)
                            end
                        end)
                        if vpRight <= vpLeft or vpBottom <= vpTop then
                            hideDrawings(); return
                        end

                        -- Auto-zoom: try with baseDist first, if drawings overflow, zoom out
                        local baseDist = state.baseDist or state.dist
                        local useDist = baseDist

                        local function computeDrawRect(dist)
                            local focus = state.focus
                            local cx2 = math.cos(state.pitch)
                            local sx2 = math.sin(state.pitch)
                            local cy2 = math.cos(state.yaw)
                            local sy2 = math.sin(state.yaw)
                            local off = Vector3.new(dist * cy2 * cx2, dist * sx2, dist * sy2 * cx2)
                            cam.CFrame = CFrame.new(focus + off, focus)

                            local cf2, size2 = getFramingBounds(state.model)
                            if not cf2 or not size2 then return nil end
                            local c2d, zC = projectPoint(cf2.Position, absSize)
                            if not c2d or not zC then return nil end

                            local fv = math.rad(cam.FieldOfView)
                            local sc = (absSize.Y * 0.5) / math.tan(fv * 0.5)
                            local pps = sc / math.max(0.01, zC)

                            local dh = math.max(2, (size2.Y * pps) * 1.22)
                            local dw = math.max(2, (math.max(size2.X, size2.Z) * pps) * 1.08)
                            dw = math.max(dw, dh * 0.35)

                            local dx = drawBase.X + c2d.X - (dw * 0.5)
                            local dy = drawBase.Y + c2d.Y - (dh * 0.5)

                            -- Total bounding box including name/bars padding
                            local espTop = dy - 25
                            local espBottom = dy + dh + 12
                            local espLeft = dx - 14
                            local espRight = dx + dw + 14


                            return espLeft, espTop, espRight, espBottom, dx, dy, dw, dh, c2d, zC, cf2, size2
                        end

                        local dr = {computeDrawRect(baseDist)}
                        if not dr[1] then hideDrawings(); return end

                        local espL, espT, espR, espB = dr[1], dr[2], dr[3], dr[4]
                        local visW = vpRight - vpLeft
                        local visH = vpBottom - vpTop
                        local espW = espR - espL
                        local espH = espB - espT

                        if espW > visW or espH > visH then
                            local ratio = math.max(espW / math.max(visW, 1), espH / math.max(visH, 1))
                            useDist = math.min(baseDist * ratio, 30)
                            dr = {computeDrawRect(useDist)}
                            if not dr[1] then hideDrawings(); return end
                        end
                        ...
                        local offset = Vector3.new(useDist * cy * cx, useDist * sx, useDist * sy * cx)
                        cam.CFrame = CFrame.new(focus + offset, focus)

                        if not (Drawing and Drawing.new) then return end
                        if not (config.NameESP or config.BoxesESP or config.HealthBar or config.ArmorBar or config.Skeleton) then hideDrawings(); return end

                        local cf, size = dr[11], dr[12]
                        if not cf or not size then hideDrawings(); return end
                        local center2d, zCenter = dr[9], dr[10]
                        ...
                        local w = math.max(2, (math.max(size.X, size.Z) * pixelsPerStud) * 1.08)
                        w = math.max(w, h * 0.35)

                        local x = drawBase.X + center2d.X - (w * 0.5)
                        local y = drawBase.Y + center2d.Y - (h * 0.5)

                        local function clampVal(v, lo, hi) return math.max(lo, math.min(hi, v)) end

                        local function setSquare(sq, sx, sy, sw, sh, col, filled, thick, zidx)
                            local l = clampVal(sx, vpLeft, vpRight)
                            local t = clampVal(sy, vpTop, vpBottom)
                            local r = clampVal(sx + sw, vpLeft, vpRight)
                            local b = clampVal(sy + sh, vpTop, vpBottom)
                            local cw = math.max(r - l, 1)
                            local ch = math.max(b - t, 1)
                            sq.Visible = true
                            sq.Color = col
                            sq.Filled = filled
                            sq.Thickness = thick
                            sq.Size = Vector2.new(cw, ch)
                            sq.Position = Vector2.new(l, t)
                            pcall(function() sq.ZIndex = zidx end)
                        end

                        local function setLine(ln, x1, y1, x2, y2, col, thick, zidx)
                            ln.From = Vector2.new(clampVal(x1, vpLeft, vpRight), clampVal(y1, vpTop, vpBottom))
                            ln.To = Vector2.new(clampVal(x2, vpLeft, vpRight), clampVal(y2, vpTop, vpBottom))
                            ln.Color = col
                            ln.Thickness = thick
                            ln.Visible = true
                            pcall(function() ln.ZIndex = zidx end)
                        end
                        ...
                        local function hideDefaultBox()
                            if drawings.box then drawings.box.Visible = false end
                            if drawings.boxOutline then drawings.boxOutline.Visible = false end
                        end

                        if config.BoxesESP then
                            if config.BoxStyle == "Corners" and #drawings.corners >= 8 then
                                hideDefaultBox()
                                local cornerLen = math.max(4, math.min(w, h) * 0.25)
                                -- TL
                                setLine(drawings.cornerOutlines[1], x, y, x + cornerLen, y, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.cornerOutlines[2], x, y, x, y + cornerLen, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.corners[1], x, y, x + cornerLen, y, config.Color, 1, 69)
                                setLine(drawings.corners[2], x, y, x, y + cornerLen, config.Color, 1, 69)
                                -- TR
                                setLine(drawings.cornerOutlines[3], x + w, y, x + w - cornerLen, y, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.cornerOutlines[4], x + w, y, x + w, y + cornerLen, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.corners[3], x + w, y, x + w - cornerLen, y, config.Color, 1, 69)
                                setLine(drawings.corners[4], x + w, y, x + w, y + cornerLen, config.Color, 1, 69)
                                -- BL
                                setLine(drawings.cornerOutlines[5], x, y + h, x + cornerLen, y + h, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.cornerOutlines[6], x, y + h, x, y + h - cornerLen, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.corners[5], x, y + h, x + cornerLen, y + h, config.Color, 1, 69)
                                setLine(drawings.corners[6], x, y + h, x, y + h - cornerLen, config.Color, 1, 69)
                                -- BR
                                setLine(drawings.cornerOutlines[7], x + w, y + h, x + w - cornerLen, y + h, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.cornerOutlines[8], x + w, y + h, x + w, y + h - cornerLen, Color3.fromRGB(0,0,0), 3, 1)
                                setLine(drawings.corners[7], x + w, y + h, x + w - cornerLen, y + h, config.Color, 1, 69)
                                setLine(drawings.corners[8], x + w, y + h, x + w, y + h - cornerLen, config.Color, 1, 69)
                            elseif drawings.box and drawings.boxOutline then
                                hideCorners()
                                setSquare(drawings.boxOutline, x - 1, y - 1, w + 2, h + 2, Color3.fromRGB(0, 0, 0), false, 3, 1)
                                setSquare(drawings.box, x, y, w, h, config.Color, false, 1, 69)
                            end
                        else
                            hideDefaultBox()
                            hideCorners()
                        ...
                        if config.NameESP and drawings.name then
                            local nameX = clampVal(x + (w * 0.5), vpLeft, vpRight)
                            local nameY = clampVal(y - (15 + 8), vpTop, vpBottom - 15)
                            drawings.name.Visible = true
                            drawings.name.Text = (state._lastBuildUsedManual and manualCharacterName)
                        ...
                        if config.HealthBar and drawings.hp and drawings.hpOutline then
                            ...
                            local hpColor = Color3.fromRGB(255, 0, 0):Lerp(Color3.fromRGB(0, 255, 0), hpPerc)
                            local bp = config.BarPosition
                            if bp == "Right" then
                                local hpH = math.max(1, math.floor((h - 2) * hpPerc))
                                setSquare(drawings.hpOutline, x + w + 3, y, 3, h, Color3.fromRGB(0, 0, 0), true, 1, 1)
                                setSquare(drawings.hp, x + w + 4, y + (h - hpH), 1, hpH, hpColor, true, 1, 69)
                            elseif bp == "Bottom" then
                                local hpW = math.max(1, math.floor((w - 2) * hpPerc))
                                setSquare(drawings.hpOutline, x, y + h + 3, w, 3, Color3.fromRGB(0, 0, 0), true, 1, 1)
                                setSquare(drawings.hp, x + 1, y + h + 4, hpW, 1, hpColor, true, 1, 69)
                            else
                                local hpH = math.max(1, math.floor((h - 2) * hpPerc))
                                setSquare(drawings.hpOutline, x - 6, y, 3, h, Color3.fromRGB(0, 0, 0), true, 1, 1)
                                setSquare(drawings.hp, x - 5, y + (h - hpH), 1, hpH, hpColor, true, 1, 69)
                            end
                        else
                            if drawings.hp then drawings.hp.Visible = false end
                            if drawings.hpOutline then drawings.hpOutline.Visible = false end
                        ...
                        if config.ArmorBar and drawings.armor and drawings.armorOutline then
                            local armorPerc = 0
                            if armorState.valueCallback and type(armorState.valueCallback) == "function" then
                                local okCb, cbVal, cbMax = pcall(armorState.valueCallback)
                                if okCb then
                                    local av = tonumber(cbVal) or 0
                                    local am = tonumber(cbMax) or 100
                                    armorPerc = math.clamp(av / math.max(am, 1), 0, 1)
                                end
                            elseif armorState.valuePath and type(armorState.valuePath) == "string" and armorState.valuePath ~= "" then
                                local okPath, pathVal = pcall(function()
                                    local obj = game
                                    for seg in string.gmatch(armorState.valuePath, "[^%.]+") do
                                        obj = obj[seg]
                                    end
                                    return tonumber(obj.Value or obj)
                                end)
                                local maxVal = 100
                                if armorState.maxValuePath and type(armorState.maxValuePath) == "string" and armorState.maxValuePath ~= "" then
                                    local okM, mVal = pcall(function()
                                        local obj = game
                                        for seg in string.gmatch(armorState.maxValuePath, "[^%.]+") do
                                            obj = obj[seg]
                                        end
                                        return tonumber(obj.Value or obj)
                                    end)
                                    if okM and mVal then maxVal = mVal end
                                end
                                if okPath and pathVal then
                                    armorPerc = math.clamp(pathVal / math.max(maxVal, 1), 0, 1)
                                end
                            elseif armorState.valueSet then
                                local av = tonumber(armorState.value) or 0
                                local am = tonumber(armorState.maxValue) or 100
                                armorPerc = math.clamp(av / math.max(am, 1), 0, 1)
                            else
                                local armorFlag = Library.Flags and Library.Flags["ArmorBarValue"]
                                local armorMaxFlag = Library.Flags and Library.Flags["ArmorBarMaxValue"]
                                if armorFlag ~= nil then
                                    local av = tonumber(armorFlag) or 0
                                    local am = tonumber(armorMaxFlag) or 100
                                    armorPerc = math.clamp(av / math.max(am, 1), 0, 1)
                                end
                            end

                            local armorColor = Color3.fromRGB(70, 150, 255):Lerp(Color3.fromRGB(30, 90, 200), 1 - armorPerc)
                            local bp = config.BarPosition
                            if bp == "Right" then
                                local armorBarX = x + w + 3 + 6
                                local armorH = math.max(1, math.floor((h - 2) * armorPerc))
                                setSquare(drawings.armorOutline, armorBarX, y, 3, h, Color3.fromRGB(0, 0, 0), true, 1, 1)
                                setSquare(drawings.armor, armorBarX + 1, y + (h - armorH), 1, armorH, armorColor, true, 1, 69)
                            elseif bp == "Bottom" then
                                local armorBarY = y + h + 3 + 6
                                local armorW = math.max(1, math.floor((w - 2) * armorPerc))
                                setSquare(drawings.armorOutline, x, armorBarY, w, 3, Color3.fromRGB(0, 0, 0), true, 1, 1)
                                setSquare(drawings.armor, x + 1, armorBarY + 1, armorW, 1, armorColor, true, 1, 69)
                            else
                                local armorBarX = x - 12
                                local armorH = math.max(1, math.floor((h - 2) * armorPerc))
                                setSquare(drawings.armorOutline, armorBarX, y, 3, h, Color3.fromRGB(0, 0, 0), true, 1, 1)
                                setSquare(drawings.armor, armorBarX + 1, y + (h - armorH), 1, armorH, armorColor, true, 1, 69)
                            end
                            ...
                            for idx, bone in ipairs(SKELETON_BONES) do
                                ...
                                if ln and p1 and p2 and p1:IsA("BasePart") and p2:IsA("BasePart") then
                                    ...
                                    if s1 and s2 then
                                        ln.Visible = true
                                        ln.From = Vector2.new(
                                            clampVal(drawBase.X + s1.X, vpLeft, vpRight),
                                            clampVal(drawBase.Y + s1.Y, vpTop, vpBottom)
                                        )
                                        ln.To = Vector2.new(
                                            clampVal(drawBase.X + s2.X, vpLeft, vpRight),
                                            clampVal(drawBase.Y + s2.Y, vpTop, vpBottom)
                                        )
                                        ln.Thickness = 2
                        ...
                        -- Per-player directional arrows
                        if config.Arrows then
                            local gameCam = workspace.CurrentCamera
                            local screenSz = gameCam and gameCam.ViewportSize or Vector2.new(1920, 1080)
                            local scx = screenSz.X * 0.5
                            local scy = screenSz.Y * 0.5
                            ...
                            for _, plr in ipairs(playerList) do
                                if plr ~= LocalPlayer then
                                    ...
                                    if root and root:IsA("BasePart") and gameCam then
                                        local worldPos = root.Position
                                        local screenPos, onScreen = gameCam:WorldToViewportPoint(worldPos)
                                        local sx, sy = screenPos.X, screenPos.Y

                                        -- Compute direction from screen center to player's screen pos
                                        local dx = sx - scx
                                        local dy = sy - scy
                                        local mag = math.sqrt(dx * dx + dy * dy)
                                        if mag < 1 then dx = 0; dy = -1; mag = 1 end
                                        local nx = dx / mag
                                        local ny = dy / mag

                                        -- Only show arrow when player is off-screen or behind camera
                                        local margin = 60
                                        local isOff = (not onScreen) or sx < margin or sx > screenSz.X - margin or sy < margin or sy > screenSz.Y - margin
                                        ...
                                        if isOff then
                                            ...
                                            -- Ensure pool slot exists
                                            if not drawings._arrowPool[arrowIdx] then
                                                local okF, fillTri = pcall(Drawing.new, "Triangle")
                                                local okO, outTri = pcall(Drawing.new, "Triangle")
                                                drawings._arrowPool[arrowIdx] = {
                                                    fill = (okF and fillTri) or nil,
                                                    outline = (okO and outTri) or nil
                                                }
                                                if drawings._arrowPool[arrowIdx].fill then drawings._arrowPool[arrowIdx].fill.Visible = false end
                                                if drawings._arrowPool[arrowIdx].outline then drawings._arrowPool[arrowIdx].outline.Visible = false end
                                            end

                                            local slot = drawings._arrowPool[arrowIdx]
                                            if slot.fill and slot.outline then
                                                -- Tip points toward player, base is behind
                                                local tipX = scx + nx * radius
                                                local tipY = scy + ny * radius
                                                local bx = scx + nx * (radius + arrowH)
                                                local by = scy + ny * (radius + arrowH)
                                                local perpX = -ny
                                                local perpY = nx
                                                local halfW = arrowW * 0.5

                                                local pA = Vector2.new(bx + perpX * halfW, by + perpY * halfW)
                                                local pB = Vector2.new(bx - perpX * halfW, by - perpY * halfW)
                                                local pC = Vector2.new(tipX, tipY)

                                                -- Outline (slightly larger behind)
                                                local oHalfW = halfW * oScale
                                                local oH = arrowH * oScale
                                                local oBx = scx + nx * (radius + oH)
                                                local oBy = scy + ny * (radius + oH)
                                                local oTipX = scx + nx * (radius - (oH - arrowH) * 0.25)
                                                local oTipY = scy + ny * (radius - (oH - arrowH) * 0.25)

                                                slot.outline.PointA = Vector2.new(oBx + perpX * oHalfW, oBy + perpY * oHalfW)
                                                slot.outline.PointB = Vector2.new(oBx - perpX * oHalfW, oBy - perpY * oHalfW)
                                                slot.outline.PointC = Vector2.new(oTipX, oTipY)
                                                slot.outline.Color = config.ArrowOutlineColor
                                                slot.outline.Filled = true
                                                slot.outline.Thickness = 1
                                                slot.outline.Visible = true
                                                pcall(function() slot.outline.ZIndex = 1 end)

                                                slot.fill.PointA = pA
                                                slot.fill.PointB = pB
                                                slot.fill.PointC = pC
                                                slot.fill.Color = config.ArrowColor
                                                slot.fill.Filled = true
                                                slot.fill.Thickness = 1
                                                slot.fill.Visible = true
                                                pcall(function() slot.fill.ZIndex = 69 end)
                                            end
                                        end
                                    end
                                end
                            ...
                            for i = arrowIdx + 1, #drawings._arrowPool do
                                local slot = drawings._arrowPool[i]
                                if slot then
                                    if slot.fill then slot.fill.Visible = false end
                                    if slot.outline then slot.outline.Visible = false end
                                end
                            end
                        else
                            for _, ap in ipairs(drawings._arrowPool) do
                    ...
                    table.insert(Library._3dPreviews, Preview)
                    ...
                    function Preview:SetFeature(name, v)
                        if name == "Bar position" or name == "BarPosition" then
                            if v == "Left" or v == "Right" or v == "Bottom" then
                                config.BarPosition = v
                            end
                            return
                        end
                        if name == "Box style" or name == "BoxStyle" then
                            if v == "Default" or v == "Corners" then
                                config.BoxStyle = v
                            end
                            return
                        end
                        if name == "Arrow color" or name == "ArrowColor" then
                            if typeof(v) == "Color3" then config.ArrowColor = v end
                            return
                        end
                        if name == "Arrow outline" or name == "ArrowOutlineColor" then
                            if typeof(v) == "Color3" then config.ArrowOutlineColor = v end
                            return
                        end
                        if name == "Arrow radius" or name == "ArrowRadius" then
                            local n = tonumber(v)
                            if n then config.ArrowRadius = math.clamp(n, 20, 1000) end
                            return
                        end
                        v = (v == true)
                        ...
                        elseif name == "Health bar" or name == "HealthBar" or name == "Health" then
                            config.HealthBar = v
                        elseif name == "Armor bar" or name == "ArmorBar" or name == "Armor" then
                            config.ArmorBar = v
                        elseif name == "skeleton" or name == "Skeleton" then
                            config.Skeleton = v
                        elseif name == "Arrows" or name == "arrows" then
                            config.Arrows = v
                        end
                    end
                    ...
                    function Preview:GetFeature(name)
                        ...
                        if name == "Box style" or name == "BoxStyle" then
                            return config.BoxStyle
                        end
                        if name == "Arrow color" or name == "ArrowColor" then
                            return config.ArrowColor
                        end
                        if name == "Arrow outline" or name == "ArrowOutlineColor" then
                            return config.ArrowOutlineColor
                        end
                        if name == "Arrow radius" or name == "ArrowRadius" then
                            return config.ArrowRadius
                        end
                        if name == "Name esp" or name == "NameESP" or name == "Name" then
                    ...
                    function Preview:SetArmorValue(value, maxValue)
                        armorState.valueSet = true
                        armorState.value = tonumber(value) or 0
                        armorState.maxValue = tonumber(maxValue) or 100
                    end

                    function Preview:SetArmorPath(valuePath, maxValuePath)
                        armorState.valuePath = (type(valuePath) == "string" and valuePath ~= "") and valuePath or nil
                        armorState.maxValuePath = (type(maxValuePath) == "string" and maxValuePath ~= "") and maxValuePath or nil
                    end

                    function Preview:SetArmorCallback(fn)
                        armorState.valueCallback = (type(fn) == "function") and fn or nil
                    end

                    function Preview:SetArmorInstance(valueObject, maxValueObject)
                        if typeof(valueObject) ~= "Instance" then return end
                        armorState.valueCallback = function()
                            local cur = 0
                            pcall(function() cur = tonumber(valueObject.Value) or 0 end)
                            local mx = 100
                            if maxValueObject and typeof(maxValueObject) == "Instance" then
                                pcall(function() mx = tonumber(maxValueObject.Value) or 100 end)
                            else
                                mx = math.max(cur, armorState._instanceMax or cur)
                                armorState._instanceMax = mx
                            end
                            return cur, mx
                        end
                        pcall(function()
                            armorState._instanceMax = tonumber(valueObject.Value) or 100
                        end)
                    end
                    ...
                    function Preview:Destroy()
                        ...
                        for i = 1, #drawings.corners do safeRemove(drawings.corners[i]) end
                        for i = 1, #drawings.cornerOutlines do safeRemove(drawings.cornerOutlines[i]) end
                        table.clear(drawings.corners)
                        table.clear(drawings.cornerOutlines)
                        safeRemove(drawings.name)
                        safeRemove(drawings.hp)
                        safeRemove(drawings.hpOutline)
                        safeRemove(drawings.armor)
                        safeRemove(drawings.armorOutline)
                        for _, ap in ipairs(drawings._arrowPool) do
                            safeRemove(ap.fill)
                            safeRemove(ap.outline)
                        end
                        table.clear(drawings._arrowPool)
    ...
    function Window:Add3DPreview(PreviewGroup, options)
        ...
        if integrated then
            ...
            left:AddDropdown({ Name = "Box style", Default = "Default", Options = {"Default", "Corners"}, Callback = function(v) preview:SetFeature("Box style", v) end })
            left:AddToggle({ Name = "Health bar", Default = false, Callback = function(v) preview:SetFeature("Health bar", v) end })
            left:AddToggle({ Name = "Armor bar", Default = false, Callback = function(v) preview:SetFeature("Armor bar", v) end })
            left:AddDropdown({ Name = "Bar position", Default = "Left", Options = {"Left", "Right", "Bottom"}, Callback = function(v) preview:SetFeature("Bar position", v) end })
            left:AddToggle({ Name = "skeleton", Default = false, Callback = function(v) preview:SetFeature("skeleton", v) end })
            left:AddToggle({ Name = "Arrows", Default = false, Callback = function(v) preview:SetFeature("Arrows", v) end })
            left:AddSlider({ Name = "Arrow radius", Min = 20, Max = 500, Default = 150, Step = 1, Callback = function(v) preview:SetFeature("Arrow radius", v) end })
            left:AddColorpicker({ Name = "Arrow color", Amount = 1, Default = Color3.fromRGB(0, 255, 170), Callback = function(c) preview:SetFeature("Arrow color", c) end })
            left:AddColorpicker({ Name = "Arrow outline", Amount = 1, Default = Color3.fromRGB(18, 20, 19), Callback = function(c) preview:SetFeature("Arrow outline", c) end })
        end
    ...
    Window.LoadConfig = function(a, b)
        ...
        do
            ...
            if type(lib) == 'table' then
                ...
                local loadedRadarSize, loadedRadarPos = nil, nil
                ...
                pcall(function()
                    ...
                    if lib.radar then
                        if type(lib.radar) == 'table' and lib.radar.size then
                            local rs = deserialize(lib.radar.size) or lib.radar.size
                            if rs then
                                loadedRadarSize = rs
                                pcall(function() Library._radarSize = rs end)
                            end
                        end
                        local posSrc = nil
                        if type(lib.radar) == 'table' and lib.radar.position then posSrc = lib.radar.position else posSrc = lib.radar end
                        local rp = deserialize(posSrc) or posSrc
                        if rp then
                            local rpTable = rp
                            if typeof and typeof(rp) == 'UDim2' then
                                rpTable = { X = rp.X.Scale, XOffset = rp.X.Offset, Y = rp.Y.Scale, YOffset = rp.Y.Offset }
                            elseif type(rp) == 'table' and rp.__t == 'ud2' and type(rp.x) == 'table' and type(rp.y) == 'table' then
                                rpTable = { X = tonumber(rp.x[1]) or 0, XOffset = tonumber(rp.x[2]) or 0, Y = tonumber(rp.y[1]) or 0, YOffset = tonumber(rp.y[2]) or 0 }
                            end
                            Library._radarPosition = rpTable
                            loadedRadarPos = rpTable
                        end
                    end
                ...
                local function reapplyFromCache()
                    ...
                    pcall(function()
                        local rs = loadedRadarSize
                        local rp = loadedRadarPos
                        if (rs or rp) and Library and Library._radar and Library._radar.Parent then
                            local mf = Library._radar:FindFirstChild("MainFrame")
                            if mf then
                                local convr = toUD2(rs)
                                if convr then mf.Size = convr end
                                local convrp = toUD2(rp)
                                if convrp then mf.Position = convrp end
                            end
                        end
                    end)
                    pcall(function()
                ...
                if loadedWindowSize or loadedWindowPos or loadedKeybindSize or loadedKeybindPos or loadedRadarSize or loadedRadarPos or loadedWatermark or loadedMobile then
    ...
    function Themes.Save(name, payload)
        ...
        local visualOptions = { roundToggles = (Library and Library.GetRoundToggles and Library:GetRoundToggles()) or false }
        local outObj = { theme = themePayload, fonts = fontsPayload, backgroundImage = bg, visualOptions = visualOptions, metadata = meta }
    ...
    function Themes.Load(name)
        ...
        local visualOptions = nil
        if type(dec.visualOptions) == "table" then
            visualOptions = {
                roundToggles = dec.visualOptions.roundToggles == true
            }
        end
        return { theme = themeOut, fonts = fontsOut, themeColorpickers = themeColorpickers, backgroundImage = bg, visualOptions = visualOptions, metadata = dec.metadata }
    end
    ...
    Window.LoadTheme = function(name)
        ...
        if dec and Library and Library.ApplyTheme then
            ...
            pcall(function()
                local vo = dec and dec.visualOptions
                if vo and Library and Library.SetRoundToggles then
                    Library:SetRoundToggles(vo.roundToggles == true, true)
                end
            end)
            pcall(function()
            ...
            pcall(function()
                ...
                if ui.RoundToggles and ui.RoundToggles.Set then
                    local roundOn = (Library and Library.GetRoundToggles and Library:GetRoundToggles()) or false
                    ui.RoundToggles:Set(roundOn, true)
                end
            end)
        end
        return dec
    ...
    Window.AddThemes = function(self, group, opts)
        ...
        local autoToggle, roundTogglesToggle
        ...
        local resetBtn = group:AddButton({ Name = "Reset to Default", Compact = true, Callback = function()
            ...
            task.defer(function()
                ...
                pcall(function()
                    if Library and Library.SetRoundToggles then Library:SetRoundToggles(false, true) end
                    if roundTogglesToggle and roundTogglesToggle.Set then roundTogglesToggle:Set(false, true) end
                end)
            end)
        end })
        ...
        roundTogglesToggle = markThemeControl(group:AddToggle({ Name = "Round Toggles", Default = (Library and Library.GetRoundToggles and Library:GetRoundToggles()) or false, Callback = function(on)
            pcall(function() if Library and Library.SetRoundToggles then Library:SetRoundToggles(on == true) end end)
        end }))
        ...
        local themeUI = { Dropdown = dd, NameBox = nameBox, Create = createBtn, Save = saveBtn, Load = loadBtn, Delete = deleteBtn, Refresh = refreshBtn, AutoToggle = autoToggle, RoundToggles = roundTogglesToggle, FontRegular = fr, FontMedium = fm, FontBold = fb, Colorpickers = themePickers, ColorpickersByKey = themePickersByKey, ImageBgToggle = imageBgToggle, ImageBgId = imageBgId }
