	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddToggle(ToggleConfig)
				...
				end				local ColorpickerContainer = Create("Frame", {
				...
				local RainbowCheckmark = Create("ImageLabel", {
					...
					ImageTransparency = getgenv().RainbowEnabled and 0 or 1,
					Parent = RainbowCheckbox
				})

				local RainbowLabel = Create("TextLabel", {
					Size = UDim2.new(0, 100, 0, 18),
					Position = UDim2.new(0, 58, 1, -32),
					BackgroundTransparency = 1,
					Text = "Rainbow",
				...
				AddConnection(Click.MouseButton1Click, function()
					...
					TweenService:Create(ToggleColorPickerFrame,TweenInfo.new(.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),{Size = ToggleColorPicker.Toggled and UDim2.new(1, 0, 0, 180) or UDim2.new(1, 0, 0, 38)}):Play()
				...
				local function StartRainbow()
					if getgenv().RainbowConnections[ToggleColorPicker] then
						getgenv().RainbowConnections[ToggleColorPicker]:Disconnect()
					end
					
					getgenv().RainbowConnections[ToggleColorPicker] = RunService.Heartbeat:Connect(function()
						if getgenv().RainbowEnabled then
							local hue = (tick() * 0.5) % 1
							local rainbowColor = Color3.fromHSV(hue, 1, 1)
							ToggleColorPicker:SetColor(rainbowColor)
							ColorpickerBox.BackgroundColor3 = rainbowColor
							ToggleColorPickerConfig.Callback(rainbowColor, ToggleColorPicker.Value)
						end
					end)
				end
				...
				local function StopRainbow()
					if getgenv().RainbowConnections[ToggleColorPicker] then
						getgenv().RainbowConnections[ToggleColorPicker]:Disconnect()
						getgenv().RainbowConnections[ToggleColorPicker] = nil
					end
				end

				local function UpdateAllRainbowCheckboxes()
					for _, gui in pairs(game:GetService("CoreGui"):GetDescendants()) do
						if gui.Name == "MainWindow" then
							for _, checkbox in pairs(gui:GetDescendants()) do
								if checkbox.Name == "RainbowCheckmark" then
				...
				RainbowButton.MouseButton1Click:Connect(function()
					...
					UpdateAllRainbowCheckboxes()
					
					if getgenv().RainbowEnabled then
						for toggleColorPicker, _ in pairs(getgenv().RainbowConnections) do
							if toggleColorPicker and toggleColorPicker.SetColor then
								StartRainbow()
							end
						end
						for _, gui in pairs(game:GetService("CoreGui"):GetDescendants()) do
							if gui.Name == "MainWindow" then
								for _, connection in pairs(gui:GetDescendants()) do
									if connection.ClassName == "Frame" and connection.Parent and connection.Parent.Name == "ColorpickerContainer" then
										local colorPicker = connection.Parent.Parent
										if colorPicker and colorPicker.SetColor then
											if not getgenv().RainbowConnections[colorPicker] then
												getgenv().RainbowConnections[colorPicker] = RunService.Heartbeat:Connect(function()
													if getgenv().RainbowEnabled then
														local hue = (tick() * 0.5) % 1
														local rainbowColor = Color3.fromHSV(hue, 1, 1)
														if colorPicker.SetColor then
															colorPicker:SetColor(rainbowColor)
														end
													end
												end)
											end
						...
						for _, connection in pairs(getgenv().RainbowConnections) do
...
function ElementFunction:AddSubTab(SubTabConfig)
    ...
    local function SelectSubTab()
        ...
        for _, ItemContainer in pairs(Container:GetChildren()) do
            if ItemContainer.Name:find("SubTabContainer_") and ItemContainer ~= SubTabContainer then
                ItemContainer.Visible = false
            end
        end
