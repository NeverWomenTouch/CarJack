        ...
        function Category:AddPage(p)
            ...
            function Page:AddBlock(g)
                ...
                if desc and #desc > 0 then
                    ...
                    local infoBtn = Create("TextButton", {BackgroundTransparency = 1, Size = UDim2.fromOffset(18,18), Position = UDim2.fromOffset(gFrame.AbsoluteSize.X - 12,1), AnchorPoint = Vector2.new(1,0), Text = "i", TextColor3 = Theme.SubText, Font = Fonts.Bold, TextSize = 14, AutoButtonColor = false, ZIndex = z + 5, Parent = gFrame})
                    Create("UICorner", {CornerRadius = UDim.new(1,0), Parent = infoBtn})
                    local iStroke = Create("UIStroke", {Color = Theme.Stroke, Thickness = 1.8, Transparency = 0, Parent = infoBtn})
                    -- hover: shift stroke color instead of hiding it
                    infoBtn.MouseEnter:Connect(function()
                        T(infoBtn,0.12,{TextColor3 = Theme.Text}):Play()
                        T(iStroke,0.15,{Color = Theme.Text}):Play()
                    end)
                    infoBtn.MouseLeave:Connect(function()
                        if not popup or not popup.Visible then
                            T(infoBtn,0.16,{TextColor3 = Theme.SubText}):Play()
                            T(iStroke,0.18,{Color = Theme.Stroke}):Play()
                        end
                    end)
                    ...
                    task.spawn(function()
                        ...
                        for _=1,10 do
                            ...
                            infoBtn.Position = UDim2.fromOffset(gFrame.AbsoluteSize.X - 12,1)
                            rs.Heartbeat:Wait()
                        end
