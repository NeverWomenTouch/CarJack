    ...
    function Category:AddSection(p)
            ...
            function Page:AddPage(g)
                ...
                function Group:AddColorpicker(o)
                    ...
                    -- Global sync system for RGB and Pulse modes
                    ...
                    local function syncRGBColor(color, sourceIndex, sourceId)
                        if not Library._globalRGBSync.active then return end
                        for id, listener in pairs(Library._globalRGBSync.listeners) do
                            if id ~= sourceId then
                                pcall(listener, color, sourceIndex)
                            end
                        end
                    end
                    
                    local function syncPulseColor(color, sourceIndex, sourceId)
                    ...
                    local function applyColorFromHSV(index)
                        ...
                        -- update inputs for this slot only
                        if slot.rgbBox then local r,g,b = toRGB255(c); slot.rgbBox.Text = string.format("%d, %d, %d", r,g,b) end
                        if slot.hexBox then local r,g,b = toRGB255(c); slot.hexBox.Text = rgbToHex(r,g,b) end
                    ...
                    local function setHSV(index, h,s,v)
                        ...
                        -- Store current effect states to preserve them
                        local wasRainbow = slot.rainbow
                        local wasPulse = slot.pulse
                        
                        -- Temporarily pause effects during manual control without disabling them
                        if wasRainbow and slot.rainbowHook then 
                            rainbowRemove(slot.rainbowHook)
                            slot.rainbowHook = nil
                            slot._resumeRainbow = true  -- Mark for resumption
                        end
                        if wasPulse and slot.pulseHook then 
                            rainbowRemove(slot.pulseHook)
                            slot.pulseHook = nil
                            slot._resumePulse = true    -- Mark for resumption
                        end
                        ...
                        -- Resume effects after a brief delay to allow manual adjustment to complete
                        game:GetService("RunService").Heartbeat:Wait()
                        if slot._resumeRainbow and wasRainbow and not slot.rainbowHook then
                            slot.rainbowHook = attachRainbow(index)
                            slot._resumeRainbow = nil
                        end
                        if slot._resumePulse and wasPulse and not slot.pulseHook then
                            -- Re-create pulse hook
                            slot.pulseHook = rainbowAdd(function(t)
                                local cycle = (t * 2) % 3
                                local pulse = (math.sin(t * 8) + 1) / 2
                                local r, g, b = 0, 0, 0
                                if cycle < 1 then r = pulse
                                elseif cycle < 2 then g = pulse
                                else b = pulse end
                                local pulsedColor = Color3.fromRGB(math.floor(r * 255), math.floor(g * 255), math.floor(b * 255))
                                slot.color = pulsedColor
                                slot.fill.BackgroundColor3 = pulsedColor
                                syncPulseColor(pulsedColor, index, slot._pulseSyncId)
                                if type(cb) == "function" then pcall(cb, pulsedColor, index) end
                            end)
                    ...
                    local function attachRainbow(i)
                        ...
                        return rainbowAdd(function(h)
                            ...
                            -- Optional sync to other RGB colorpickers (opt-in)
                            if doSync and slot._syncId then
                                syncRGBColor(c, i, slot._syncId)
                            end
                            
                            if open and activeIndex == i then
                                if slots[i].updateSVBackground then slots[i].updateSVBackground(hh) end
                                if slot.hueSlider and slot.hueGrab then slot.hueGrab.Position = UDim2.fromOffset(math.floor(slot.hueSlider.AbsoluteSize.X/2), hh * slot.hueSlider.AbsoluteSize.Y) end
                                applyColorFromHSV(i)
                            else
                    ...
                    local function openPanel(btnFor, index)
                        ...
                        if open and panel then
                            ...
                            panel.Position = UDim2.fromOffset(px, py)
                            -- refresh UI to the selected slot using slot-specific elements
                            local slot = slots[index]
                            local h,s,v = table.unpack(slot.hsv)
                            if slot.updateSVBackground then slot.updateSVBackground(h) end
                            if slot.svCursor and slot.sv then slot.svCursor.Position = UDim2.fromOffset(s*(slot.sv.AbsoluteSize.X), (1-v)*(slot.sv.AbsoluteSize.Y)) end
                            if slot.hueGrab and slot.hueSlider then slot.hueGrab.Position = UDim2.fromOffset(math.floor(slot.hueSlider.AbsoluteSize.X/2), h*(slot.hueSlider.AbsoluteSize.Y)) end
                            if slot.rgbBox then local r,g,b = toRGB255(slot.color); slot.rgbBox.Text = string.format("%d, %d, %d", r,g,b) end
                            if slot.hexBox then local r,g,b = toRGB255(slot.color); slot.hexBox.Text = rgbToHex(r,g,b) end
                        ...
                        if not slot.panel then
                            slot.panel = Create("TextButton", {AutoButtonColor = false, Text = "", BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, Visible = false, Size = UDim2.fromOffset(0,0), ZIndex = 12000, Parent = RootGui}, {
                                Create("UICorner", {CornerRadius = UDim.new(0,4)}),
                        ...
                        panel = slot.panel  -- Set for backward compatibility
                            ...
                            -- Store UI elements and functions in the slot for this panel
                            slot.sv = sv
                            slot.svCursor = svCursor
                            slot.hueGrab = hueGrab
                            slot.hueSlider = hueSlider
                            slot.rgbBox = rgbBox
                            slot.hexBox = hexBox
                            slot.cbMark = cbMark
                            slot.pulseMark = pulseMark
                            
                            slot.setCheckbox = function(on)
                                cbMark.Visible = on and true or false
                            end
                            slot.setPulseCheckbox = function(on)
                                pulseMark.Visible = on and true or false
                            end
                            
                            -- Store functions that operate on this slot's UI elements
                            slot.updateSVBackground = function(h)
                                if sv then sv.BackgroundColor3 = Color3.fromHSV(h,1,1) end
                            end
                            -- Initialize as off
                            slot.setCheckbox(false)
                            slot.setPulseCheckbox(false)
                            ...
                            local function handleHueDown(inp)
                                if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                                    ...
                                    local slot = slots[hueDragIndex]
                                    if slot and slot.updateSVBackground then slot.updateSVBackground(h) end
                            ...
                            rgbBox.FocusLost:Connect(function()
                                ...
                                if r then
                                    ...
                                    local uislot = slots[activeIndex]
                                    if uislot.updateSVBackground then uislot.updateSVBackground(h) end
                                    -- update cursors
                                    if uislot.svCursor and uislot.sv then uislot.svCursor.Position = UDim2.fromOffset(s*uislot.sv.AbsoluteSize.X, (1-v)*uislot.sv.AbsoluteSize.Y) end
                                    if uislot.hueGrab and uislot.hueSlider then uislot.hueGrab.Position = UDim2.fromOffset(math.floor(uislot.hueSlider.AbsoluteSize.X/2), h*uislot.hueSlider.AbsoluteSize.Y) end
                                    applyColorFromHSV(activeIndex)
                                    
                                    -- Update color for effects without disrupting them
                                    if wasRainbow then
                                        slot.hsv = {h,s,v} -- Update base HSV for rainbow to use
                                    end
                                    if wasPulse then
                                        -- Let pulse continue with its own color generation
                                    end
                                else
                                    local c = slots[activeIndex].color
                            ...
                            hexBox.FocusLost:Connect(function()
                                ...
                                if r then
                                    ...
                                    local h,s,v = Color3.toHSV(c)
                                    slots[activeIndex].hsv = {h,s,v}
                                    local uislot2 = slots[activeIndex]
                                    if uislot2.updateSVBackground then uislot2.updateSVBackground(h) end
                                    if uislot2.svCursor and uislot2.sv then uislot2.svCursor.Position = UDim2.fromOffset(s*uislot2.sv.AbsoluteSize.X, (1-v)*uislot2.sv.AbsoluteSize.Y) end
                                    if uislot2.hueGrab and uislot2.hueSlider then uislot2.hueGrab.Position = UDim2.fromOffset(math.floor(uislot2.hueSlider.AbsoluteSize.X/2), h*uislot2.hueSlider.AbsoluteSize.Y) end
                                    applyColorFromHSV(activeIndex)
                            ...
                            cbRow.MouseButton1Click:Connect(function()
                                ...
                                if slot.setCheckbox then slot.setCheckbox(slot.rainbow) end
                                if slot.rainbow then
                                    -- Clean shutdown of pulse
                                    if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                    slot.pulse = false
                                    if slot.setPulseCheckbox then slot.setPulseCheckbox(false) end
                                    
                                    -- Optional: enable global RGB sync
                                    if doSync then
                                        Library._globalRGBSync.active = true
                                        local syncId = tostring(math.random(1000000, 9999999))
                                        slot._syncId = syncId
                                        
                                        -- Register sync listener (RGB-only)
                                        Library._globalRGBSync.listeners[syncId] = function(syncColor, syncIndex)
                                            if not slot.rainbow then return end
                                            local h,s,v = Color3.toHSV(syncColor)
                                            slot.hsv = {h,s,v}
                                            if Library._rainbowBus and Library._rainbowBus.h then
                                            ...
                                            slot.color = syncColor
                                            slot.fill.BackgroundColor3 = syncColor
                                            if type(cb) == "function" then pcall(cb, syncColor, idx) end
                                            if open and activeIndex == idx then
                                                if slot.updateSVBackground then slot.updateSVBackground(h) end
                                                if slot.svCursor and slot.sv then slot.svCursor.Position = UDim2.fromOffset(s*(slot.sv.AbsoluteSize.X), (1-v)*(slot.sv.AbsoluteSize.Y)) end
                                                if slot.hueGrab and slot.hueSlider then slot.hueGrab.Position = UDim2.fromOffset(math.floor(slot.hueSlider.AbsoluteSize.X/2), h*(slot.hueSlider.AbsoluteSize.Y)) end
                                                if slot.rgbBox and not slot.rgbBox:IsFocused() then local r,g,b = math.floor(syncColor.R*255+0.5), math.floor(syncColor.G*255+0.5), math.floor(syncColor.B*255+0.5); slot.rgbBox.Text = string.format("%d, %d, %d", r,g,b) end
                                                if slot.hexBox and not slot.hexBox:IsFocused() then local r,g,b = math.floor(syncColor.R*255+0.5), math.floor(syncColor.G*255+0.5), math.floor(syncColor.B*255+0.5); slot.hexBox.Text = string.format("#%02X%02X%02X", r,g,b) end
                                            end
                                        end
                                    end
                                    
                                    -- Clean shutdown and restart rainbow
                                    if slot.rainbowHook then rainbowRemove(slot.rainbowHook); slot.rainbowHook = nil end
                                    slot.rainbowHook = attachRainbow(idx)
                                    -- Immediately sync starting color so all RGB pickers align
                                    if doSync then
                                        local c0 = hsvToColor(slot.hsv)
                                        if slot._syncId then syncRGBColor(c0, idx, slot._syncId) end
                                    end
                                else
                                    -- Clean shutdown of rainbow and restore manual control
                                    if slot.rainbowHook then rainbowRemove(slot.rainbowHook); slot.rainbowHook = nil end
                                    
                                    -- Remove from global RGB sync
                                    if doSync then
                                        if slot._syncId then
                                            Library._globalRGBSync.listeners[slot._syncId] = nil
                                            slot._syncId = nil
                                        end
                                        -- Check if any other colorpickers still have RGB active
                                        local hasActiveRGB = false
                                    ...
                                    -- Ensure UI reflects current HSV values
                                    local h,s,v = table.unpack(slot.hsv)
                                    if slot.updateSVBackground then slot.updateSVBackground(h) end
                                    if slot.svCursor and slot.sv then slot.svCursor.Position = UDim2.fromOffset(s*(slot.sv.AbsoluteSize.X), (1-v)*(slot.sv.AbsoluteSize.Y)) end
                                    if slot.hueGrab and slot.hueSlider then slot.hueGrab.Position = UDim2.fromOffset(math.floor(slot.hueSlider.AbsoluteSize.X/2), h*(slot.hueSlider.AbsoluteSize.Y)) end
                            ...
                            pulseRow.MouseButton1Click:Connect(function()
                                ...
                                -- Update checkbox visual
                                if slot.setPulseCheckbox then slot.setPulseCheckbox(slot.pulse) end
                                if slot.pulse then
                                    -- Turn off rainbow and prepare pulse state
                                    if slot.rainbowHook then rainbowRemove(slot.rainbowHook); slot.rainbowHook = nil end
                                    slot.rainbow = false
                                    if slot.setCheckbox then slot.setCheckbox(false) end
                                    
                                    -- Optional: Enable global Pulse sync
                                    if doSync then
                                        Library._globalPulseSync.active = true
                                        local syncId = tostring(math.random(1000000, 9999999))
                                        slot._pulseSyncId = syncId
                                    end
                                    -- Seed pulse hue offset off current hue
                                    if Library._rainbowBus and Library._rainbowBus.h then
                                        local currentH = slot.hsv[1] or 0
                                        slot.pulseHueOffset = (currentH - Library._rainbowBus.h) % 1
                                    end
                                    ...
                                    if doSync and slot._pulseSyncId then
                                        Library._globalPulseSync.listeners[slot._pulseSyncId] = function(syncColor, syncIndex)
                                            if not slot.pulse then return end
                                            local h,s,v = Color3.toHSV(syncColor)
                                            slot.hsv = {h,s,slot.hsv[3] or v}
                                            if Library._rainbowBus and Library._rainbowBus.h then
                                                slot.pulseHueOffset = (h - Library._rainbowBus.h) % 1
                                            end
                                            slot.color = syncColor
                                            slot.fill.BackgroundColor3 = syncColor
                                            if type(cb) == "function" then pcall(cb, syncColor, idx) end
                                    ...
                                    -- HSV-based full-spectrum pulse with brightness breathing
                                    if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                    slot.pulseHook = rainbowAdd(function(h)
                                        -- Cycle hue with offset, pulse value
                                        local s = slot.hsv[2] or 1
                                        local baseV = slot.hsv[3] or 1
                                        local hh = (h + (slot.pulseHueOffset or 0)) % 1
                                        local vPulse = 0.35 + 0.65 * (0.5 + 0.5 * math.sin(tick() * 4)) -- nice breathing between 0.35..1.0
                                        local c = Color3.fromHSV(hh, s, math.clamp(vPulse * baseV, 0, 1))
                                        slot.hsv = {hh, s, baseV}
                                        slot.color = c
                                        slot.fill.BackgroundColor3 = c
                                        
                                        if doSync and slot._pulseSyncId then syncPulseColor(c, idx, slot._pulseSyncId) end
                                        if type(cb) == "function" then pcall(cb, c, idx) end
                                        
                                        if open and activeIndex == idx then
                                            if rgbBox and not rgbBox:IsFocused() then
                                    ...
                                    -- Immediately sync starting color so all Pulse pickers align
                                    if doSync and slot._pulseSyncId then
                                        local c0 = hsvToColor(slot.hsv)
                                        syncPulseColor(c0, idx, slot._pulseSyncId)
                                    end
                                else
                                    -- Clean shutdown of pulse and restore manual control
                                    if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                    
                                    -- Remove from global Pulse sync
                                    if doSync then
                                        if slot._pulseSyncId then
                                            Library._globalPulseSync.listeners[slot._pulseSyncId] = nil
                                            slot._pulseSyncId = nil
                                        end
                                        -- Check if any other colorpickers still have Pulse active
                                        local hasActivePulse = false
                                    ...
                                    if slot.svCursor and slot.sv then slot.svCursor.Position = UDim2.fromOffset(s*(slot.sv.AbsoluteSize.X), (1-v)*(slot.sv.AbsoluteSize.Y)) end
                                    if slot.hueGrab and slot.hueSlider then slot.hueGrab.Position = UDim2.fromOffset(math.floor(slot.hueSlider.AbsoluteSize.X/2), h*(slot.hueSlider.AbsoluteSize.Y)) end
                                    -- Update input boxes to match current color
                        ...
                        -- set UI to current slot (per-slot UI references)
                        local slotForUI = slots[index]
                        local h,s,v = table.unpack(slotForUI.hsv)
                        if slotForUI.updateSVBackground then slotForUI.updateSVBackground(h) end
                        if slotForUI.svCursor and slotForUI.sv then slotForUI.svCursor.Position = UDim2.fromOffset(s*(slotForUI.sv.AbsoluteSize.X), (1-v)*(slotForUI.sv.AbsoluteSize.Y)) end
                        if slotForUI.hueGrab and slotForUI.hueSlider then slotForUI.hueGrab.Position = UDim2.fromOffset(math.floor(slotForUI.hueSlider.AbsoluteSize.X/2), h*(slotForUI.hueSlider.AbsoluteSize.Y)) end
                        if slotForUI.rgbBox then local r,g,b = toRGB255(slotForUI.color); slotForUI.rgbBox.Text = string.format("%d, %d, %d", r,g,b) end
                        if slotForUI.hexBox then local r,g,b = toRGB255(slotForUI.color); slotForUI.hexBox.Text = rgbToHex(r,g,b) end
                        -- Reflect slot state and resume effects if needed
                        if slotForUI.setCheckbox then slotForUI.setCheckbox(slotForUI.rainbow or false) end
                        if slotForUI.setPulseCheckbox then slotForUI.setPulseCheckbox(slotForUI.pulse or false) end
                        if slotForUI.rainbow and not slotForUI.rainbowHook then
                            slotForUI.rainbowHook = attachRainbow(index)
                        end
                        if slotForUI.pulse and not slotForUI.pulseHook then
                            slotForUI.pulseHook = rainbowAdd(function(hh)
                                local s2 = slotForUI.hsv[2] or 1
                                local baseV = slotForUI.hsv[3] or 1
                                local hAdj = (hh + (slotForUI.pulseHueOffset or 0)) % 1
                                local vPulse = 0.35 + 0.65 * (0.5 + 0.5 * math.sin(tick() * 4))
                                local c = Color3.fromHSV(hAdj, s2, math.clamp(vPulse * baseV, 0, 1))
                                slotForUI.hsv = {hAdj, s2, baseV}
                                slotForUI.color = c
                                slotForUI.fill.BackgroundColor3 = c
                                if type(cb) == "function" then pcall(cb, c, index) end
                            end)
                        end
                    ...
                    function Colorpicker:SetRainbow(on, index)
                        local function setOne(i, onv)
                            ...
                            if slot.rainbow then
                                ...
                                -- If panel open on this control and swatch, update checkbox visuals
                                if open and activeIndex == i and slot.setCheckbox then
                                    slot.setCheckbox(true)
                                    if slot.setPulseCheckbox then slot.setPulseCheckbox(false) end
                                end
                            else
                                if slot.rainbowHook then rainbowRemove(slot.rainbowHook); slot.rainbowHook = nil end
                                slot._resumeRainbow = nil
                                if open and activeIndex == i and slot.setCheckbox then slot.setCheckbox(false) end
                            end
                        end
                    ...
                    function Colorpicker:SetPulse(on, index)
                        local function setOne(i, onv)
                            ...
                            if slot.pulse then
                                ...
                                if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                slot.pulseHook = rainbowAdd(function(t)
                                    local cycle = (t * 2) % 3 -- Cycle every 1.5 seconds through 3 states
                                    local pulse = (math.sin(t * 8) + 1) / 2 -- Fast pulse for visibility
                                    local r, g, b = 0, 0, 0
                                    if cycle < 1 then -- Red phase
                                        r = pulse
                                    elseif cycle < 2 then -- Green phase  
                                        g = pulse
                                    else -- Blue phase
                                        b = pulse
                                    end
                                ...
                                if open and activeIndex == i and slot.setPulseCheckbox then
                                    slot.setPulseCheckbox(true)
                                    if slot.setCheckbox then slot.setCheckbox(false) end
                                end
                            else
                                if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                if open and activeIndex == i and slot.setPulseCheckbox then slot.setPulseCheckbox(false) end
                            end
                        end
                        if not index then index = self:_getActiveIndex() end
                        setOne(index, on)
