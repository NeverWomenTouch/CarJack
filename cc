...
do
    ...
    function Library:_updateNotificationPositions()
        ...
        local stackOffset = 35 -- notification height + gap
        
        for i, notif in pairs(self._activeNotifications) do
            if notif and notif.Parent then
                local frame = notif:FindFirstChild("NotifFrame")
                if frame then
                    local newY = baseY + (stackOffset * (i-1))
                    T(frame, 0.2, {
                        Position = UDim2.new(1, -10, frame.Position.Y.Scale, newY)
...
function Library:Notification(text, notifyType, timer)
        ...
        if notifyType == 1 then
            ...
            -- Stack notifications vertically by adjusting Y offset
            local stackOffset = notifHeight + 5 -- 5 pixel gap between notifications
            local activeCount = 0
            for _, notif in pairs(self._activeNotifications) do
                if notif and notif.Parent then
                    activeCount = activeCount + 1
                end
            end
            finalOffsetY = finalOffsetY + (stackOffset * activeCount)
            ...
            local label = Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1, -18, 1, 0), Position = UDim2.fromOffset(9,0), Text = text, Font = Fonts.Medium, TextSize = notifTextSize, TextColor3 = Library._watermarkTextColor or Theme.Text, TextXAlignment = Enum.TextXAlignment.Left, TextYAlignment = Enum.TextYAlignment.Center, Parent = inner})
            ...
            table.insert(self._activeNotifications, notifGui)
            ...
            self._activeModalNotification = modalFrame
            ...
            local function closeModal()
                ...
                t.Completed:Connect(function()
                    if overlay and overlay.Parent then
                        overlay:Destroy() -- This will also destroy the modalFrame since it's a child
                    end
                end)
            ...
            return overlay -- Return the overlay which contains the modal
        end
    end
...
function Library:CreateLibrary(opts)
    ...
    local function filePath(name) return configsFolder() .. "/" .. sanitize(name) .. ".json" end
        ...
        function Config.Save(name, data)
            ...
            local payload = { controls = orderedControls, metadata = meta }
