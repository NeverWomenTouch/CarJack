...
local function ShowTooltip(text, targetFrame)
    ...
    local tooltipX = mouse.X + 15
    local tooltipY = mouse.Y + 10
    
    if tooltipX + tooltipWidth > screenSize.X - 10 then
        tooltipX = mouse.X - tooltipWidth - 15
    end
    if tooltipY + tooltipHeight > screenSize.Y - 10 then
        tooltipY = mouse.Y - tooltipHeight - 10
    end
    
    tooltipX = math.max(10, math.min(tooltipX, screenSize.X - tooltipWidth - 10))
    tooltipY = math.max(10, math.min(tooltipY, screenSize.Y - tooltipHeight - 10))
	...
	function TabFunction:MakeTab(TabConfig)
		...
		local function GetElements(ItemParent)
			...
			function ElementFunction:AddButton(ButtonConfig)
				...
				-- Add tooltip functionality - attach to the entire ButtonFrame for better detection
			...
			function ElementFunction:AddMultiDropdown(MultiDropdownConfig)
				...
				local MultiDropdown = {Value = {}, Options = MultiDropdownConfig.Options, Buttons = {}, Toggled = false, Type = "MultiDropdown", Save = MultiDropdownConfig.Save}
				local MaxElements = 5

				for _, defaultValue in pairs(MultiDropdownConfig.Default) do
					if table.find(MultiDropdownConfig.Options, defaultValue) then
						table.insert(MultiDropdown.Value, defaultValue)
					end
				end
				...
				local function GetSelectedText()
					...
					elseif #MultiDropdown.Value == 1 then
						return MultiDropdown.Value[1]
					else
						return tostring(#MultiDropdown.Value) .. " Selected"
					end
				end

				local MultiDropdownList = MakeElement("List")

				local MultiDropdownContainer = AddThemeObject(SetProps(SetChildren(MakeElement("ScrollFrame", Color3.fromRGB(255, 0, 255), 4), {
					MultiDropdownList
				}), {
					Parent = ItemParent,
				...
				local MultiDropdownFrame = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 0, 255), 0, 5), {
					Size = UDim2.new(1, 0, 0, 38),
					Parent = ItemParent,
					ClipsDescendants = true
				}), {
				...
				local function AddOptions(Options)
					for _, Option in pairs(Options) do
						local isSelected = table.find(MultiDropdown.Value, Option) ~= nil
						...
						AddConnection(OptionBtn.MouseButton1Click, function()
							local valueIndex = table.find(MultiDropdown.Value, Option)
							local isCurrentlySelected = valueIndex ~= nil
							
							if isCurrentlySelected then
								table.remove(MultiDropdown.Value, valueIndex)
							...
							MultiDropdownFrame.F.Selected.Text = GetSelectedText()
							MultiDropdownConfig.Callback(MultiDropdown.Value)
							
							if MultiDropdownConfig.Save then
								SaveCfg(game.GameId)
							end
						end)
				...
				function MultiDropdown:Set(Values)
					...
					for Option, Button in pairs(MultiDropdown.Buttons) do
						local isSelected = table.find(MultiDropdown.Value, Option) ~= nil
				...
				function MultiDropdown:SelectAll()
					MultiDropdown:Set(MultiDropdown.Options)
				end
