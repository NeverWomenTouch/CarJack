                ...
                function Group:AddColorpicker(o)
                    ...
                    local function attachRainbow(i)
                        ...
                        return rainbowAdd(function(h)
                            ...
                            -- Sync to other RGB colorpickers
                        ...
                        if not panel then
                            ...
                            local function handleSVDown(inp)
                                if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                                    ...
                                    -- pause rainbow/pulse for this slot while dragging, if active
                                    local slot = slots[svDragIndex]
                                    -- Clear any leftover resume flags
                                    slot._resumeRainbow = nil
                                    slot._resumePulse = nil
                                    ...
                                    if slot and slot.pulse and slot.pulseHook then
                                        rainbowRemove(slot.pulseHook)
                                        slot._resumePulse = true
                                        slot.pulseHook = nil
                                    end
                            ...
                            table.insert(conns, UserInputService.InputEnded:Connect(function(inp)
                                if svDragging and (inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch) then
                                    ...
                                    -- resume rainbow/pulse if it was paused
                                    ...
                                    if slot and slot._resumePulse and slot.pulse and not slot.pulseHook then
                                        -- Re-create pulse hook
                                        slot.pulseHook = rainbowAdd(function(t)
                                            local cycle = (t * 2) % 3
                                            local pulse = (math.sin(t * 8) + 1) / 2
                                            ...
                                            slot.color = pulsedColor
                                            slot.fill.BackgroundColor3 = pulsedColor
                                            if type(cb) == "function" then pcall(cb, pulsedColor, dragIndex) end
                                        end)
                                        slot._resumePulse = nil
                                    end
                                    svDragIndex = nil
                            ...
                            local function handleHueDown(inp)
                                if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
                                    ...
                                    -- Store original states for manual override
                                    slot._wasRainbow = slot.rainbow
                                    slot._wasPulse = slot.pulse
                                    slot._manualOverride = true
                                    ...
                                    setHSVDrag(hueDragIndex, h, nil, nil); updateSVBackground(h)
                                end
                            end
                            ...
                            table.insert(conns, UserInputService.InputChanged:Connect(function(inp)
                                if hueDragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
                                    ...
                                    hueGrab.Position = UDim2.fromOffset(math.floor(hueSlider.AbsoluteSize.X/2), relY)
                                    local h = clamp01(relY / math.max(1, hueSlider.AbsoluteSize.Y))
                                    setHSVDrag(hueDragIndex or activeIndex, h, nil, nil); updateSVBackground(h)
                                end
                            end))
                            table.insert(conns, UserInputService.InputEnded:Connect(function(inp)
                                if hueDragging and (inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch) then
                                    ...
                                    -- Resume effects from new manual position if they were active
                                    if slot and slot._wasRainbow and slot.rainbow and not slot.rainbowHook then
                                        -- Start rainbow from current hue position
                                        local currentH = slot.hsv[1]
                                        slot.rainbowHook = rainbowAdd(function(h)
                                            -- Use manual position as base and continue cycling from there
                                            local adjustedH = (currentH + h) % 1
                                            local s,v = slot.hsv[2], slot.hsv[3]
                                            slot.hsv = {adjustedH,s,v}
                                            if open and activeIndex == dragIndex then
                                                updateSVBackground(adjustedH)
                                                if hueSlider and hueGrab then hueGrab.Position = UDim2.fromOffset(math.floor(hueSlider.AbsoluteSize.X/2), adjustedH * hueSlider.AbsoluteSize.Y) end
                                                applyColorFromHSV(dragIndex)
                                            else
                                                local c = hsvToColor(slot.hsv)
                                                slot.color = c
                                                slot.fill.BackgroundColor3 = c
                                                if type(cb) == "function" then pcall(cb, c, dragIndex) end
                                            end
                                        end)
                                    end
                                    if slot and slot._wasPulse and slot.pulse and not slot.pulseHook then
                                        -- Continue pulse from current color
                                        slot.pulseHook = rainbowAdd(function(t)
                                            local cycle = (t * 2) % 3
                                            local pulse = (math.sin(t * 8) + 1) / 2
                                            local r, g, b = 0, 0, 0
                                    ...
                                    if slot then
                                        slot._wasRainbow = nil
                                        slot._wasPulse = nil
                                        slot._manualOverride = nil
                                    end
                                    hueDragIndex = nil
                                end
                            end))
                            ...
                            cbRow.MouseButton1Click:Connect(function()
                                ...
                                if slot.rainbow then
                                    ...
                                    Library._globalRGBSync.listeners[syncId] = function(syncColor, syncIndex)
                                        if slot.rainbow and open and activeIndex == idx then
                            ...
                            pulseRow.MouseButton1Click:Connect(function()
                                ...
                                if slot.pulse then
                                    ...
                                    -- Register sync listener
                                    Library._globalPulseSync.listeners[syncId] = function(syncColor, syncIndex)
                                        if slot.pulse and open and activeIndex == idx then
                                            slot.color = syncColor
                                            slot.fill.BackgroundColor3 = syncColor
                                            if type(cb) == "function" then pcall(cb, syncColor, idx) end
                                        end
                                    ...
                                    -- Clean shutdown and restart pulse
                                    if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                    slot.pulseHook = rainbowAdd(function(t)
                                        ...
                                        -- Sync to other pulse colorpickers
                                        syncPulseColor(pulsedColor, idx, slot._pulseSyncId)
                                        
                                        if open and activeIndex == idx then
                                            -- Update UI inputs without triggering feedback loops
                                            if rgbBox and not rgbBox:IsFocused() then
                                                rgbBox.Text = string.format("%d, %d, %d", math.floor(r * 255), math.floor(g * 255), math.floor(b * 255))
                                            end
                                            if hexBox and not hexBox:IsFocused() then
                                                hexBox.Text = string.format("#%02X%02X%02X", math.floor(r * 255), math.floor(g * 255), math.floor(b * 255))
                                            end
                                        end
