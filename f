...
function library:Window(title, version, info, preset, closebind)
    ...
    function library:Notification(txt, desc, btn, img, timer)
        ...
        ni.Image = img or "rbxassetid://4483345998"
        ni.ZIndex = 104
    ...
    function tabhold:Tab(text)
        ...
        function tabcontent:KeybindT(text, default, keybind, callback, flag)
            ...
            function ToggleState(state)
                toggled = state
                if toggled then
                    FrameToggleCircle:TweenPosition(UDim2.new(0.587, 0, 0.222000003, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
                    FrameToggle3.BackgroundTransparency = 0
                    
                    if keybindMode == "Always" then
                        keyState = true
                        keybindT.keyState = true
                        pcall(callback, true)
                    end
                else
                    FrameToggleCircle:TweenPosition(UDim2.new(0.127000004, 0, 0.222000003, 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quad, 0.2, true)
                    FrameToggle3.BackgroundTransparency = 1
                    
                    if keybindMode == "Always" or keybindMode == "Hold" then
                        keyState = false
                        keybindT.keyState = false
                        pcall(callback, false)
            ...
            library.Conn[#library.Conn + 1] = Toggle.MouseButton2Click:Connect(function()
                ToggleMenu()
            end)
            
            library.Conn[#library.Conn + 1] = KeybindDisplay.MouseButton1Click:Connect(function()
                ToggleMenu()
            end)
            
            library.Conn[#library.Conn + 1] = KeybindDisplay.MouseEnter:Connect(function()
            ...
            library.Conn[#library.Conn + 1] = KeybindButton.MouseButton1Click:Connect(function()
            ...
            library.Conn[#library.Conn + 1] = ToggleButton.MouseButton1Click:Connect(function()
            ...
            library.Conn[#library.Conn + 1] = HoldButton.MouseButton1Click:Connect(function()
                if isChangingMode then return end
                isChangingMode = true
            ...
            library.Conn[#library.Conn + 1] = AlwaysButton.MouseButton1Click:Connect(function()
                if isChangingMode then return end
                isChangingMode = true
            ...
            for _, button in pairs(modeButtons) do
                library.Conn[#library.Conn + 1] = button.MouseEnter:Connect(function()
                    if button.Text ~= keybindMode then
                        tweensv:Create(button, TweenInfo.new(0.2), {BackgroundTransparency = 0.2}):Play()
                    end
                end)
                
                library.Conn[#library.Conn + 1] = button.MouseLeave:Connect(function()
                    if button.Text ~= keybindMode then
            ...
            library.Conn[#library.Conn + 1] = KeybindButton.MouseEnter:Connect(function()
                tweensv:Create(KeybindButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(55, 55, 55)}):Play()
            end)
            
            library.Conn[#library.Conn + 1] = KeybindButton.MouseLeave:Connect(function()
            ...
            library.Conn[#library.Conn + 1] = uis.InputEnded:Connect(function(input, gameProcessed)
                if gameProcessed then return end
                if input.UserInputType ~= Enum.UserInputType.Keyboard then return end
                if assignedKey == nil or input.KeyCode ~= assignedKey then return end
...
function library:ConfigSystem(tab, name)
    ...
    local cfg = { list = {}, pick = nil, auto = nil }
    ...
    local function sync()
        ...
        for _, file in ipairs(files) do
            ...
            local item = fname and fname:match("^(.+)%.json$")
            if item then
                table.insert(cfg.list, item)
                local got, content = pcall(readfile, file)
                if got then
                    local data = game:GetService("HttpService"):JSONDecode(content)
                    if type(data) == "table" and type(data.metadata) == "table" and data.metadata.autoLoad then
                        cfg.auto = item
                    end
                end
            end
    ...
    local function pack(val)
        ...
        elseif typ == "table" then
            ...
            elseif typeof(val.value) == "Color3" then
                local colorData = { __type = "Color3", r = val.value.R, g = val.value.G, b = val.value.B }
                if val.rainbow ~= nil then colorData.__rainbow = val.rainbow end
                return colorData
            end
            if val.keyCode then
                local keyData = { __type = "Keybind", keyName = val.keyName or (val.keyCode and val.keyCode.Name) or "Unknown" }
                keyData.value = { __type = "EnumItem", enumType = "Enum.KeyCode", value = val.keyCode.Name }
                if val.mode then keyData.mode = val.mode end
                if type(val.value) == "boolean" then keyData.toggleState = val.value end
                return keyData
            elseif val.value ~= nil and typeof(val.value) == "EnumItem" then
                local keyData = { __type = "Keybind", keyName = val.value.Name }
                keyData.value = { __type = "EnumItem", enumType = "Enum.KeyCode", value = val.value.Name }
                if val.mode then keyData.mode = val.mode end
                if type(val.value) == "boolean" then keyData.toggleState = val.value end
                return keyData
            end
            ...
            for k, v in pairs(val) do
                if type(v) ~= "function" then out[k] = pack(v) end
            end
            return out
        else
            return val
    ...
    local function unpack(val)
        ...
        if val.__type == "Color3" then
            if val.__rainbow then
                return { value = Color3.new(val.r, val.g, val.b), rainbow = val.__rainbow }
            end
            return Color3.new(val.r, val.g, val.b)
        elseif val.__type == "EnumItem" then
            local typ = val.enumType
            local name = typ:match("Enum%.([^%s]+)")
            if name and Enum[name] then return Enum[name][val.value] end
            return nil
        elseif val.__type == "Keybind" then
            ...
            local result = { keyName = val.keyName, keyCode = keyEnum or Enum.KeyCode[val.keyName] }
    ...
    local function diff(old, new)
        ...
        for flag, val in pairs(old or {}) do if not flag:match("_rainbow$") then omap[flag] = val end end
        for flag, val in pairs(new or {}) do
            if not flag:match("_rainbow$") then
                ...
                if omap[flag] then
                    ...
                    if oldJson ~= newJson then changed = changed + 1 else same = same + 1 end
                else
                    add = add + 1
                end
            end
        end
        for flag, _ in pairs(omap) do if not nmap[flag] then del = del + 1 end end
    ...
    local menu = tab:NewDropdown("Select Config", cfg.list, function(item) cfg.pick = item end, false, "config_selector")
    local silentAutoLoadUpdate = false

    local function doLoadConfig(configName, showNotify)
        local path = dir .. "/" .. configName .. ".json"
        local ok, raw = pcall(readfile, path)
        if not ok then
            if showNotify then library:Notification("Error", "Config does not exist.", "OK", "rbxassetid://83065037218239") end
            return false
        end
        local data = game:GetService("HttpService"):JSONDecode(raw)
        if type(data.values) ~= "table" then
            if showNotify then library:Notification("Error", "Invalid config format.", "OK", "rbxassetid://83065037218239") end
            return false
        end
        local loaded, normalValues, rainbowValues = 0, {}, {}
        for flag, val in pairs(data.values) do
            if flag:match("_rainbow$") then
                local baseFlag = flag:gsub("_rainbow$", "")
                rainbowValues[baseFlag] = val
            elseif flag ~= "config_autoload" then
                normalValues[flag] = val
            end
        end
        for flag, val in pairs(normalValues) do
            ...
            if type(element) == "table" and type(element.set) == "function" then
                ...
                elseif type(val) == "table" and val.__type == "Keybind" then
                    ...
                    if keyEnum then
                        if val.mode or val.toggleState ~= nil then
                            value = { keyCode = keyEnum, mode = val.mode, value = val.toggleState }
                        else
                            value = keyEnum
                        end
                    end
                elseif type(val) == "number" or type(val) == "boolean" or type(val) == "string" then
                    value = val
                elseif type(val) == "table" and (val.selected or val.options) then
                    value = val.selected or val
                else
                    value = unpack(val)
                end
                if value ~= nil then element:set(value) loaded = loaded + 1 end
            end
        end
        for baseFlag, val in pairs(rainbowValues) do
            local element = library.flags[baseFlag]
            if element and type(element.setRainbow) == "function" then element:setRainbow(val) end
        end
        if showNotify then
            library:Notification("Config Loaded", "\"" .. configName .. "\": " .. loaded .. " settings applied", "OK", "rbxassetid://83065037218239", 6)
        end
        return true
    end
    ...
    local autoLoad = tab:Toggle("Auto Load Selected Config", false, function(on)
        ...
        if not cfg.pick then library:Notification("Error", "No config selected.", "OK", "rbxassetid://83065037218239") return end
        local path = dir .. "/" .. cfg.pick .. ".json"
        local ok, raw = pcall(readfile, path)
        if ok then
            local data = game:GetService("HttpService"):JSONDecode(raw)
            if type(data) ~= "table" then
                library:Notification("Error", "Invalid config format.", "OK", "rbxassetid://83065037218239") return
            end
            ...
            if on then
                for _, name in ipairs(cfg.list) do
                    if name ~= cfg.pick then
                        ...
                        if has then
                            local info = game:GetService("HttpService"):JSONDecode(file)
            ...
            local saved = pcall(writefile, path, jsonData)
            if saved then
                if on then
                    cfg.auto = cfg.pick
                    library:Notification("Success", "Auto-load enabled for config: " .. cfg.pick, "OK", "rbxassetid://83065037218239", 6)
                else
                    if cfg.auto == cfg.pick then cfg.auto = nil end
                    library:Notification("Success", "Auto-load disabled for config: " .. cfg.pick, "OK", "rbxassetid://83065037218239", 6)
                end
            else
                library:Notification("Error", "Failed to update config file.", "OK", "rbxassetid://83065037218239")
                silentAutoLoadUpdate = true
                autoLoad:set(not on)
            end
        else
            library:Notification("Error", "Failed to read config file.", "OK", "rbxassetid://83065037218239")
            silentAutoLoadUpdate = true
            autoLoad:set(false)
        end
    end, "config_autoload")
    ...
    tab:Button("Create Config", function()
        ...
        if name == "" then library:Notification("Error", "Please enter a config name.", "OK", "rbxassetid://83065037218239") return end
        if not name:match("^[%w_]+$") then library:Notification("Error", "Config name can only contain letters, numbers, and underscores.", "OK", "rbxassetid://83065037218239") return end
        for _, item in ipairs(cfg.list) do if item:lower() == name:lower() then library:Notification("Error", "Config already exists.", "OK", "rbxassetid://83065037218239") return end end
        local data = { metadata = { createdAt = os.time(), lastModified = os.time(), autoLoad = false, name = name }, values = {} }
        local count = 0
        for flag, elem in pairs(library.flags) do
            if elem and type(elem) == "table" then
                ...
                if type(elem.rainbow) == "boolean" then data.values[flag.."_rainbow"] = elem.rainbow end
            end
        end
        local path = dir .. "/" .. name .. ".json"
        local jsonData = game:GetService("HttpService"):JSONEncode(data)
        local ok = pcall(writefile, path, jsonData)
        if ok then
            sync()
            ...
            cfg.pick = name
            nameBox:set("")
            library:Notification("Success", "Config \"" .. name .. "\" created with " .. count .. " settings!", "OK", "rbxassetid://83065037218239", 6)
        else
            library:Notification("Error", "Failed to create config file.", "OK", "rbxassetid://83065037218239")
        end
    end)
    ...
    tab:Button("Save/Overwrite Config", function()
        ...
        if ok then
            ...
            if type(data) == "table" and type(data.values) == "table" then
                old = data.values
                local isAutoLoad = data.metadata and data.metadata.autoLoad or false
                data.metadata = data.metadata or {}
                data.metadata.lastModified = os.time()
                data.metadata.autoLoad = isAutoLoad
                data.values = {}
                local now = {}
                for flag, elem in pairs(library.flags) do
                    if elem and type(elem) == "table" then
                        if not flag:match("_rainbow$") then
                            if (flag:find("[bB]ind") or flag:find("[kK]eybind")) and elem.keyCode then
                                local keyData = { __type = "Keybind", keyName = elem.keyName or (elem.keyCode and elem.keyCode.Name) or "Unknown" }
                                keyData.value = { __type = "EnumItem", enumType = "Enum.KeyCode", value = elem.keyCode.Name }
                                if elem.mode then keyData.mode = elem.mode end
                                if type(elem.value) == "boolean" then keyData.toggleState = elem.value end
                                data.values[flag] = keyData
                            elseif flag:find("[cC]olor") then
                                local colorData = { __type = "Color3", r = elem.value.R, g = elem.value.G, b = elem.value.B }
                                if elem.rainbow ~= nil then colorData.__rainbow = elem.rainbow end
                                data.values[flag] = colorData
                            elseif type(elem.value) == "boolean" or type(elem.value) == "number" or type(elem.value) == "string" then
                                data.values[flag] = elem.value
                            elseif type(elem.value) == "table" then
                                data.values[flag] = elem.value
                            else
                                data.values[flag] = pack(elem)
                            end
                ...
                local jsonData = game:GetService("HttpService"):JSONEncode(data)
                ok = pcall(writefile, path, jsonData)
                if ok then
                    local msg, hasChanges = "", false
                    if add > 0 then msg = msg .. add .. " new setting" .. (add > 1 and "s" or "") .. " added" hasChanges = true end
                    if changed > 0 then if msg ~= "" then msg = msg .. ", " end msg = msg .. changed .. " setting" .. (changed > 1 and "s" or "") .. " changed" hasChanges = true end
                    if del > 0 then if msg ~= "" then msg = msg .. ", " end msg = msg .. del .. " setting" .. (del > 1 and "s" or "") .. " removed" hasChanges = true end
                    if not hasChanges then msg = "No settings have been modified" end
                    local total = add + changed + same
                    if hasChanges then msg = msg .. " (Total: " .. total .. ")" end
                    library:Notification("Config Saved", "\"" .. cfg.pick .. "\": " .. msg, "OK", "rbxassetid://83065037218239")
                else
                    library:Notification("Error", "Failed to save config file.", "OK", "rbxassetid://83065037218239")
                end
            else
                library:Notification("Error", "Invalid config format.", "OK", "rbxassetid://83065037218239")
            end
        else
            library:Notification("Error", "Config does not exist.", "OK", "rbxassetid://83065037218239")
        end
    end)
    ...
    tab:Button("Delete Config", function()
        ...
        if not ok then return library:Notification("Error", "Failed to delete config file.", "OK", "rbxassetid://83065037218239") end
        ...
        library:Notification("Success", "Config deleted: \"" .. gone .. "\"", "OK", "rbxassetid://83065037218239")
    end)
    ...
    tab:Button("Refresh Config List", function()
        ...
        if menu then
            if type(menu.Refresh) == "function" then
                menu:Refresh(items)
                if cfg.pick and not table.find(items, cfg.pick) then cfg.pick = nil end
                library:Notification("Config List Refreshed", #items .. " configs found", "OK", "rbxassetid://83065037218239")
            else
                menu = tab:NewDropdown("Select Config", items, function(item) cfg.pick = item end, false, "config_selector")
            end
        else
            menu = tab:NewDropdown("Select Config", items, function(item) cfg.pick = item end, false, "config_selector")
        end
    ...
    if cfg.auto then
        cfg.pick = cfg.auto
        if menu and type(menu.set) == "function" then menu:set(cfg.auto) end
        doLoadConfig(cfg.auto, true)
        silentAutoLoadUpdate = true
        autoLoad:set(true)
    end
    return cfg
