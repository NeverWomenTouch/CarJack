...
function Library:CreateLibrary(opts)
    ...
    local function filePath(name) return configsFolder() .. "/" .. sanitize(name) .. ".json" end
        ...
        function Config.Save(name, data)
            ...
            pcall(function()
                if root then
                    pcall(function() libraryInfo.window = { size = serialize(root.Size), position = serialize(root.Position) } end)
                else
                    pcall(function() libraryInfo.window = { size = serialize(UDim2.new(0,0,0,0)), position = serialize(UDim2.new(0,0,0,0)) } end)
                end
                if Library and Library._keybindListPosition then
                    ...
                    pcall(function()
                        ...
                        libraryInfo.keybindList = { X = k.X or 0, XOffset = k.XOffset or 0, Y = k.Y or 0, YOffset = k.YOffset or 0 }
                    end)
                end
        ...
        function Config.Load(name)
            ...
            if ok and type(raw) == "string" and #raw > 0 then
                ...
                if okj and type(dec) == "table" then
                            local out = {}
                            -- If the saved payload contains library-level info, populate Library cached positions/sizes
                            pcall(function()
                                if Library and type(dec.library) == 'table' then
                                    local lib = dec.library
                                    -- window
                                    if lib.window and lib.window.size then
                                        local s = deserialize(lib.window.size) or lib.window.size
                                        if s then pcall(function() Library._librarySize = s end) end
                                    end
                                    if lib.window and lib.window.position then
                                        local p = deserialize(lib.window.position) or lib.window.position
                                        if p then pcall(function() Library._libraryPosition = p end) end
                                    end
                                    -- keybind list
                                    if lib.keybindList then
                                        local kp = deserialize(lib.keybindList) or lib.keybindList
                                        if kp then
                                            if typeof and typeof(kp) == 'UDim2' then
                                                Library._keybindListPosition = { X = kp.X.Scale, XOffset = kp.X.Offset, Y = kp.Y.Scale, YOffset = kp.Y.Offset }
                                            else
                                                Library._keybindListPosition = kp
                                            end
                                        end
                                    end
                                    -- watermark
                                    if lib.watermark then
                                        ...
                                        if wp then
                                            if typeof and typeof(wp) == 'UDim2' then
                                                Library._watermarkPosition = { X = wp.X.Scale, XOffset = wp.X.Offset, Y = wp.Y.Scale, YOffset = wp.Y.Offset }
                                            else
                                                Library._watermarkPosition = wp
                                            end
                                        end
                                    end
                                    -- mobile toggle
                                    if lib.mobileToggle then
                                        ...
                                        if mp then
                                            if typeof and typeof(mp) == 'UDim2' then
                                                Library._mobileTogglePosition = { X = mp.X.Scale, XOffset = mp.X.Offset, Y = mp.Y.Scale, YOffset = mp.Y.Offset }
                                            else
                                                Library._mobileTogglePosition = mp
                                            end
                                        end
                                    end
                                end
    ...
    -- If a saved library position exists, apply it so recreated UI appears where user saved it
    pcall(function()
        local lp = Library and Library._libraryPosition
        if lp then
            if type(lp) == 'table' and lp.X then
                root.Position = UDim2.new(lp.X or 0, lp.XOffset or 0, lp.Y or 0, lp.YOffset or 0)
            else
                root.Position = lp
            end
        end
            ...
            function Page:AddPage(g)
                ...
                function Group:AddKeybindToggle(o)
                    ...
                    local function openCtx()
                        ...
                        -- compute menu height (4 items: list toggle + 3 mode items) and safe-guard numeric values
                        local menuH = (itemH * 4) + (gap * 4) + (vPad * 2) + 1
                        ctxMenu.Size = UDim2.fromOffset(menuW, menuH)

                        -- guard AbsolutePosition / AbsoluteSize in case the button isn't yet laid out
                        local btnAbs = keybindBtn.AbsolutePosition or Vector2.new(0, 0)
                        local btnSize = keybindBtn.AbsoluteSize or Vector2.new(0, 0)

                        -- guard GuiInset (pcall in case service errors and fallback to zero inset)
            ...
            if okRaw and type(raw) == "string" and #raw > 0 then
                ...
                if okj and type(dec) == "table" and type(dec.library) == "table" then
                    ...
                    pcall(function()
                        ...
                        -- Keybind list: normalize to table {X, XOffset, Y, YOffset} so _createKeybindList can reuse it
                        if lib.keybindList then
                            local kp = deserialize(lib.keybindList) or lib.keybindList
                            if kp then
                                local kpTable = kp
                                ...
                                if Library._keybindList and Library._keybindList.Parent then
                                    ...
                                    if mf then
                                        mf.Position = UDim2.new(kpTable.X or 0, kpTable.XOffset or 0, kpTable.Y or 0, kpTable.YOffset or 0)
                                    end
                                end
                            end
                        end
