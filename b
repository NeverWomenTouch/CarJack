    ...
    function tabhold:Tab(text)
        ...
        function tabcontent:Colorpicker(text, preset, callback, flag)
            ...
            function updateSpeedSlider(speed)
                ...
                RainbowColorPicker = true
                if colorpicker then
                    colorpicker.rainbow = true
                    colorpicker.rainbowSpeed = rainbowSpeed
                end
...
function library:ConfigSystem(tab, name)
    ...
    local autoLoad = tab:Toggle("Auto Load Selected Config", false, function(on)
        ...
        if not cfg.pick then return library:Notification("Error", "No config selected.", "OK", "rbxassetid://4483345998") end
        ...
        if not ok then
            ...
            autoLoad:set(false)
            return library:Notification("Error", "Failed to read config file.", "OK", "rbxassetid://4483345998")
        end
        ...
        if not valid or type(data) ~= "table" then
            silentAutoLoadUpdate = true
            autoLoad:set(false)
            return library:Notification("Error", "Invalid config format.", "OK", "rbxassetid://4483345998")
        end
        ...
        if not saved then
            silentAutoLoadUpdate = true
            autoLoad:set(not on)
            return library:Notification("Error", "Failed to update config file.", "OK", "rbxassetid://4483345998")
        end
    ...
    tab:Button("Create Config", function()
        ...
        if name == "" then return library:Notification("Error", "Please enter a config name.", "OK", "rbxassetid://4483345998") end
        if not name:match("^[%w_]+$") then return library:Notification("Error", "Config name can only contain letters, numbers, and underscores.", "OK", "rbxassetid://4483345998") end
        
        for _, item in ipairs(cfg.list) do
            if item:lower() == name:lower() then
                return library:Notification("Error", "Config already exists.", "OK", "rbxassetid://4483345998")
            end
        end
        ...
        if not ok then return library:Notification("Error", "Failed to create config file.", "OK", "rbxassetid://4483345998") end
    ...
    tab:Button("Save/Overwrite Config", function()
        ...
        local path = dir.."/"..cfg.pick..".json"
        local ok, raw = pcall(readfile, path)
        if not ok then return library:Notification("Error", "Config does not exist.", "OK", "rbxassetid://4483345998") end
        ...
        if not ok then return library:Notification("Error", "Failed to save config file.", "OK", "rbxassetid://4483345998") end
    ...
    tab:Button("Delete Config", function()
        ...
        if not ok then return library:Notification("Error", "Failed to delete config file.", "OK", "rbxassetid://4483345998") end
    ...
    task.spawn(function()
        ...
        if not success then
            library:Notification("Auto-Load Error", tostring(err), "OK", "rbxassetid://4483345998")
        end
    end)
    return cfg
end
