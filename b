...
function library:Window(title, version, info, preset, closebind)
    ...
    function tabhold:Tab(text)
        ...
        function tabcontent:NewSlider(SliderText, min, max, precise, UserFunc, defvalue, flag)
            ...
            -- Define UpdateSlider within the proper scope
            local function UpdateSlider(val, mouseX)
                local percent
                
                if val then
                    -- Exact value was provided
                    percent = (val - min) / (max - min)
                elseif mouseX then
                    -- Mouse position was provided
                    percent = math.clamp((mouseX - Sliding.AbsolutePosition.X) / Sliding.AbsoluteSize.X, 0, 1)
                else
                    -- Use current mouse position from ms global
            ...
            -- Create an InputChanged connection specific to this slider
            ...
            library.Conn[#library.Conn + 1] = Slider.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    -- Disconnect any existing connection to avoid multiple updates
                    if inputChangedConnection then
                        inputChangedConnection:Disconnect()
                    end
                    
                    -- Create a new connection specifically for this slider
                    inputChangedConnection = uis.InputChanged:Connect(function(changedInput)
            ...
            library.Conn[#library.Conn + 1] = Slider.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    -- Disconnect the input changed connection when done dragging
                    if inputChangedConnection then
                        inputChangedConnection:Disconnect()
            ...
            library.Conn[#library.Conn + 1] = Slider.AncestryChanged:Connect(function(_, parent)
                if parent == nil and inputChangedConnection then
                    inputChangedConnection:Disconnect()
                    inputChangedConnection = nil
                end
            end)
...
function library:ConfigSystem(tab, name)
    local folder = library.Folder or "Dendrite.cc"
    ...
    local function RefreshConfigList()
        table.clear(configSystem.configList)
        
        local success, files = pcall(listfiles, folder)
        if not success then 
            warn("Failed to list files in folder:", folder)
            return {} 
        end
        ...
        for _, file in ipairs(files) do
            ...
            if configName then
                table.insert(configSystem.configList, configName)
                ...
                if success then
                    ...
                    if success and type(data) == "table" and 
                       type(data.metadata) == "table" and data.metadata.autoLoad then
                        configSystem.autoLoadConfig = configName
                    end
                end
            end
        end
        
        return configSystem.configList
    end
    
    configSystem.RefreshConfigList = RefreshConfigList
    ...
    local function serializeValue(value)
        ...
        elseif valueType == "table" then
            if value.rainbow ~= nil then
                local result = serializeValue(value.value)
                if type(result) == "table" then
                    result.__rainbow = value.rainbow
                end
                return result
            end
    ...
    local function deserializeValue(value)
        ...
        elseif value.__type == "EnumItem" then
            ...
            local enumName = enumType:match("Enum%.([^%s]+)")
            
            if enumName and Enum[enumName] then
                return Enum[enumName][value.value]
            end
            return nil
        elseif value.__rainbow ~= nil then
            local result = deserializeValue(value)
            return result
        else
            local result = {}
            for k, v in pairs(value) do
    ...
    local function verifyUIElements()
        local missingSetMethods = {}
        for flag, element in pairs(library.flags) do
            if element ~= nil and type(element) == "table" then
                if type(element.set) ~= "function" then
                    table.insert(missingSetMethods, flag)
                end
            end
        end
        
        if #missingSetMethods > 0 then
            warn("[Config System] The following UI elements are missing .set() methods:", table.concat(missingSetMethods, ", "))
        end
    end
    ...
    local configTextbox = tab:Textbox("Config Name", false, function() end, "config_name_input")
    
    local dropdown = tab:NewDropdown("Select Config", configSystem.configList, function(selected)
        configSystem.selectedConfig = selected
    end, false, "config_selector")
    
    local autoLoadToggle = tab:Toggle("Auto Load Selected Config", false, function(state)
        if not (state and configSystem.selectedConfig) then return end
        
        local filePath = folder .. "/" .. configSystem.selectedConfig .. ".json"
        ...
        if success then
            ...
            for _, configName in ipairs(configSystem.configList) do
                if configName ~= configSystem.selectedConfig then
                    local otherFilePath = folder .. "/" .. configName .. ".json"
                    local otherSuccess, otherContent = pcall(readfile, otherFilePath)
                    
                    if otherSuccess then
                        local success, otherData = pcall(function()
                            return game:GetService("HttpService"):JSONDecode(otherContent)
                        end)
                        
                        if success and otherData.metadata then
                            otherData.metadata.autoLoad = false
                            local writeSuccess = pcall(writefile, otherFilePath, game:GetService("HttpService"):JSONEncode(otherData))
                            if not writeSuccess then
                                warn("Failed to update autoLoad for config:", configName)
                            end
                        end
                    end
                end
            ...
            local writeSuccess = pcall(writefile, filePath, game:GetService("HttpService"):JSONEncode(data))
            if writeSuccess then
                configSystem.autoLoadConfig = configSystem.selectedConfig
                library:Notification("Success", "Auto-load enabled for config: " .. configSystem.selectedConfig, "OK")
            else
    ...
    tab:Button("Create Config", function()
        local configName = library.flags["config_name_input"] and library.flags["config_name_input"].value or ""
        ...
        if not configName:match("^[%w_]+$") then
            library:Notification("Error", "Config name can only contain letters, numbers, and underscores.", "OK")
            return
        end
        
        for _, existingConfig in ipairs(configSystem.configList) do
            if existingConfig:lower() == configName:lower() then
        ...
        for flag, element in pairs(library.flags) do
            if element and type(element) == "table" and element.value ~= nil then
                config.values[flag] = serializeValue(element.value)
                
                if element.rainbow ~= nil then
                    config.values[flag.."_rainbow"] = element.rainbow
                end
            end
        end
        
        local filePath = folder .. "/" .. configName .. ".json"
        local success = pcall(writefile, filePath, game:GetService("HttpService"):JSONEncode(config))
        
        if success then
            RefreshConfigList()
            dropdown:Refresh(configSystem.configList)
            dropdown:set(configName) -- Select the newly created config
            configTextbox:set("")
            
            library:Notification("Success", "Config created successfully!", "OK")
        else
    ...
    tab:Button("Save Config", function()
        if not configSystem.selectedConfig then
        ...
        for flag, element in pairs(library.flags) do
            if element and type(element) == "table" and element.value ~= nil then
                data.values[flag] = serializeValue(element.value)
                savedCount = savedCount + 1
                
                if element.rainbow ~= nil then
                    data.values[flag.."_rainbow"] = element.rainbow
                end
            end
        end
        
        success = pcall(writefile, filePath, game:GetService("HttpService"):JSONEncode(data))
        
        if success then
            library:Notification("Success", "Config saved successfully! (" .. savedCount .. " settings saved)", "OK")
        else
...
tab:Button("Load Config", function()
    ...
    if not success or type(data.values) ~= "table" then
            ...
            task.spawn(function()
                pcall(function()
                    if type(library.flags[flag]) == "table" and type(library.flags[flag].set) == "function" then
                        library.flags[flag]:set(deserializedValue)
    ...
    task.delay(0.3, function()
        for flag, value in pairs(data.values) do
            ...
            if library.flags[baseFlagName] and type(library.flags[baseFlagName].setRainbow) == "function" then
                pcall(function()
                    library.flags[baseFlagName]:setRainbow(value)
                end)
            end
        end
        
        library:Notification("Config Loaded", loadedCount .. " settings applied successfully", "OK")
    end)
end)
 ...
 tab:Button("Delete Config", function()
    ...
    local filePath = folder .. "/" .. configSystem.selectedConfig .. ".json"
    local configToDelete = configSystem.selectedConfig
    ...
    if configSystem.autoLoadConfig == configSystem.selectedConfig then
        configSystem.autoLoadConfig = nil
        autoLoadToggle:set(false)
    end
    ...
    library:Notification("Success", "Config deleted successfully: " .. configToDelete, "OK")
end)
 ...
 tab:Button("Refresh Config List", function()
    ...
    if dropdown then
        if type(dropdown.Refresh) == "function" then
            ...
            if configSystem.selectedConfig and not table.find(configs, configSystem.selectedConfig) then
                configSystem.selectedConfig = nil
            end
            library:Notification("Success", "Config list refreshed! (" .. #configs .. " configs found)", "OK")
        else
            library:Notification("Debug", "Dropdown exists but Refresh function not found", "OK")
            dropdown = tab:NewDropdown("Select Config", configs, function(selected)
                configSystem.selectedConfig = selected
            end, false, "config_selector")
        end
    else
        library:Notification("Debug", "Dropdown is nil - recreating", "OK")
        dropdown = tab:NewDropdown("Select Config", configs, function(selected)
            configSystem.selectedConfig = selected
        end, false, "config_selector")
    end
    ...
    -- Run a one-time check to verify UI elements
    task.spawn(function() 
        task.wait(2) -- Wait for UI to initialize
        verifyUIElements()
    end)
...
task.spawn(function()
    ...
    if not configSystem.autoLoadConfig then return end
    
    local filePath = folder .. "/" .. configSystem.autoLoadConfig .. ".json"
    local success, fileContent = pcall(readfile, filePath)
    ...
    if not success or type(data.values) ~= "table" then return end
    ...
    configSystem.selectedConfig = configSystem.autoLoadConfig
    
    task.delay(0.5, function()
        pcall(function()
            dropdown:set(configSystem.autoLoadConfig)
            autoLoadToggle:set(true)
        end)
        
        library:Notification("Config Auto-loaded", "Loaded \"" .. configSystem.autoLoadConfig .. "\" with " .. loadedCount .. " settings", "OK")
    end)
end)
