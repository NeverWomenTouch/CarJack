            ...
            function Page:AddBlock(g)
                ...
                function Group:AddDropdown(o)
                    ...
                    local trigger = Create("TextButton", {BackgroundColor3 = Theme.Button, AutoButtonColor = false, Text = "", Size = UDim2.new(1-labelWidth,-4,1,0), Position = UDim2.new(labelWidth,4,0,0), Parent = row}, {
                        Create("UICorner", {CornerRadius = UDim.new(0,5)}),
                        Create("UIStroke", {Name = "Stroke", Color = Theme.Stroke, Thickness = 1, Transparency = 0.45}),
                        Create("UIPadding", {PaddingLeft = UDim.new(0,8), PaddingRight = UDim.new(0,18)})
                    })
                    local valueLbl = Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1,-18,1,0), Text = default and tostring(default) or placeholder, Font = Fonts.Medium, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = default and Theme.Text or Theme.SubText, Parent = trigger})
                    local caret = Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.fromOffset(12,10), AnchorPoint = Vector2.new(1,0.5), Position = UDim2.new(1,-3,0.5,0), Text = "v", Font = Fonts.Bold, TextSize = 12, TextColor3 = Theme.SubText, Parent = trigger})
                    ...
                    local conns = {}
                    local activeIndex = 1
                    local Dropdown = { id = id, _value = default, _options = {}, _filtered = {}, _itemMap = {}, _onOpen = nil, _onClose = nil }
                    ...
                    local function buildPanel()
                        ...
                        if enableSearch then
                            ...
                            Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.fromOffset(16,24), Position = UDim2.fromOffset(innerPad, y), Text = "🔎", Font = Fonts.Medium, TextSize = 12, TextColor3 = Theme.SubText, TextXAlignment = Enum.TextXAlignment.Center, Parent = panel})
                            searchBox = Create("TextBox", {BackgroundColor3 = Theme.Button, ClearTextOnFocus = false, Text = "", PlaceholderText = "Search...", PlaceholderColor3 = Theme.SubText, TextColor3 = Theme.Text, Font = Fonts.Medium, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Center, Size = UDim2.new(1, -(innerPad*2 + 16 + 6), 0, 24), Position = UDim2.fromOffset(innerPad + 16 + 6, y), Parent = panel}, {
                                Create("UICorner", {CornerRadius = UDim.new(0,5)}),
                                Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.4})
                            })
                            y = y + 24 + innerPad
                    ...
                    local function renderItems()
                        ...
                        for _, val in ipairs(src) do
                            ...
                            hit.MouseButton1Down:Connect(function() Dropdown:Set(val) end)
                            hit.MouseButton1Click:Connect(function() Dropdown:Set(val) end)
                    ...
                    function Dropdown:SetOptions(nl)
                        ...
                        if self._value ~= nil and not table.find(self._options, self._value) then
                            self._value = nil; valueLbl.Text = placeholder; valueLbl.TextColor3 = Theme.SubText
                        else
                            updateSelection()
                        end
                    end
                    ...
                    if Dropdown._value == nil then valueLbl.Text = placeholder valueLbl.TextColor3 = Theme.SubText end
