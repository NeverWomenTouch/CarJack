...
function library:Window(title, version, info, preset, closebind)
    ...
    function tabhold:Tab(text)
        ...
        function tabcontent:NewSlider(text, low, high, precise, callback, default, flag)
            ...
            local function update(value, mouseX)
                local pct
                ...
                elseif mouseX then
                    pct = math.clamp((mouseX - bar.AbsolutePosition.X) / bar.AbsoluteSize.X, 0, 1)
                end
            ...
            local slider = {
                value = precise and tonumber(state.show) or math.floor(state.real),
            ...
            library.Conn[#library.Conn + 1] = btn.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    state.active = true
                    if state.conn then
                        state.conn:Disconnect()
                    end
                    state.conn = ui.InputChanged:Connect(function(changed)
            ...
            library.Conn[#library.Conn + 1] = btn.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    state.active = false
                    if state.conn then
                        state.conn:Disconnect()
...
function library:ConfigSystem(tab, name)
    ...
    ---- to fix: slider value and bind aint getting saved.
    ...
    local function pack(val)
        ...
        elseif typ == "table" then
            -- Slider: must have .set and .value (number)
            if type(val.set) == "function" and type(val.value) == "number" then
                return {__type="Slider", value=val.value}
            end
            -- Keybind: must have .set and .value (Enum.KeyCode)
            if type(val.set) == "function" and typeof(val.value) == "EnumItem" and val.value.EnumType == Enum.KeyCode then
                return {
                    __type = "Bind",
                    keyName = val.value.Name,
                    keyCode = {__type="EnumItem", enumType="Enum.KeyCode", value=val.value.Name}
                }
            end
            -- KeybindT: must have .set and .keyCode (Enum.KeyCode)
            if type(val.set) == "function" and val.keyCode and typeof(val.keyCode) == "EnumItem" and val.keyCode.EnumType == Enum.KeyCode then
                local t = {
                    __type = "KeybindT",
                    keyName = val.keyName or val.keyCode.Name,
                    keyCode = {__type="EnumItem", enumType="Enum.KeyCode", value=val.keyCode.Name},
                    mode = val.mode,
                    value = val.value,
    ...
    local function unpack(val)
        ...
        elseif val.__type == "Bind" then
            return Enum.KeyCode[val.keyName or (val.keyCode and val.keyCode.value) or "Unknown"]
        elseif val.__type == "KeybindT" then
            local keyEnum = Enum.KeyCode[val.keyName or (val.keyCode and val.keyCode.value) or "Unknown"]
            return {
                keyCode = keyEnum,
                mode = val.mode,
                value = val.value,
    ...
    local autoLoad = tab:Toggle("Auto Load Selected Config", false, function(on)
        ...
        if not cfg.pick then return library:Notification("Error", "No config selected.", "OK", "rbxassetid://4370314188") end
        ...
        if not ok then
            ...
            autoLoad:set(false)
            return library:Notification("Error", "Failed to read config file.", "OK", "rbxassetid://4370314188")
        end
        ...
        if not valid or type(data) ~= "table" then
            silentAutoLoadUpdate = true
            autoLoad:set(false)
            return library:Notification("Error", "Invalid config format.", "OK", "rbxassetid://4370314188")
        end
        ...
        if not saved then
            silentAutoLoadUpdate = true
            autoLoad:set(not on)
            return library:Notification("Error", "Failed to update config file.", "OK", "rbxassetid://4370314188")
        end
    ...
    tab:Button("Create Config", function()
        ...
        if name == "" then return library:Notification("Error", "Please enter a config name.", "OK", "rbxassetid://4370314188") end
        if not name:match("^[%w_]+$") then return library:Notification("Error", "Config name can only contain letters, numbers, and underscores.", "OK", "rbxassetid://4370314188") end
        
        for _, item in ipairs(cfg.list) do
            if item:lower() == name:lower() then
                return library:Notification("Error", "Config already exists.", "OK", "rbxassetid://4370314188")
            end
        end
        ...
        if not ok then return library:Notification("Error", "Failed to create config file.", "OK", "rbxassetid://4370314188") end
    ...
    tab:Button("Save/Overwrite Config", function()
        ...
        local path = dir.."/"..cfg.pick..".json"
        local ok, raw = pcall(readfile, path)
        if not ok then return library:Notification("Error", "Config does not exist.", "OK", "rbxassetid://4370314188") end
        ...
        if not ok then return library:Notification("Error", "Failed to save config file.", "OK", "rbxassetid://4370314188") end
    ...
    tab:Button("Load Config", function()
        if not cfg.pick then
            return library:Notification("Error", "No config selected.", "OK", "rbxassetid://83065037218239")
        end
        local path = dir .. "/" .. cfg.pick .. ".json"
        local ok, raw = pcall(readfile, path)
        if not ok then
            return library:Notification("Error", "Config does not exist.", "OK", "rbxassetid://83065037218239")
        end
        ...
        if not valid or type(data.values) ~= "table" then
            return library:Notification("Error", "Invalid config format.", "OK", "rbxassetid://83065037218239")
        end
        ...
        task.delay(0.3, function()
            ...
            library:Notification("Config Loaded", "\"" .. cfg.pick .. "\": " .. loaded .. " settings applied", "OK", "rbxassetid://83065037218239", 6)
        end)
    end)
    ...
    tab:Button("Delete Config", function()
        ...
        if not ok then return library:Notification("Error", "Failed to delete config file.", "OK", "rbxassetid://4370314188") end
    ...
    task.spawn(function()
        ...
        task.delay(0.3, function()
            ...
            task.delay(0.2, function()
                silentAutoLoadUpdate = true
                autoLoad:set(true)
                library:Notification("Config Auto-loaded", "\"" .. cfg.auto .. "\": " .. loaded .. " settings applied", "OK", "rbxassetid://83065037218239", 6)
            end)
        end)
    end)
    return cfg
