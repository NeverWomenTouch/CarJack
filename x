            ...
            function Page:AddPage(g)
                ...
                function Group:AddDropdown(...)
                    ...
                    local function positionPanel(finalH)
                        ...
                        local openAboveY = openBelowY - (finalH or 0) - tSz.Y - 4
                        local viewportH = rootSz.Y
                        local globalBottom = (tAbs.Y - rootAbs.Y) + tSz.Y + 2 + (finalH or 0)
                        local flip = (globalBottom + 8) > viewportH
                        panel.Position = UDim2.fromOffset(px, flip and openAboveY or openBelowY)
                ...
                function Group:AddColorpicker(o)
                    ...
                    for i=1, amount do
                        ...
                        slots[i] = { btn = btn, fill = fill, color = defaults[i], hsv = { Color3.toHSV(defaults[i]) }, rainbow = startRainbow, rainbowHook = nil }
                    end
                    ...
                    local function openCtx(btnFor)
                        ...
                        local menuW, sidePad, vPad, gap = 130, 8, 8, 6
                        ctxMenu = Create("Frame", {BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, Size = UDim2.fromOffset(menuW,0), ZIndex = menuZBase + 1, Parent = RootGui}, {
                            Create("UICorner", {CornerRadius = UDim.new(0,4)}),
                            -- Match library outline color/transparency
                            Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.3})
                        })
                        Create("ImageLabel", {BackgroundTransparency = 1, Image = "rbxassetid://4996891970", ImageColor3 = Theme.Bg, ImageTransparency = 0.92, Size = UDim2.fromScale(1,1), ZIndex = (ctxMenu.ZIndex or 1)-1, Parent = ctxMenu}).SliceCenter = Rect.new(20,20,280,280)
                        local content = Create("Frame", {BackgroundTransparency = 1, Size = UDim2.new(1, -(sidePad*2), 1, -(vPad*2)), Position = UDim2.fromOffset(sidePad, vPad), ZIndex = ctxMenu.ZIndex + 1, Parent = ctxMenu}, {
                            Create("UIListLayout", {Padding = UDim.new(0,gap), SortOrder = Enum.SortOrder.LayoutOrder})
                        })
                        local function item(text, on)
                            local it = Create("TextButton", {BackgroundColor3 = Theme.Button, AutoButtonColor = false, Text = text, TextColor3 = Theme.Text, Font = Fonts.Medium, TextSize = 12, Size = UDim2.new(1,0,0,22), Parent = content}, {
                                Create("UICorner", {CornerRadius = UDim.new(0,4)}),
                                Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.45})
                            })
                            it.MouseEnter:Connect(function() T(it,0.10,{BackgroundColor3 = Theme.Hover}):Play() end)
                            it.MouseLeave:Connect(function() T(it,0.10,{BackgroundColor3 = Theme.Button}):Play() end)
                            it.MouseButton1Click:Connect(function() on(); closeCtx() end)
                        end
                    ...
                    local function openPanel(btnFor, index)
                        ...
                        if open and panel then
                            ...
                            if setCheckbox then local slot = slots[index]; setCheckbox(slot.rainbow) end
                            return
                        end
                        ...
                        if not panel then
                            panel = Create("TextButton", {AutoButtonColor = false, Text = "", BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, Visible = false, Size = UDim2.fromOffset(0,0), ZIndex = 12000, Parent = RootGui}, {
                                Create("UICorner", {CornerRadius = UDim.new(0,4)}),
                                -- Keep panel stroke but hide it; we'll draw a top-layer border overlay so it sits above inner content
                                Create("UIStroke", {Name = "Stroke", Color = Theme.Stroke, Thickness = 1, Transparency = 1})
                            })
                            local shadow = Create("ImageLabel", {BackgroundTransparency = 1, Image = "rbxassetid://4996891970", ImageColor3 = Theme.Bg, ImageTransparency = 0.85, Size = UDim2.fromScale(1,1), ZIndex = panel.ZIndex - 1, Parent = panel})
                            shadow.ScaleType = Enum.ScaleType.Slice; shadow.SliceCenter = Rect.new(20,20,280,280)
                            ...
                            hueSlider = Create("Frame", {Active = true, BackgroundTransparency = 0, BackgroundColor3 = Color3.fromRGB(255,255,255), BorderSizePixel = 0, Size = UDim2.fromOffset(HUE_W, HUE_H), Position = UDim2.fromOffset(0, 0), ZIndex = rightCol.ZIndex + 1, Parent = rightCol}, {Create("UICorner", {CornerRadius = UDim.new(0,4)}), Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.3})})
                            ...
                            local cbRow = Create("TextButton", {BackgroundTransparency = 1, AutoButtonColor = false, Text = "", Size = UDim2.fromOffset(60, 16), Position = UDim2.fromOffset(0, HUE_H + 10), ZIndex = rightCol.ZIndex + 1, Parent = rightCol})
                            ...
                            Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1,-16,1,0), Position = UDim2.fromOffset(14,0), Text = "Rainbow", Font = Fonts.Medium, TextSize = 10, TextColor3 = Theme.SubText, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = cbRow.ZIndex + 1, Parent = cbRow})
                        ...
                        -- always open above aligned to the right
