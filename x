...
function Library:CreateLibrary(opts)
    ...
    -- Global search system (created once for entire window)
    ...
    function Window:ToggleSearch(show)
        ...
        if show then
            ...
            -- Position popup correctly relative to search button (fixed positioning)
            if Window._currentSearchBtn then
                ...
                -- Position to the right of the button, not center
                local px = btnAbs.X - rootAbs.X + btnSize.X - 210
                if px < 4 then px = 4 end
                self._globalSearchPopup.Position = UDim2.fromOffset(px, btnAbs.Y - rootAbs.Y + btnSize.Y + 6)
            end
    ...
    UserInputService.InputBegan:Connect(function(inp)
        ...
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            -- Check if click is inside search button
            local insideBtn = false
            if Window._currentSearchBtn then
                local btnAbs = Window._currentSearchBtn.AbsolutePosition
                local btnSz = Window._currentSearchBtn.AbsoluteSize
                insideBtn = pos.X >= btnAbs.X and pos.X <= btnAbs.X+btnSz.X and pos.Y >= btnAbs.Y and pos.Y <= btnAbs.Y+btnSz.Y
            end
            
            -- Close search if clicking anywhere except search popup or button
    ...
    -- Create search button for this category (references global search system)
    ...
    searchBtn.MouseButton1Click:Connect(function() 
        ...
        if Window._globalSearchPopup.Visible then
            Window:ToggleSearch(false)
        else
        ...
        function Window:ToggleSearch(show)
            ...
            if show then
                ...
                if Window._currentSearchBtn then
                    ...
                    local rootAbs = root.AbsolutePosition
                    local btnSize = Window._currentSearchBtn.AbsoluteSize
                    local px = btnAbs.X - rootAbs.X - (210/2) + (btnSize.X/2)
                    if px < 4 then px = 4 end
                    self._globalSearchPopup.Position = UDim2.fromOffset(px, btnAbs.Y - rootAbs.Y + btnSize.Y + 6)
                end
        ...
        function Window:ApplySearchFilter()
            ...
            if match and match.page and match.groupFrame then
                ...
                if wasSearchOpen then
                    self._globalSearchPopup.Visible = true
                    -- Reposition popup relative to current category's search button
                    if self._currentSearchBtn and self._currentSearchBtn.Parent then
                        local btnAbs = self._currentSearchBtn.AbsolutePosition
                        local contentAbs = content.AbsolutePosition
                        local btnSize = self._currentSearchBtn.AbsoluteSize
                        local px = btnAbs.X - contentAbs.X - (210/2) + (btnSize.X/2)
                        if px < 4 then px = 4 end
                        self._globalSearchPopup.Position = UDim2.fromOffset(px, btnAbs.Y - contentAbs.Y + btnSize.Y + 6)
                    end
                end
    ...
    local pageArea = Create("Frame", {BackgroundTransparency = 1, ClipsDescendants = true, Size = UDim2.new(1, -10, 1, -(24 + 10 + 6)), Position = UDim2.fromOffset(5, 24 + 9), Parent = catContainer})
        function Category:Select()
            ...
            -- Update current search button reference for this category
            Window._currentSearchBtn = searchBtn
