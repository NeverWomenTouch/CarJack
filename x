    ...
    do -- Components
        ...
        Library.Pages.GlobalChat = function(self)
            ...
            local function StartPresenceHeartbeat()
                ...
                Library:Thread(function()
                    while not BackendState.Destroyed do
                        ...
                        if Config and RequestFunction then
                            local Encoded = JsonEncode({
                                clientId = BackendState.ClientId,
                                senderId = BackendState.Profile.SenderId or tostring(LocalPlayer.UserId),
                            ...
                            if Encoded then
                                local Success, ResponseBody = DoHttpRequest({
                                    Url = BuildAuthorizedUrl(Config, Config.HttpUrl .. "/presence", {
                                        clientId = BackendState.ClientId,
                                        senderId = BackendState.Profile.SenderId or tostring(LocalPlayer.UserId),
                                        username = GetLocalUsername()
                                    }),
                    ...
                    while not BackendState.Destroyed do
                        ...
                        if Config and RequestFunction and Config.HttpUrl and Config.HttpUrl ~= "" then
                            ...
                            if Success then
                                ...
                                if Payload and Payload.typingUsers ~= nil then
                                    UpdateTypingText(Payload.typingUsers)
                                else
                                    UpdateTypingText({ })
                                end
            ...
            local function StartPolling()
                if BackendState.Polling then
                    return
                end

                if not RequestFunction then
                    SetConnectionAppearance("No HTTP support", FromRGB(255, 120, 120))
                    return
                end

                BackendState.Polling = true
                SetConnectionAppearance("Polling...", FromRGB(255, 209, 102))

                Library:Thread(function()
                    while BackendState.Polling and not BackendState.Destroyed do
                        local Config = BackendState.Config
                        if not Config or not Config.HttpUrl or Config.HttpUrl == "" then
                            SetConnectionAppearance("No backend URL", FromRGB(255, 120, 120))
                            StopPolling()
                            break
                        end
                        ...
                        local Success, ResponseBody = DoHttpRequest({
                            Url = BuildAuthorizedUrl(Config, Config.HttpUrl .. "/messages", {
                                afterId = BackendState.LastMessageId,
                                clientId = BackendState.ClientId,
                                senderId = BackendState.Profile.SenderId or tostring(LocalPlayer.UserId),
                                username = GetLocalUsername(),
                        ...
                        if Success then
                            HandlePayload(JsonDecode(ResponseBody))
                            task.wait(Config.PollInterval or 3)
                        else
                            SetConnectionAppearance("Retrying...", FromRGB(255, 120, 120))
                            task.wait(Config.RetryDelay or 5)
                        end
                    end
                end)
            end
