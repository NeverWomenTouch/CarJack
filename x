...
local Library = { _windows = {}, _controls = {}, _theme = Theme, _fonts = Fonts, _version = "3.1.0", _searchEntries = {} }
...
function Library:CreateLibrary(opts)
    ...
    -- search popup + button (per category view) -----------------------------
    local searchPopup = Create("Frame", {BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, Visible = false, Size = UDim2.fromOffset(210, 30), Position = UDim2.fromOffset(0,0), AnchorPoint = Vector2.new(0,0), ZIndex = z + 500, Parent = catContainer}, {
    ...
    local searchBox = Create("TextBox", {BackgroundTransparency = 1, ClearTextOnFocus = false, Text = "", PlaceholderText = "Search...", PlaceholderColor3 = Theme.SubText, TextColor3 = Theme.Text, Font = Fonts.Medium, TextSize = 13, Size = UDim2.new(1,0,1,0), TextXAlignment = Enum.TextXAlignment.Left, Parent = searchPopup})
    ...
    -- track whether columns are filtered so we can restore
    local filteredPage
    local activeHighlightStroke, activeStrokeOrigColor, activeStrokeOrigThickness, activeStrokeOrigTrans
    local function restoreColumns()
        if filteredPage and filteredPage._columnsFrames then
            for _, cfr in ipairs(filteredPage._columnsFrames) do cfr.Visible = true end
        end
        filteredPage = nil
        if activeHighlightStroke and activeHighlightStroke.Parent then
            activeHighlightStroke.Color = activeStrokeOrigColor or activeHighlightStroke.Color
            activeHighlightStroke.Thickness = activeStrokeOrigThickness or 1
            activeHighlightStroke.Transparency = activeStrokeOrigTrans or activeHighlightStroke.Transparency
        end
        activeHighlightStroke = nil
    end
    local function toggleSearch(show)
        show = (show == nil) and (not searchPopup.Visible) or show
        if show then
            searchPopup.Visible = true
            -- comfy intro animation (soft glow + fade + size ease)
            local stroke = searchPopup:FindFirstChild("Stroke")
            if stroke then stroke.Thickness = 2.2 stroke.Transparency = 0.75 end
            searchPopup.BackgroundTransparency = 1
            searchPopup.Size = UDim2.fromOffset(0,0)
            -- position before anim
            local function positionPopup()
                local btnAbs = searchBtn.AbsolutePosition
                local parentAbs = catContainer.AbsolutePosition
                local btnSize = searchBtn.AbsoluteSize
                local px = btnAbs.X - parentAbs.X - (210/2) + (btnSize.X/2)
                if px < 4 then px = 4 end
                searchPopup.Position = UDim2.fromOffset(px, subBar.Position.Y.Offset + subBar.Size.Y.Offset + 6)
            end
            positionPopup()
            -- soft glow behind
            local glow = Create("ImageLabel", {BackgroundTransparency = 1, Image = "rbxassetid://4996891970", ImageColor3 = Theme.Accent, ImageTransparency = 0.92, Size = UDim2.fromOffset(20,20), AnchorPoint = Vector2.new(0.5,0.5), Position = UDim2.new(0, searchPopup.Position.X.Offset + 105, 0, searchPopup.Position.Y.Offset + 15), ZIndex = searchPopup.ZIndex - 1, Parent = catContainer})
            T(glow,0.45,{ImageTransparency = 1, Size = UDim2.fromOffset(260,60)}):Play()
            task.delay(0.5, function() pcall(function() glow:Destroy() end) end)
            T(searchPopup,0.22,{Size = UDim2.fromOffset(210,30)}):Play()
            T(searchPopup,0.25,{BackgroundTransparency = 0}):Play()
            if stroke then
                T(stroke,0.35,{Thickness = 1, Transparency = 0.4}):Play()
            end
    ...
    UserInputService.InputBegan:Connect(function(inp)
        if searchPopup.Visible and (inp.KeyCode == Enum.KeyCode.Escape) then toggleSearch(false) end
    end)
    UserInputService.InputBegan:Connect(function(inp)
        if not searchPopup.Visible then return end
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            local pos = UserInputService:GetMouseLocation()
            local abs = searchPopup.AbsolutePosition
            local sz = searchPopup.AbsoluteSize
            local inside = pos.X >= abs.X and pos.X <= abs.X+sz.X and pos.Y >= abs.Y and pos.Y <= abs.Y+sz.Y
            local btnAbs = searchBtn.AbsolutePosition
            local btnSz = searchBtn.AbsoluteSize
            local insideBtn = pos.X >= btnAbs.X and pos.X <= btnAbs.X+btnSz.X and pos.Y >= btnAbs.Y and pos.Y <= btnAbs.Y+btnSz.Y
            if not inside and not insideBtn then toggleSearch(false) end
        end
    end)
    local function applyFilter()
        local term = searchBox.Text or ""
        if term == "" then
            restoreColumns()
            return
        end
        local match = Window:Search(term, true) -- returns entry, suppress highlight flash
        if match and match.page and match.groupFrame then
            ...
            -- outline accent (stroke pulse) only
            local stroke = nil
            for _, child in ipairs(match.groupFrame:GetChildren()) do
                if child:IsA("UIStroke") then stroke = child break end
            end
            if stroke then
                if activeHighlightStroke ~= stroke then
                    if activeHighlightStroke and activeHighlightStroke.Parent then
                        activeHighlightStroke.Color = activeStrokeOrigColor or activeHighlightStroke.Color
                        activeHighlightStroke.Thickness = activeStrokeOrigThickness or 1
                        activeHighlightStroke.Transparency = activeStrokeOrigTrans or activeHighlightStroke.Transparency
                    end
                    activeHighlightStroke = stroke
                    activeStrokeOrigColor = stroke.Color
                    activeStrokeOrigThickness = stroke.Thickness
                    activeStrokeOrigTrans = stroke.Transparency
                end
    ...
    searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        if not searchPopup.Visible then return end
        if searchBox.Text == "" then restoreColumns() return end
        lastChange = lastChange + 1
        local version = lastChange
        task.delay(0.15, function() if version == lastChange then applyFilter() end end)
    end)
    searchBox.FocusLost:Connect(function(enter)
        if enter then applyFilter() end
    end)
    searchBox.ReturnPressedFromOnScreenKeyboard:Connect(applyFilter)
    local pageArea = Create("Frame", {BackgroundTransparency = 1, ClipsDescendants = true, Size = UDim2.new(1, -10, 1, -(24 + 10 + 6)), Position = UDim2.fromOffset(5, 24 + 9), Parent = catContainer})
        ...
        function Category:AddPage(p)
            ...
            function Page:AddBlock(g)
                ...
                local function registerSearch(text)
                    if not text or text == "" then return end
                    Library._searchEntries[#Library._searchEntries+1] = {textLower = string.lower(text), category = Category, page = Page, groupFrame = gFrame}
                end
                -- removed group name registration so search only matches control titles
                -- Toggle
                ...
                function Group:AddDropdown(o)
                    ...
                    local valueLbl = Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1,0,1,0), Text = default and tostring(default) or "Select...", Font = Fonts.Medium, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = Theme.SubText, Parent = box})
                    -- chevron icon (two lines) rotating on open/close
                    local chev = Create("Frame", {BackgroundTransparency = 1, Size = UDim2.fromOffset(14,14), AnchorPoint = Vector2.new(0.5,0.5), Position = UDim2.new(1,-10,0.5,0), Parent = box})
                    -- curved chevron arms (rounded ends) to follow window corner style
                    local ch1 = Create("Frame", {BackgroundColor3 = Theme.Text, BorderSizePixel = 0, Size = UDim2.fromOffset(10,2), Position = UDim2.fromOffset(2,6), Parent = chev}, {
                        Create("UICorner", {CornerRadius = UDim.new(0,2)})
                    })
                    local ch2 = Create("Frame", {BackgroundColor3 = Theme.Text, BorderSizePixel = 0, Size = UDim2.fromOffset(10,2), Position = UDim2.fromOffset(2,6), Parent = chev}, {
                        Create("UICorner", {CornerRadius = UDim.new(0,2)})
                    })
                    ch1.Rotation = 45 ch2.Rotation = -45
                    -- dropdown panel (internal) appears below row; sized dynamically
                    local panel = Create("Frame", {BackgroundColor3 = Theme.Bg, BorderSizePixel = 0, Visible = false, Size = UDim2.fromOffset(0,0), ZIndex = z + 200, Parent = gFrame}, {
                        Create("UICorner", {CornerRadius = UDim.new(0,4)}),
                        Create("UIStroke", {Name="Stroke", Color = Theme.Stroke, Thickness = 1, Transparency = 0.4})
                    })
                    local panelScroll = Create("ScrollingFrame", {BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.fromOffset(4,4), Size = UDim2.new(1,-8,1,-8), CanvasSize = UDim2.fromOffset(0,0), AutomaticCanvasSize = Enum.AutomaticSize.Y, ScrollingDirection = Enum.ScrollingDirection.Y, ScrollBarThickness = 2, ScrollBarImageColor3 = Theme.Scrollbar, ZIndex = z + 201, Parent = panel}, {
                    ...
                    local function positionPanel()
                        panel.Position = UDim2.fromOffset(box.AbsolutePosition.X - gFrame.AbsolutePosition.X, row.AbsolutePosition.Y - gFrame.AbsolutePosition.Y + row.AbsoluteSize.Y + 2)
                        panel.Size = UDim2.fromOffset(box.AbsoluteSize.X, 0)
                    end
                    positionPanel()
                    box:GetPropertyChangedSignal("AbsoluteSize"):Connect(positionPanel)
                    box:GetPropertyChangedSignal("AbsolutePosition"):Connect(positionPanel)
                    local function animateChevron(open)
                        local base1, base2 = 45, -45
                        local targetAdd = open and 180 or 0
                        T(ch1,0.18,{Rotation = base1 + targetAdd}):Play()
                        T(ch2,0.18,{Rotation = base2 + targetAdd}):Play()
                    end
                    ...
                    local function openPanel()
                        ...
                        panel.Size = UDim2.fromOffset(box.AbsoluteSize.X,0)
                        local count = #(Dropdown._options or {})
                        local itemH = 24
                        local targetH = math.min(count, maxVisible) * (itemH + 3) + 8
                        T(panel,0.20,{Size = UDim2.fromOffset(box.AbsoluteSize.X, targetH)}):Play()
                        -- sequential reveal
                        ...
                        for _, child in ipairs(panelScroll:GetChildren()) do
                            if child:IsA("TextButton") then
                                ...
                                task.delay(idx*delayStep, function()
                                    if not opening or not child.Parent then return end
                                    T(child,0.15,{BackgroundTransparency = 0}):Play()
                                    if lbl then T(lbl,0.15,{TextTransparency = 0}):Play() end
                                end)
                            end
                        end
                    end
                    local function toggle()
                        if opening then closePanel() else openPanel() end
                    end
                    ...
                    -- Dropdown table already created above so functions could reference it
                    ...
                    function Dropdown:SetOptions(nl)
                        ...
                        for _, val in ipairs(nl or {}) do
                            ...
                            local item = Create("TextButton", {AutoButtonColor = false, BackgroundColor3 = Theme.Button, Size = UDim2.new(1,0,0,24), Text = "", Parent = panelScroll, ZIndex = z + 202}, {
                                Create("UICorner", {CornerRadius = UDim.new(0,4)}),
                                Create("UIStroke", {Name = "Stroke", Color = Theme.Stroke, Thickness = 1, Transparency = 0.6})
                            })
                            local l = Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1,-14,1,0), Position = UDim2.fromOffset(8,0), Text = tostring(val), Font = Fonts.Medium, TextSize = 12, TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left, Parent = item})
                            ...
                            item.MouseLeave:Connect(function() T(item,0.12,{BackgroundColor3 = Theme.Button}):Play() T(item.Stroke,0.12,{Transparency = 0.6}):Play() end)
                            item.MouseButton1Click:Connect(function() Dropdown:Set(val) end)
                        end
                        panelScroll.CanvasPosition = Vector2.new(0,0)
                        if opening then closePanel() end
                    end
    ...
    -- perform search across indexed entries (first match)
