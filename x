...
function Library:CreateLibrary(opts)
    ...
    local DIR = tostring(opts.ConfigFolder or Library.ConfigFolder or Base)
        ...
        -- Some executors on Windows require backslash paths for file APIs.
        -- Use helpers that transparently try both '/' and '\\' variants.
        local function cfgPathVariants(path)
        ...
        local function cfgIsFolder(path)
            if not gif then return false end
            local p1, p2 = cfgPathVariants(path)
            local ok, res = pcall(function() return gif(p1) end)
            if ok and res then return true end
            if p2 then
                local ok2, res2 = pcall(function() return gif(p2) end)
                if ok2 and res2 then return true end
            end
            return false
        end

        local function cfgMakeFolder(path)
            if not gmf then return end
            local p1, p2 = cfgPathVariants(path)
            pcall(function() gmf(p1) end)
            if p2 then pcall(function() gmf(p2) end) end
        end

        local function cfgRead(path)
            if not gr then return nil end
            local p1, p2 = cfgPathVariants(path)
            local ok, raw = pcall(function() return gr(p1) end)
            if ok and type(raw) == "string" then return raw end
            if p2 then
                local ok2, raw2 = pcall(function() return gr(p2) end)
                if ok2 and type(raw2) == "string" then return raw2 end
            end
            return nil
        end

        local function cfgWrite(path, data)
            if not gw then return false end
            local p1, p2 = cfgPathVariants(path)
            local ok = pcall(function() gw(p1, data) end)
            if ok then return true end
            if p2 then
                local ok2 = pcall(function() gw(p2, data) end)
                if ok2 then return true end
            end
            return false
        end

        local function cfgDelete(path)
            if not gd then return false end
            local p1, p2 = cfgPathVariants(path)
            local ok = pcall(function() gd(p1) end)
            if ok then return true end
            if p2 then
                local ok2 = pcall(function() gd(p2) end)
                if ok2 then return true end
            end
            return false
        end

        local function cfgListFiles(path)
            if not glf then return nil end
            local p1, p2 = cfgPathVariants(path)
            local ok, files = pcall(function() return glf(p1) end)
            if ok and type(files) == "table" then return files end
            if p2 then
                local ok2, files2 = pcall(function() return glf(p2) end)
                if ok2 and type(files2) == "table" then return files2 end
            end
            return nil
        end

        local function ensureFoldersCompat()
            if not gmf then return end
            if not cfgIsFolder(DIR) then cfgMakeFolder(DIR) end
            local cdir = DIR .. "/Configs"
            if not cfgIsFolder(cdir) then cfgMakeFolder(cdir) end
        end
        ...
        local function atomicWriteCompat(path, content)
            if MEM_MODE ~= nil and MEM_MODE == true then return end
            local p1, p2 = cfgPathVariants(path)
            local ok = pcall(function() atomicWrite(p1, content) end)
            if ok then return true end
            if p2 then
                local ok2 = pcall(function() atomicWrite(p2, content) end)
                if ok2 then return true end
            end
            return false
        end
    ...
    local function filePath(name) return configsFolder() .. "/" .. sanitize(name) .. ".json" end
        ...
        function Config.Save(name, data)
            ...
            if gmf then pcall(ensureFoldersCompat) end
            local okw = atomicWriteCompat(path, json)
            if not okw then
                
                local alt = sanitize(Base) .. "_" .. sanitize(s) .. ".json"
                local altOk = atomicWriteCompat(alt, json)
                if not altOk then
                    cfgErr("[Config] Save failed:", "fallback write failed")
                    return false
                end
                sessionAdd(s)
        ...
        function Config.List()
            ...
            if glf and cfgIsFolder(configsFolder()) then
                local files = cfgListFiles(configsFolder())
                local out = {}
        ...
        function Config.MostRecentAutoLoad()
            ...
            for _, name in ipairs(list) do
                local raw = cfgRead(filePath(name))
                if type(raw) == "string" and #raw > 0 then
                    local okj, dec = pcall(function() return HttpService:JSONDecode(raw) end)
            ...
            function Page:AddPage(g)
                ...
                function Group:Add3DPreviewWidget(o)
                    ...
                    local function rebuild()
                        ...
                        -- Prefer cloning the live character so the preview keeps the exact loaded
                        -- appearance (hair/accessories/meshes) even in ViewportFrames.
                        do
                        ...
                        -- Fallback: if cloning fails, build from HumanoidDescription.
                        ...
                        pcall(function()
                            ...
                            if cf0 and size0 then
                                ...
                                -- Previously this pushed the model down a lot on taller rigs.
                                -- Keep a modest margin so the feet don't clip while staying centered.
                                ...
                                if srcRoot and dstRoot and srcRoot:IsA("BasePart") and dstRoot:IsA("BasePart") then
                                    for i = 1, #map do
                                        ...
                                        if s and d and s.Parent and d.Parent and d ~= dstRoot then
                                            ...
                                            if not ok then
                                                -- Ignore any transient mapping errors.
                                            end
                                        end
                                    end
                                end
    ...
    Window.LoadConfig = function(a, b)
        ...
        -- Track last explicit load to avoid double-loading from the auto-load task.
        Window._lastLoadedConfigName = safe
        ...
        for id, value in pairs(data) do
            if type(id) == "string" and id:sub(1,2) == "__" then
                ...
                if not ctrl then
                    ...
                    -- NOTE: _applyConfigValue is a plain function, not a method.
                    -- Using ':' shifts arguments and breaks config application.
                    local ok = (libRef._applyConfigValue and libRef._applyConfigValue(targetId, ctrl, value))
                        or applyConfigValue(targetId, ctrl, value)
    ...
    local nameBox = group:AddTextbox({ Name = "Config Name", Placeholder = namePlaceholder, Default = "" })
        ...
        autoToggle = group:AddToggle({ Name = "Auto Load Config", Default = false, Callback = function(on)
            ...
            if (not raw or raw == "") then
                pcall(function() raw = nameBox and nameBox.Get and nameBox:Get() end)
            end
            local safe = raw and sanitize(raw) or nil
            if not safe or safe == "" then return end
    ...
    task.spawn(function()
        ...
        if toLoad and type(toLoad) == "string" and #toLoad > 0 then
            pcall(function()
                if Window._lastLoadedConfigName and Window._lastLoadedConfigName == toLoad then
                    return
                end
