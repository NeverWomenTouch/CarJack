    ...
    function tabhold:Tab(text)
        ...
        function tabcontent:NewDropdown(placeholder, Selectables, func, IsMulti, flag)
            ...
            local function NewSelectable(Text, val)
                ...
                if not IsMulti and (SelectedValue == Text or (val ~= nil and SelectedValue == val)) then
                ...
                library.Conn[#library.Conn + 1] = SelectableButton.MouseButton1Click:Connect(function()
                    if not IsMulti then
                        ...
                        pcall(function() func(Text, val) end)
                        ...
                        pcall(function() func(Stored, IsSelectableChoosed.Value) end)
                    end
                end)
            ...
            local function RefreshOptions(newOptions, selected)
                ...
                for _, option in pairs(newOptions or Selectables) do
...
function library:ConfigSystem(tab, name)
    ...
    function configSystem:RefreshConfigList()
        configSystem.configList = {}
        ...
        for _, file in ipairs(files) do
            ...
            if configName then
                ...
                local success, fileContent = pcall(function() return readfile(file) end)
                if success then
                    local success, data = pcall(function() return game:GetService("HttpService"):JSONDecode(fileContent) end)
                    if success and data.metadata and data.metadata.autoLoad then
    ...
    function deserializeValue(value)
        if type(value) == "table" and value.__type then
            ...
            elseif value.__type == "EnumItem" then
                local enumName = string.match(value.enumType, "Enum%.(.+)$")
                if enumName then
    ...
    local configTextbox = tab:Textbox("Config Name", false, function(text)
    ...
    local autoLoadToggle = tab:Toggle("Auto Load Selected Config", false, function(state)
        if state and configSystem.selectedConfig then
            local filePath = folder .. "/" .. configSystem.selectedConfig .. ".json"
            local success, fileContent = pcall(function() return readfile(filePath) end)
            ...
            if success then
                ...
                for _, configName in ipairs(configSystem.configList) do
                    if configName ~= configSystem.selectedConfig then
                        ...
                        local otherSuccess, otherContent = pcall(function() return readfile(otherFilePath) end)
                        
                        if otherSuccess then
                            local otherData = game:GetService("HttpService"):JSONDecode(otherContent)
                            if otherData.metadata then
                                otherData.metadata.autoLoad = false
                                writefile(otherFilePath, game:GetService("HttpService"):JSONEncode(otherData))
                            end
                        end
                    end
                end
    ...
    tab:Button("Create Config", function()
        ...
        if configName == "" then
            return library:Notification("Error", "Please enter a config name.", "OK")
        end
        
        if not string.match(configName, "^[%w_]+$") then
            return library:Notification("Error", "Config name can only contain letters, numbers, and underscores.", "OK")
        end
        
        for _, existingConfig in ipairs(configSystem.configList) do
            if existingConfig:lower() == configName:lower() then
                return library:Notification("Error", "Config already exists.", "OK")
            end
        end
    ...
    tab:Button("Save Config", function()
        ...
        library:Notification("Success", "Config saved successfully!", "OK")
    end)
    ...
    tab:Button("Load Config", function()
        ...
        local success, data = pcall(function() return game:GetService("HttpService"):JSONDecode(fileContent) end)
        if not success or not data.values then
        ...
        for flag, value in pairs(data.values) do
            if library.flags[flag] and library.flags[flag].set then
                ...
                if library.flags[flag].rainbow ~= nil and typeof(deserializedValue) == "Color3" then
                    library.flags[flag]:set(deserializedValue)
                    
                    if data.values[flag.."_rainbow"] ~= nil then
                        library.flags[flag]:setRainbow(data.values[flag.."_rainbow"])
                    end
                else
                    pcall(function()
                        library.flags[flag]:set(deserializedValue)
        ...
        library:Notification("Success", "Config loaded successfully!", "OK")
    end)
    ...
    tab:Button("Delete Config", function()
        ...
        local filePath = folder .. "/" .. configSystem.selectedConfig .. ".json"
        local success = pcall(function() return delfile(filePath) end)
        
        if not success then
            return library:Notification("Error", "Failed to delete config.", "OK")
        end
        ...
        library:Notification("Success", "Config deleted successfully!", "OK")
    end)
    
    tab:Button("Refresh Config List", function()
        configSystem:RefreshConfigList()
        dropdown:Refresh(configSystem.configList)
        library:Notification("Success", "Config list refreshed!", "OK")
    end)
    
    if configSystem.autoLoadConfig then
        local filePath = folder .. "/" .. configSystem.autoLoadConfig .. ".json"
        local success, fileContent = pcall(function() return readfile(filePath) end)
        
        if success then
            local success, data = pcall(function() return game:GetService("HttpService"):JSONDecode(fileContent) end)
            if success and data.values then
                for flag, value in pairs(data.values) do
                    if library.flags[flag] and library.flags[flag].set then
                        local deserializedValue = deserializeValue(value)
                ...
                library:Notification("Info", "Auto-loaded config: " .. configSystem.autoLoadConfig, "OK")
            end
        end
    end
