			...
			function ElementFunction:AddDropdown(DropdownConfig)
				...
				local function GetSelectedText()
					if DropdownConfig.Multi then
						...
						return count == 0 and "None Selected" or (count == 1 and Dropdown.Value[1] or tostring(count) .. " Selected")
					else
					...
					for _, option in ipairs(options) do
						...
						local OptionBtn = AddThemeObject(SetProps(SetChildren(MakeElement("Button", Color3.fromRGB(255,0,255)), {
							...
							AddThemeObject(SetProps(MakeElement("Label", option, 13, isSelected and 0 or 0.4), {
								Position = UDim2.new(0, DropdownConfig.Multi and 30 or 8, 0, 0),
								Size = UDim2.new(1, DropdownConfig.Multi and -30 or -8, 1, 0),
								Name = "Title"
							}), "Text")
						}), {
...
function ElementFunction:AddToggleColorPicker(ToggleColorPickerConfig)
    ...
    local function updateRainbowCheckbox()
        ...
        local targetBgColor = RainbowMode and Color3.fromRGB(45, 45, 45) or Color3.fromRGB(25, 25, 25)
        local targetStrokeTransparency = RainbowMode and 0 or 0.3
        local targetGlowTransparency = RainbowMode and 0.6 or 0.9
        
        TweenService:Create(RainbowCheckbox, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
            BackgroundColor3 = targetBgColor
        }):Play()
        
        TweenService:Create(RainbowStroke, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
            Transparency = targetStrokeTransparency,
            Color = ToggleColorPicker.ColorValue,
            Thickness = RainbowMode and 2 or 1.5
        }):Play()

        TweenService:Create(RainbowGlow, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
        ...
        if RainbowMode then
            ...
            RainbowCheck.ImageColor3 = Color3.fromRGB(255, 255, 255)
            TweenService:Create(RainbowCheck, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                Size = UDim2.new(0.8, 0, 0.8, 0)
            }):Play()
        else
    ...
    AddConnection(RainbowClick.MouseEnter, function()
        TweenService:Create(RainbowCheckbox, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
            BackgroundColor3 = RainbowCheckbox.BackgroundColor3:Lerp(Color3.fromRGB(70, 70, 70), 0.6)
        }):Play()
    ...
    local function UpdateColorPicker()
        ...
        RainbowGlow.BackgroundColor3 = ToggleColorPicker.ColorValue
