...
if env and env.libloaded then
    ...
    pcall(function()
        local coreGui = cg or SRV("CoreGui")
        if coreGui then
            for _, gui in pairs(coreGui:GetDescendants()) do
                if gui and gui.Name == library.LibName then
                    gui:Destroy()
                end
            end
        end
...
if env then env.libloaded = true end
...
function library:Unload()
    ...
    pcall(function()
        if cg and cg:FindFirstChild("Show") then
        ...
        pcall(function()
            ...
            while env and env.libloaded do
                ...
                for _, v in ipairs(textDisplays) do
                    if not (env and env.libloaded) then break end
    ...
    function tabhold:Tab(text)
        ...
        function tabcontent:KeybindT(text, default, keybind, callback, flag)
            ...
            function tabcontent:NewDropdown(placeholder, Selectables, func, IsMulti, EnableSearch, flag)
                ...
                local function newItem(text, val)
                    ...
                    library.Conn[#library.Conn+1] = SelectableButton.MouseEnter:Connect(function()
                        tweensv:Create(SelectableButton, TweenInfo.new(0.15), {BackgroundTransparency = 0.96}):Play()
                    end)
                    library.Conn[#library.Conn+1] = SelectableButton.MouseLeave:Connect(function()
                        tweensv:Create(SelectableButton, TweenInfo.new(0.15), {BackgroundTransparency = 1}):Play()
                    end)


                    library.Conn[#library.Conn+1] = SelectableButton.MouseButton1Click:Connect(function()
                        if IsMulti then
                            local newState = not (Stored[text] == true)
                            Stored[text] = newState or nil
                            dropdown.value = Stored
                            applyVisual(SelectableButton, newState)
                            if func then pcall(func, Stored, newState) end
                        else

                            for key, rec in pairs(itemsByKey) do
                                if rec and rec.button then
                                    applyVisual(rec.button, false)
                                end
                            end
                            SelectedValue = val or text
                            dropdown.value = SelectedValue
                            DropdownBox.Text = text
                            applyVisual(SelectableButton, true)
                            if func then pcall(func, text, val) end
                    ...
                    itemsByKey[text] = {button=SelectableButton, value=val, selected=isSelected}
                    return SelectableButton
                end

                local function diffRefresh(newOptions, preserveOnEmpty)
                    if type(newOptions) ~= "table" then return end

                    local wanted = {}
                    for _, opt in ipairs(newOptions) do
                        local key = (type(opt) == "table") and (opt.text or tostring(opt.value)) or tostring(opt)
                        local val = (type(opt) == "table") and opt.value or opt
                        if key and key ~= "" then wanted[key] = val end
                    end
                    

                    local Players = game:GetService("Players")
                    if next(wanted) == nil and preserveOnEmpty and #Players:GetPlayers() > 1 then
                        return
                    end
                    ...
                    for key, val in pairs(wanted) do
                        newItem(key, val)
                    end
                    

                    if IsMulti then
                        for k,_ in pairs(Stored) do
                            if not wanted[k] then Stored[k] = nil end
                        end
                        dropdown.value = Stored
                    else
                        if SelectedValue and not wanted[SelectedValue] and not wanted[tostring(SelectedValue)] then
                            SelectedValue = nil
                            dropdown.value = nil
                            DropdownBox.Text = placeholder
                        else
                            DropdownBox.Text = SelectedValue or placeholder
                        end
                    end
                end
                ...
                function NewSelectable(Text, val)
                    ...
                    if not IsMulti then
                        ...
                        isSelected = (SelectedValue == val) or (SelectedValue == Text) or (val == nil and SelectedValue == Text)
                        if isSelected then
                    ...
                    elseif IsMulti then
                        -- Multi selection check - check both text and value
                        isSelected = (Stored[Text] == true) or (val and Stored[val] == true)
                        if isSelected then
                            InnerCircle.Size = UDim2.new(0, 12, 0, 12)
                            ChoosedCircle.BackgroundTransparency = 0
                            ChoosedCircleStroke.Thickness = 0
                ...
                dropdown.set = function(self, selected)
                    ...
                    if selected == nil then
                        if IsMulti then
                            for k,_ in pairs(Stored) do Stored[k] = nil end
                            self.value = Stored
                        else
                            SelectedValue = nil
                            self.value = nil
                        ...
                        for key, rec in pairs(itemsByKey) do if rec and rec.button then applyVisual(rec.button, false) end end
                        return self
                    end

                    if IsMulti and type(selected) == "table" then
                        for k,_ in pairs(Stored) do Stored[k] = nil end
                        for k, v in pairs(selected) do if v == true then Stored[k] = true end end
                        self.value = Stored
                        for key, rec in pairs(itemsByKey) do applyVisual(rec.button, Stored[key] == true) end
                    else
                        -- single select by value or text
                        local chosenText = nil
                        ...
                        for _, option in pairs(self.options) do
                            ...
                            if optionValue == selected or optionText == selected then
                                chosenText = optionText; chosenValue = optionValue; break
                            end
                        end
                        if not chosenText then
                            chosenText = tostring(selected)
                            chosenValue = selected
                        end
                        SelectedValue = chosenValue
                        self.value = chosenValue
                        DropdownBox.Text = chosenText
                        for key, rec in pairs(itemsByKey) do applyVisual(rec.button, key == chosenText) end
                    end
                    return self
                end
                
                dropdown.Refresh = function(self, newOptions, selected)
                    if not DropdownList or not DropdownList.Parent then return self end
                    local targetSelection = (selected ~= nil) and selected or self.value
                    -- update options
                    if newOptions then
                        if type(newOptions) == "function" then
                            local ok, options = pcall(newOptions)
                            if ok and type(options) == "table" then self.options = options else return self end
                        else
                            self.options = newOptions
                        end
                    end
                    if type(self.options) ~= "table" then return self end
                    -- diff apply
                    diffRefresh(self.options, true)
                    -- restore selection
                    if IsMulti then
                        if type(targetSelection) == "table" then
                            for k,_ in pairs(Stored) do Stored[k] = nil end
                            for k,v in pairs(targetSelection) do if v == true then Stored[k] = true end end
                            self.value = Stored
                            for key, rec in pairs(itemsByKey) do applyVisual(rec.button, Stored[key] == true) end
                        end
                    else
                        if targetSelection ~= nil then
                            ...
                            for _, option in pairs(self.options) do
                                local optionValue = (type(option)=="table") and option.value or option
                                local optionText = (type(option)=="table") and (option.text or tostring(option.value)) or tostring(option)
                                if optionValue == targetSelection or optionText == targetSelection then chosenText = optionText break end
                            end
                            SelectedValue = targetSelection
                            self.value = targetSelection
                            DropdownBox.Text = chosenText or tostring(targetSelection)
                            for key, rec in pairs(itemsByKey) do applyVisual(rec.button, key == (chosenText or tostring(targetSelection))) end
                        end
                    end
                    return self
                end
                ...
                if EnableSearch == true and SearchBar then
                        ...
                        for key, rec in pairs(itemsByKey) do
                            if rec and rec.button then
                                if searchText == "" then
                                    ...
                                    rec.button.Visible = string.find(string.lower(key), searchText, 1, true) ~= nil
                                end
                            end
                        end
                    end)
                    ...
                    library.Conn[#library.Conn + 1] = SearchBar.FocusLost:Connect(function(enterPressed)
                        if enterPressed and not IsMulti then
                            local searchText = string.lower(SearchBar.Text)
                            local firstMatch = nil
                            for key, rec in pairs(itemsByKey) do
                                if string.find(string.lower(key), searchText, 1, true) then firstMatch = {text=key, value=rec.value, button=rec.button}; break end
                            end
                            if firstMatch then
                                for k, r in pairs(itemsByKey) do if r and r.button then applyVisual(r.button, false) end end
                                SelectedValue = firstMatch.value or firstMatch.text
                                dropdown.value = SelectedValue
                                DropdownBox.Text = firstMatch.text
                                applyVisual(firstMatch.button, true)
                                if func then pcall(func, firstMatch.text, firstMatch.value) end
                                tweensv:Create(ImageLabel, TweenInfo.new(0.3), {Rotation = 90}):Play()
                                Tsd:TweenPosition(UDim2.new(0.5, 0, 5, 0), "Out", "Quart", 0.3)
                                SearchBar.Text = ""
                            end
                ...
                if type(Selectables) == "function" then
                    local Players = game:GetService("Players")
                    local pending = false
                    local function debounced()
                        ...
                        delay(0.1, function()
                            pending = false
                            local ok, opts = pcall(Selectables)
                            if ok and type(opts) == "table" then dropdown:Refresh(opts) end
                        end)
                    end
                    debounced()
                    library.Conn[#library.Conn + 1] = Players.PlayerAdded:Connect(debounced)
                    library.Conn[#library.Conn + 1] = Players.PlayerRemoving:Connect(debounced)
                else
...
function library:ConfigSystem(tab, name)
    ...
    local function sync()
        ...
        for _, file in ipairs(files) do
            ...
            if item then
                ...
                if got then
                    local valid, data = pcall(httpsv.JSONDecode, httpsv, content)
    ...
    local valid, data = pcall(httpsv.JSONDecode, httpsv, raw)
        ...
        if on then
            for _, name in ipairs(cfg.list) do
                if name ~= cfg.pick then
                    ...
                    if has then
                        local good, info = pcall(httpsv.JSONDecode, httpsv, file)
                        if good and info.metadata then
                            info.metadata.autoLoad = false
                            pcall(writefile, other, httpsv:JSONEncode(info))
                        end
                    end
                end
            end
    ...
    local saved = pcall(writefile, path, httpsv:JSONEncode(data))
        if not saved then
            silentAutoLoadUpdate = true
            autoLoad:set(not on)
    ...
    local ok = pcall(writefile, path, httpsv:JSONEncode(data))
    ...
    ok = pcall(writefile, path, httpsv:JSONEncode(data))
    ...
    local valid, data = pcall(httpsv.JSONDecode, httpsv, raw)
        ...
        for flag, val in pairs(data.values) do
            if not (flag:match("_rainbow$") or flag == "config_autoload") then
            ...
            end -- close conditional skipping rainbow / autoload flags
            end
        end
        ...
        local success, err = pcall(function()
            ...
            for flag, val in pairs(data.values) do
                ...
                if library.flags[flag] then
                    ...
                    if success then loaded = loaded + 1 end
                end -- close conditional skipping rainbow / autoload flags (auto-load loop)
                end
            end
...
library.getPlayerList = function(excludeLocal)
    local players = {}
    ...
    local success, result = pcall(function()
        ...
        -- Default to excluding local player unless explicitly set to false
        excludeLocal = excludeLocal ~= false
        
        for _, player in pairs(Players:GetPlayers()) do
            if player and player.Name and type(player.Name) == "string" and player.Name ~= "" then
