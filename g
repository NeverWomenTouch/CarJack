...
function ElementFunction:AddSubTab(SubTabConfig)
    ...
    if not Container:FindFirstChild("SubTabsHolder") then
        -- Create SubTabsHolder if it doesn't exist (existing code)
    ...
    -- Create main SubTab container
    local SubTabContainer = SetProps(MakeElement("TFrame"), {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
        Parent = Container,
    ...
    -- Create columns layout for left and right sections
    local ColumnsLayout = SetChildren(SetProps(MakeElement("TFrame"), {
        Size = UDim2.new(1, 0, 0, 0),
        AutomaticSize = Enum.AutomaticSize.Y,
    ...
    -- Position right column to the right
    RightColumn.Position = UDim2.new(0.5, 5, 0, 0)
    
    -- The rest of your existing SubTab code (button creation, etc.)
    ...
    local function SelectSubTab()
        ...
        for _, child in pairs(SubTabContainer:GetDescendants()) do
            if not (child:IsA("UIListLayout") or child:IsA("UIPadding")) and child:IsA("Frame") then
        ...
        task.spawn(function()
            ...
            for _, child in pairs(SubTabContainer:GetDescendants()) do
                if not (child:IsA("UIListLayout") or child:IsA("UIPadding")) and child:IsA("Frame") then
                    if not (child.Name:find("Section") or child.Name:find("Column") or child.Name:find("Layout")) then
    ...
    -- Add button event handlers (same as your existing code)
    ...
    -- Function to create a subsection on the left column
    ...
    -- Function to create a subsection on the right column
    ...
    -- Keep the existing AddSection function for backward compatibility
