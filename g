        ...
        function Category:AddPage(p)
            ...
            function Page:AddBlock(g)
                ...
                if desc and #desc > 0 then
                    Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(1,0,0,16), Position = UDim2.fromOffset(0,y), Text = desc, Font = Fonts.Regular, TextSize = 12, TextColor3 = Theme.SubText, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = z + 2, Parent = gFrame})
                    y = y + 18
                end
                ...
                function Group:AddSlider(o)
                    ...
                    local cb = o.Callback
                    local id = ("%s/%s/%s/%s/%s"):format(Window.Name, Category.Name, Page.Name, Group.Name, label)
                    local row = Create("Frame", {BackgroundTransparency = 1, Size = UDim2.new(1,-2,0,40), Position = UDim2.fromOffset(0,nextY(40)), Parent = gFrame})
                    local labelWidth = 0.44
                    Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(labelWidth,-6,0,18), Text = label, Font = Fonts.Medium, TextSize = 13, TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left, Parent = row})
                    local right = Create("Frame", {BackgroundTransparency = 1, Size = UDim2.new(1-labelWidth,0,1,0), Position = UDim2.new(labelWidth,0,0,0), Parent = row})
                    local valueLabel = Create("TextLabel", {BackgroundTransparency = 1, Size = UDim2.new(0,64,0,18), Position = UDim2.new(1,-64,0,0), Text = tostring(default), Font = Fonts.Medium, TextSize = 13, TextColor3 = Theme.SubText, TextXAlignment = Enum.TextXAlignment.Right, Parent = right})
                    local bar = Create("Frame", {BackgroundColor3 = Theme.Button, Size = UDim2.new(1,-70,0,6), Position = UDim2.new(0,0,0,26), Parent = right}, {Create("UICorner", {CornerRadius = UDim.new(0,2)}), Create("UIStroke", {Color = Theme.Stroke, Thickness = 1, Transparency = 0.4})})
                    ...
                    function Slider:Set(v, silent)
                        ...
                        T(knob,0.1,{Position = UDim2.new(pct,0,0.5,0)}):Play()
