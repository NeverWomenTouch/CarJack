...
local Library do
    ...
    do -- Trial System
        ...
        local function GetTrialGameName()
            ...
            if placeId > 0 then
                local success, info = pcall(function()
                    return game:GetService("MarketplaceService"):GetProductInfo(placeId, Enum.InfoType.Place)
                end)
        ...
        local function OpenPremiumInvite()
            ...
            if HttpReq then
                opened = pcall(function()
                    ...
                    HttpReq({
                        ...
                        Body = HttpService:JSONEncode(Invite)
                    })
                end)
            end
            ...
            if opened then
                NotifyTrial("Premium Required", "Discord invite opened. Link copied to clipboard.", 5)
            elseif copied then
                NotifyTrial("Premium Required", "Discord link copied to clipboard.", 5)
            else
                NotifyTrial("Premium Required", "Join Discord: " .. InviteUrl, 7)
            end
        end
        ...
        local function MakeTrialModal(titleText, messageText, buttons)
            ...
            local Modal = Instances:Create("Frame", {
                Parent = gui.Instance,
                Name = "TrialModal",
                Size = UDim2New(0, 380, 0, 220),
                Position = UDim2New(0.5, -190, 0.5, -110),
            ...
            local btnWidth = 160
            local btnGap = 10
            local totalWidth = btnCount * btnWidth + (btnCount - 1) * btnGap
            local startX = (380 - totalWidth) / 2

            for i, btnData in ipairs(buttons) do
                local xOffset = startX + (i - 1) * (btnWidth + btnGap)
                local Btn = Instances:Create("TextButton", {
                    Parent = Modal.Instance,
                    Text = btnData.Text,
                    FontFace = Library.Font,
                    TextSize = 13,
                    TextColor3 = Library.Theme["Text"],
                    Size = UDim2New(0, btnWidth, 0, 35),
                    Position = UDim2New(0, xOffset, 1, -45),
                    BackgroundColor3 = btnData.Accent and Library.Theme["Accent"] or Library.Theme["Element"],
                    BorderSizePixel = 0,
                    AutoButtonColor = false
                })
            ...
            TitleLabel:MakeDraggable(Modal)

            return gui, Modal, MsgLabel
        end

        local function ShowTrialModal(trialDurationHours, canClaimTrial)
            local trialState = {
                TrialClaimed = false,
                Closed = false
            }
            ...
            local allowClaim = canClaimTrial ~= false
            gui, Modal, MsgLabel = MakeTrialModal(
                "Express Hub | Premium Required",
                allowClaim
                    and ("This script requires premium.\nTry a " .. tostring(trialDurationHours) .. " hour free trial to get started.")
                    or "Your free trial has expired.\nBuy premium to continue using this script.",
                {
                    {
                        Text = allowClaim and "Try Free Trial" or "Buy Premium",
                        Accent = true,
                        Callback = function()
                            if Processed then return end
                            Processed = true
                            if not allowClaim then
                                OpenPremiumInvite()
                                Processed = false
                                return
                            end

                            MsgLabel.Instance.Text = "Checking trial status..."
                            task.spawn(function()
                                local success = ClaimTrialWithHwid(trialDurationHours)
                                if success then
                                    trialState.TrialClaimed = true
                                    trialState.Closed = true
                                    gui.Instance:Destroy()
                                else
                                    MsgLabel.Instance.Text = "Could not connect to server.\nPlease try again."
                                    Processed = false
                                end
                            end)
                        end
            ...
            while not trialState.Closed do
                task.wait(0.05)
            end

            return trialState.TrialClaimed
        end

        local function ShowTrialExpiredModal()
            local done = false
            local gui, Modal
            gui, Modal = MakeTrialModal(
                "Express Hub | Trial Expired",
                "Your free trial has expired.\nPlease use the premium version to continue.",
                {
                    {
                        Text = "Close",
                        Accent = true,
        ...
        Library.ShowTrialModal = ShowTrialModal
        Library.ShowTrialExpiredModal = ShowTrialExpiredModal
    ...
    do -- Components
        Library.MakeWindow = function(self, Data)
            ...
            -- Handle trial system for premium content
            local PremiumRequired = Data.PremiumRequired or false
            local TrialTime = Data.TrialTime or 2
            ...
            if PremiumRequired then
                ...
                if not isTrialActive then
                    local trialClaimed = Library.ShowTrialModal(TrialTime, not (hasClaimedTrial and isTrialExpired))
