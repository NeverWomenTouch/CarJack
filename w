...
function Library:CreateLibrary(opts)
    ...
    do
        ...
        -- Per-game subfolder based on PlaceId only
        ...
        local function ensureFolder()
            if _isfolder and not _isfolder(Base) then _makefolder(Base) end
        ...
        local function fsAvailable()
            ...
            -- Probe actual write capability
            local probeName = "__probe_" .. tostring(math.random(100000,999999))
            local probePath = Folder .. "/" .. probeName .. ".tmp"
            local content = "ok"
            local wrote = pcall(function() _writefile(probePath, content) end)
            if not wrote then return false end
            local rdOk, rd = pcall(function() return _readfile(probePath) end)
            pcall(function() _delfile(probePath) end)
            if not rdOk then return false end
            return rd == content
        end
        ...
        local function atomicWrite(path, content)
            ...
            if _isfile(path) then pcall(_delfile, path) end
