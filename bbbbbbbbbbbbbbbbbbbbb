...
do
    ...
    local function createWatermark()
        ...
        -- Simple watermark dragging - robust PC/mobile, under-cursor
    local dragging = false
    local dragStart = Vector2.new(0, 0) -- GUI-space coords aligned with ScreenGui inset behavior
    local frameStart = Vector2.new(0, 0) -- AbsolutePosition in same GUI-space
...
do
    ...
    local startPos = Vector2.new(0, 0) -- viewport coords (minus GuiInset)
    local frameStartPos = Vector2.new(0, 0) -- AbsolutePosition (viewport coords)
        
        frame.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                isDragging = true
                
                -- Record starting input position in viewport coords
                local pos
                if input.UserInputType == Enum.UserInputType.Touch then
                    pos = input.Position
                else
                ...
                local inset = game:GetService("GuiService"):GetGuiInset()
                startPos = Vector2.new(pos.X - inset.X, pos.Y - inset.Y)
        ...
        UserInputService.InputChanged:Connect(function(input)
            ...
            local deltaY = currentPos.Y - startPos.Y
            
            -- Calculate new position
            local newX = frameStartPos.X + deltaX
            local newY = frameStartPos.Y + deltaY
            
            -- Get screen size for bounds checking
            local viewportSize = workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1920,1080)
            local frameSize = frame.AbsoluteSize
            
            -- Simple bounds - allow dragging to any edge (0,0 to screen size)
...
function Library:CreateLibrary(opts)
    ...
    function Category:AddSection(p)
            ...
            function Page:AddPage(g)
                ...
                function Group:AddKeybind(o)
                    ...
                    function Keybind:Set(keycode, silent)
                        ...
                        if self._keybindId then
                            local keyName = keycode and (keycode.Name or mouseButtonNames[keycode]) or "None"
                    ...
                    UserInputService.InputBegan:Connect(function(input)
                        ...
                        elseif waiting and input.KeyCode == Enum.KeyCode.Escape then
                            ...
                            stopVisual()
                            unbindSink()
                            box.Text = Keybind._value and (Keybind._value.Name or mouseButtonNames[Keybind._value] or "None") or "None"
                ...
                function Group:AddKeybindToggle(o)
                    ...
                    local keybindBtn = Create("TextButton", {BackgroundColor3 = Theme.Button, AutoButtonColor = false, Size = UDim2.fromOffset(60, 18), Text = getKeyName(currentKey, currentInputType), Font = Fonts.Regular, TextSize = 11, TextColor3 = Theme.SubText, Parent = rightContainer}, {Create("UICorner", {CornerRadius = UDim.new(0,4)}), Create("UIStroke", {Name = "Stroke", Color = Theme.Stroke, Thickness = 1, Transparency = 0.3})})
                    ...
                    local function updateKeybindVisual()
                        if isBinding then
                            ...
                            keybindBtn.Text = getKeyName(currentKey, currentInputType)
    ...
    Window.SaveConfig = function(name) return Config.Save(name, Library:_getSnapshot()) end
    Window.LoadConfig = function(name) local data = Config.Load(name) for id, value in pairs(data) do Library:_setValue(id, value, true) end end
