...
local Library do
    ...
    Library = {
        ...
        GlobalChat = {
            ...
            Token = "gchat_2026_9f6f2f4e83a84b0dbce13f5c0fb915",
            PollInterval = 3,
            MessageLimit = 50,
            PresenceInterval = 5,
            RetryDelay = 5,
        ...
        Library.Pages.GlobalChat = function(self)
            ...
            local Items = { } do
                ...
                Items["TypingText"] = Instances:Create("TextLabel", {
                    ...
                    Text = "",
                    Visible = false,
                    Size = UDim2New(1, -84, 0, 16),
                    Position = UDim2New(0, 10, 1, -60),
                    BackgroundTransparency = 1,
                    BorderSizePixel = 0,
                    TextSize = 13,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ...
                    ZIndex = 8
                })  Items["TypingText"]:AddToTheme({TextColor3 = "Text"})

                Items["TypingDebug"] = Instances:Create("TextLabel", {
                    Parent = Items["GlobalChat"].Instance,
                    Name = "\0",
                    FontFace = Library.Subfont,
                    TextColor3 = FromRGB(255, 170, 170),
                    BorderColor3 = FromRGB(0, 0, 0),
                    Text = "",
                    Visible = false,
                    Size = UDim2New(1, -20, 0, 12),
                    BackgroundTransparency = 1,
                    Position = UDim2New(0, 10, 1, -102),
                    BorderSizePixel = 0,
                    TextSize = 11,
                    TextXAlignment = Enum.TextXAlignment.Left,
                    ZIndex = 5
            ...
            if MainPageItems and not MainPageItems["UnreadBadge"] then
                MainPageItems["UnreadBadge"] = Instances:Create("Frame", {
                    ...
                    BorderColor3 = FromRGB(0, 0, 0),
                    BorderSizePixel = 0,
                    BackgroundColor3 = FromRGB(237, 66, 69)
                })
                ...
                MainPageItems["UnreadBadgeText"] = Instances:Create("TextLabel", {
                    ...
                    BorderSizePixel = 0,
                    TextSize = 10,
                    TextStrokeColor3 = FromRGB(90, 20, 22),
            ...
            local function UpdateUnreadBadge()
                ...
                if Count > 0 then
                    UnreadBadgeText.Instance.Text = StringFormat("+%d", math.min(Count, 9))
                    UnreadBadge.Instance.Size = Count >= 10 and UDim2New(0, 21, 0, 18) or UDim2New(0, 18, 0, 18)
                end
            end
            ...
            local function FormatTypingUsers(TypingUsers)
                if type(TypingUsers) ~= "table" then
                    return ""
                end

                local Names = { }

                for _, Value in TypingUsers do
                    if type(Value) ~= "table" then
                        continue
                    end

                    local IsLocalClient = tostring(Value.clientId or "") == BackendState.ClientId
                    local IsLocalSender = tostring(Value.senderId or "") == tostring(LocalPlayer.UserId)
                    local Username = tostring(Value.username or "")

                    if not IsLocalClient and not IsLocalSender and Username ~= "" then
                        TableInsert(Names, Username)
                    end
                end
                ...
                for _, Name in Names do
                    TableInsert(Parts, StringFormat("%s is typing..", Name))
                end

                return TableConcat(Parts, ", ")
            end

            local function UpdateTypingText(TypingUsers)
                BackendState.TypingUsers = type(TypingUsers) == "table" and TypingUsers or { }

                local Text = FormatTypingUsers(BackendState.TypingUsers)
                Items["TypingText"].Instance.Text = Text
                Items["TypingText"].Instance.Visible = Text ~= ""
            end

            local function SetTypingDebug(Text, Color)
                local Label = Items["TypingDebug"].Instance
            ...
            PerformSync = function(FetchFullHistory, TypingOverride)
                local Config = BackendState.Config
                if not Config or not RequestFunction or not Config.HttpUrl or Config.HttpUrl == "" then
                    return false
                end

                local IsTyping = TypingOverride
                if IsTyping == nil then
                    IsTyping = StringLen(ChatSystem:GetTypedMessage()) > 0
                end

                local AfterId = FetchFullHistory and 0 or BackendState.LastMessageId
                local Payload = {
                    ...
                    afterId = AfterId,
                    limit = (Config and Config.MessageLimit) or 50
                }

                local Success, ResponseBody = DoHttpRequest({
                    Url = BuildAuthorizedUrl(Config, Config.HttpUrl .. "/messages", Payload),
                    Method = "GET",
                    Headers = GetRequestHeaders(Config)
                })

                if not Success then
                    SetTypingDebug("messages sync failed", FromRGB(255, 140, 140))
                    return false
                end

                local Decoded = JsonDecode(ResponseBody)
                local TypingUsers = Decoded and Decoded.typingUsers
                local TypingCount = type(TypingUsers) == "table" and #TypingUsers or 0
                SetTypingDebug(StringFormat("messages typing count: %d", TypingCount), FromRGB(150, 220, 255))

                if TypingCount > 0 then
                    local Names = { }

                    for _, Value in TypingUsers do
                        if type(Value) == "table" and tostring(Value.username or "") ~= "" then
                            TableInsert(Names, tostring(Value.username))
                        end
                    end

                    if #Names > 0 then
                        SetTypingDebug(StringFormat("messages typing count: %d [%s]", TypingCount, TableConcat(Names, ", ")), FromRGB(150, 220, 255))
                    end
                end

                HandlePayload(Decoded)
                BackendState.TypingActive = IsTyping
                BackendState.LastTypingSentAt = os.clock()
                return true
            end

            local function SendTypingState(IsTyping, Force)
                local Config = BackendState.Config
                ...
                local Now = os.clock()
                if not Force and BackendState.TypingActive == IsTyping then
                    if not IsTyping then
                        return true
                    end

                    if Now - (BackendState.LastTypingSentAt or 0) < 1.25 then
                        return true
                    end
                end

                local Sent = PerformSync(false, IsTyping)
            ...
            local function StartTypingHeartbeat()
                if BackendState.TypingHeartbeatRunning then
                    return
                end

                BackendState.TypingHeartbeatRunning = true

                Library:Thread(function()
                    while not BackendState.Destroyed do
                        local TypedMessage = ChatSystem:GetTypedMessage()

                        if StringLen(TypedMessage) > 0 then
                    ...
                    BackendState.TypingHeartbeatRunning = false
                end)
            end

            local function SyncTypingState(Force)
                if StringLen(ChatSystem:GetTypedMessage()) > 0 then
                    SendTypingState(true, Force)
                    return
                end

                if Force or BackendState.TypingActive then
                    SendTypingState(false, true)
                end
            end
            ...
            HandlePayload = function(Payload)
                ...
                if Payload.typingUsers ~= nil then
                    UpdateTypingText(Payload.typingUsers)
                end
            ...
            local function StartTypingPolling()
                if BackendState.TypingPollRunning then
                    return
                end

                BackendState.TypingPollRunning = true

                Library:Thread(function()
                    while not BackendState.Destroyed do
                        PerformSync(not BackendState.InitialSyncComplete, nil)

                        task.wait(1)
                    end

                    BackendState.TypingPollRunning = false
                end)
            end
            ...
            local function SendChatMessage(Message)
                ...
                if not Success then
                    SetConnectionAppearance("Send failed", FromRGB(255, 120, 120))
                    return false
                end
            ...
            function ChatSystem:SendMessage(Avatar, Username, Message, IsLocalPlayer, IsAnonymous, CreatedAt, SenderKey)
                ...
                local function CreateBubble(Parent)
                    ...
                    Instances:Create("UISizeConstraint", {
                        Parent = BubbleItems["RealMessage"].Instance,
                        Name = "\0",
                        MaxSize = Vector2New(300, 140)
                    })
                end

                if LastGroup and LastGroup.GroupKey == GroupKey and LastGroup.IsAnonymous == ShouldHideIdentity and LastGroup.IsLocalPlayer == IsLocalPlayer then
                    CreateBubble(LastGroup.Stack.Instance)
                    task.defer(function()
                ...
                local SubItems = { } do
                    ...
                    CreateBubble(SubItems["MessageStack"].Instance)
                end
            ...
            local function CommitTypedMessage()
                ...
                local Sent = SendChatMessage(TypedMessage)
                if Sent then
