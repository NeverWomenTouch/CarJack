...
function library:ConfigSystem(tab, name)
    ...
    function pack(val)
        ...
        elseif typ == "table" then
            ...
            if val.keyCode then
                ...
                if val.mode then
                    keyData.mode = val.mode
                end
                
                if type(val.value) == "boolean" then
                    keyData.toggleState = val.value
                end
    ...
    function unpack(val)
        ...
        elseif val.__type == "Keybind" then
            ...
            local result = {
                keyName = val.keyName,
                keyCode = keyEnum or Enum.KeyCode[val.keyName]
            }
            
            if val.mode then
                result.mode = val.mode
            end
            
            if val.toggleState ~= nil then
                result.value = val.toggleState
            end
    ...
    local autoLoad = tab:Toggle("Auto Load Selected Config", false, function(on)
        if silentAutoLoadUpdate then
            silentAutoLoadUpdate = false
            return
        end
        ...
        if ok then
            ...
            if saved then
                if on then
                    ...
                    library:Notification("Success", "Auto-load disabled for config: " .. cfg.pick, "OK")
                end
            else
    ...
    tab:Button("Save/Overwrite Config", function()
        ...
        if ok then
            ...
            if valid and type(data) == "table" and type(data.values) == "table" then
                ...
                if data.metadata and data.metadata.autoLoad then
                    isAutoLoad = true
                end
                    ...
                    if elem and type(elem) == "table" then
                        if not flag:match("_rainbow$") then
                            if (flag:find("[bB]ind") or flag:find("[kK]eybind")) and elem.keyCode then
                                ...
                                if elem.mode then
                                    keyData.mode = elem.mode
                                end
                                
                                if type(elem.value) == "boolean" then
                                    keyData.toggleState = elem.value
                                end
                            ...
                            elseif type(elem.value) == "number" then
                                data.values[flag] = elem.value
    ...
    tab:Button("Load Config", function()
        ...
        for flag, val in pairs(data.values) do
            ...
            if flag == "config_autoload" then continue end
