                ...
                function Group:AddColorpicker(o)
                    ...
                    local function openPanel(btnFor, index)
                                ...
                                if slot.rainbow then
                                    ...
                                    Library._globalRGBSync.listeners[syncId] = function(syncColor, syncIndex)
                                        ...
                                        if open and activeIndex == idx then
                                            ...
                                            if svCursor and sv then svCursor.Position = UDim2.fromOffset(s*(sv.AbsoluteSize.X), (1-v)*(sv.AbsoluteSize.Y)) end
                                            if hueGrab and hueSlider then hueGrab.Position = UDim2.fromOffset(math.floor(hueSlider.AbsoluteSize.X/2), h*(hueSlider.AbsoluteSize.Y)) end
                                            if rgbBox and not rgbBox:IsFocused() then local r,g,b = math.floor(syncColor.R*255+0.5), math.floor(syncColor.G*255+0.5), math.floor(syncColor.B*255+0.5); rgbBox.Text = string.format("%d, %d, %d", r,g,b) end
                                            if hexBox and not hexBox:IsFocused() then local r,g,b = math.floor(syncColor.R*255+0.5), math.floor(syncColor.G*255+0.5), math.floor(syncColor.B*255+0.5); hexBox.Text = string.format("#%02X%02X%02X", r,g,b) end
                                        end
                                    end
                            ...
                            pulseRow.MouseButton1Click:Connect(function()
                                ...
                                if slot.pulse then
                                    ...
                                    slot.pulseHook = rainbowAdd(function(h)
                                        ...
                                        syncPulseColor(c, idx, slot._pulseSyncId)
                                    ...
                                    local c0 = hsvToColor(slot.hsv)
                                    if slot._pulseSyncId then syncPulseColor(c0, idx, slot._pulseSyncId) end
                                else
                        ...
                        if setCheckbox then -- reflect slot state
                            local slot = slots[index]
                            -- Update checkmark visuals to match selected slot
                            setCheckbox(slot.rainbow)
                            setPulseCheckbox(slot.pulse)
                            if slot.rainbow and not slot.rainbowHook then
                                slot.rainbowHook = attachRainbow(index)
                            end
                            if slot.pulse and not slot.pulseHook then
                                -- Recreate pulse hook using HSV-based pulsing
                                slot.pulseHook = rainbowAdd(function(h)
                                    local s = slot.hsv[2] or 1
                                    local baseV = slot.hsv[3] or 1
                                    local hh = (h + (slot.pulseHueOffset or 0)) % 1
                    ...
                    function Colorpicker:SetRainbow(on, index)
                        local function setOne(i, onv)
                            ...
                            if slot.rainbow then
                                ...
                                if open and activeIndex == i and setCheckbox then
                                    setCheckbox(true); setPulseCheckbox(false)
                                end
                            else
                                if slot.rainbowHook then rainbowRemove(slot.rainbowHook); slot.rainbowHook = nil end
                                slot._resumeRainbow = nil
                                if open and activeIndex == i and setCheckbox then setCheckbox(false) end
                            end
                        end
                    ...
                    function Colorpicker:SetPulse(on, index)
                        local function setOne(i, onv)
                            ...
                            if slot.pulse then
                                ...
                                if open and activeIndex == i and setPulseCheckbox then
                                    setPulseCheckbox(true); if setCheckbox then setCheckbox(false) end
                                end
                            else
                                if slot.pulseHook then rainbowRemove(slot.pulseHook); slot.pulseHook = nil end
                                if open and activeIndex == i and setPulseCheckbox then setPulseCheckbox(false) end
                            end
                        end
                        if not index then index = self:_getActiveIndex() end
                        setOne(index, on)
