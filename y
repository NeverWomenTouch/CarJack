...
function tabcontent:KeybindT(text, default, keybind, callback, flag)
    ...
    Toggle.TextColor3 = Color3.fromRGB(0, 0, 0)
    Toggle.TextSize = 14
    Toggle.ClipsDescendants = true -- Important for the slide effect
    ...
    ToggleTitle.TextSize = 18
    ToggleTitle.TextXAlignment = Enum.TextXAlignment.Left
    ToggleTitle.ZIndex = 3 -- Make sure text stays on top

    -- Right side toggle
    FrameToggle1.Name = "FrameToggle1"
    FrameToggle1.Parent = Toggle
    FrameToggle1.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
    FrameToggle1.Position = UDim2.new(0.85, 0, 0.5, -9) -- Right side
    FrameToggle1.Size = UDim2.new(0, 37, 0, 18)
    FrameToggle1.ZIndex = 3 -- Make sure toggle stays on top
    ...
    FrameToggleCircle.Size = UDim2.new(0, 10, 0, 10)
    FrameToggleCircle.ZIndex = 3

    FrameToggleCircleCorner.CornerRadius = UDim.new(1, 0)
    FrameToggleCircleCorner.Parent = FrameToggleCircle

    -- Keybind display - as a button now for better interaction
    KeybindDisplay.Name = "KeybindDisplay"
    KeybindDisplay.Parent = Toggle
    KeybindDisplay.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    KeybindDisplay.BackgroundTransparency = 0.4
    KeybindDisplay.Position = UDim2.new(0.6, 0, 0.5, -10)
    KeybindDisplay.Size = UDim2.new(0.2, 0, 0, 20)
    ...
    -- Add a corner and stroke to the keybind display
    ...
    -- Create a background for the dropdown content
    DropdownBackground.Name = "DropdownBackground"
    DropdownBackground.Parent = Toggle
    DropdownBackground.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
    ...
    -- Create keybind section and mode buttons
    local function createDropdownContent()
        ...
        KeybindSection.Size = UDim2.new(0.9, 0, 0, 35)
        ...
        ModeSection.Size = UDim2.new(0.9, 0, 0, 35)
        ...
        -- Create mode buttons with hover effects and selection colors
        local modeButtons = {}
        ...
        local function createModeButton(name, order)
            ...
            -- If selected, text color matches background, else white
            ...
            -- Connect the click event for mode selection
            library.Conn[#library.Conn + 1] = ModeButton.MouseButton1Click:Connect(function()
                keybindMode = name
                
                -- Update all buttons
                for modeName, button in pairs(modeButtons) do
                    if modeName == keybindMode then
                        tweensv:Create(button, TweenInfo.new(0.2), {
                            BackgroundColor3 = PresetColor,
                            BackgroundTransparency = 0,
                            TextColor3 = Color3.fromRGB(35, 35, 35) -- Text color to match background
                        }):Play()
                    else
                        tweensv:Create(button, TweenInfo.new(0.2), {
            ...
            -- Add hover effects
            library.Conn[#library.Conn + 1] = ModeButton.MouseEnter:Connect(function()
                if name ~= keybindMode then
                    tweensv:Create(ModeButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.2}):Play()
                end
            end)
            
            library.Conn[#library.Conn + 1] = ModeButton.MouseLeave:Connect(function()
                if name ~= keybindMode then
                    tweensv:Create(ModeButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.4}):Play()
                end
            end)
        ...
        -- Add hover effects for the keybind button
    ...
    -- Create content but keep it invisible until needed
    local dropdownElements = createDropdownContent()

    -- Function to toggle the keybind menu
    local function ToggleMenu()
        ...
        if MenuOpen then
            ...
            -- Update canvas size to accommodate the expanded toggle
            task.delay(0.05, function()
                Tab.CanvasSize = UDim2.new(0, 0, 0, TabLayout.AbsoluteContentSize.Y + 90)
            end)
        else
            -- Animate the background to collapse
    ...
    -- Create keybindT object first to avoid nil reference
    local keybindT = {
        value = toggled,
        keyCode = assignedKey,
        mode = keybindMode,
        set = function(self, state)
            -- Handle different ways this might be called:
            -- 1. From config loading with a boolean (toggle state)
            if type(state) == "boolean" then
                ToggleState(state)
                self.value = state
                return self
            ...
            -- 2. From config loading with a keybind object (both state and key)
            if type(state) == "table" then
                ...
                if typeof(state.keyCode) == "EnumItem" and state.keyCode.EnumType == Enum.KeyCode then
                    ...
                    if dropdownElements.keyButton then
                        dropdownElements.keyButton.Text = state.keyCode.Name == "Unknown" and "None" or state.keyCode.Name
                    end
                end
                
                -- Set the mode
                if type(state.mode) == "string" and (state.mode == "Toggle" or state.mode == "Hold" or state.mode == "Always") then
                    keybindMode = state.mode
                    self.mode = state.mode
                    
                    -- Update mode buttons
                    if dropdownElements.modeButtons then
                        for modeName, button in pairs(dropdownElements.modeButtons) do
                            if modeName == keybindMode then
                                button.BackgroundColor3 = PresetColor
                                button.BackgroundTransparency = 0
                                button.TextColor3 = Color3.fromRGB(35, 35, 35) -- Text color to match background
                            else
            ...
            -- 3. Direct key setting (just changes the key)
            if typeof(state) == "EnumItem" and state.EnumType == Enum.KeyCode then
                ...
                if dropdownElements.keyButton then
                    dropdownElements.keyButton.Text = state.Name == "Unknown" and "None" or state.Name
                end
                return self
            end
        ...
        setKey = function(self, key)
            if typeof(key) == "EnumItem" and key.EnumType == Enum.KeyCode then
                ...
                if dropdownElements.keyButton then
                    dropdownElements.keyButton.Text = key.Name == "Unknown" and "None" or key.Name
                end
            elseif typeof(key) == "string" then
                ...
                if success and keyEnum then
                    ...
                    if dropdownElements.keyButton then
                        dropdownElements.keyButton.Text = keyEnum.Name == "Unknown" and "None" or keyEnum.Name
                    end
                end
            end
            return self
    ...
    -- Define ToggleState with keybindT already created
    ...
    coroutine.wrap(function()
        while altloaded do
            ...
            if dropdownElements.modeButtons then
                for mode, button in pairs(dropdownElements.modeButtons) do
    ...
    -- Variable to track if a key is being held (for Hold mode)
    local isHolding = false
    
    -- Mouse events for the toggle
    library.Conn[#library.Conn + 1] = Toggle.MouseButton1Click:Connect(function()
    ...
    -- Right-click toggle to open menu
    library.Conn[#library.Conn + 1] = Toggle.MouseButton2Click:Connect(function()
        ToggleMenu()
    end)
    
    -- Click on keybind display to open menu
    library.Conn[#library.Conn + 1] = KeybindDisplay.MouseButton1Click:Connect(function()
        ToggleMenu()
    end)
    
    -- Add hover effect for the keybind display
    library.Conn[#library.Conn + 1] = KeybindDisplay.MouseEnter:Connect(function()
        tweensv:Create(KeybindDisplay, TweenInfo.new(0.2), {BackgroundTransparency = 0.2}):Play()
        tweensv:Create(KeybindDisplayStroke, TweenInfo.new(0.2), {Transparency = 0.2}):Play()
    end)
    ...
    library.Conn[#library.Conn + 1] = uis.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard and 
           assignedKey and input.KeyCode == assignedKey then
            
            -- Only allow keybind to work if toggle is enabled
            if toggled then
                -- Handle different keybind modes
                if keybindMode == "Toggle" then
                    -- In toggle mode, pressing the key toggles the state
                    ToggleState(not toggled)
                elseif keybindMode == "Hold" then
                    -- In hold mode, just track that we're holding
                    isHolding = true
                    -- No need to change toggle state as it's already on
                elseif keybindMode == "Always" then
                    -- Always mode doesn't change the toggle state
                end
            end
        end
    end)
    ...
    library.Conn[#library.Conn + 1] = uis.InputEnded:Connect(function(input, gameProcessed)
        if not gameProcessed and input.UserInputType == Enum.UserInputType.Keyboard and 
           assignedKey and input.KeyCode == assignedKey and keybindMode == "Hold" and isHolding then
            isHolding = false
            -- Only trigger callback if toggle is on and we were holding
            if toggled then
    ...
    if ok then
        ...
        if valid and type(data) == "table" and type(data.values) == "table" then
            ...
            for flag, elem in pairs(library.flags) do
                if elem and type(elem) == "table" then
                    if not flag:match("_rainbow$") then
                        ...
                        elseif type(elem.value) == "boolean" then
                            -- For toggles
                            data.values[flag] = elem.value
                        ...
                        elseif type(elem.value) == "string" then
                            -- For textboxes
                            data.values[flag] = elem.value
                        elseif flag:find("dropdown") or elem.options then
                            -- For dropdowns
                            data.values[flag] = elem.value
                        else
                            data.values[flag] = pack(elem)
            ...
            if ok then
                ...
                -- Add total count to the notification
