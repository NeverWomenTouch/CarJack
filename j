...
function Library:CreateLibrary(opts)
    ...
    local function filePath(name) return configsFolder() .. "/" .. sanitize(name) .. ".json" end
        ...
        function Config.Ensure()
            ...
            -- diagnostics: show detected capabilities and target folder
            pcall(function()
                warn("[Config] ensure: MEM_MODE=", tostring(MEM_MODE), " gw=", tostring(gw~=nil), " gr=", tostring(gr~=nil), " gif=", tostring(gif~=nil), " gmf=", tostring(gmf~=nil), " glf=", tostring(glf~=nil), " DIR=", tostring(DIR), " configs=", tostring(configsFolder()))
            end)
        ...
        function Config.Save(name, data)
            Config.Ensure()
            local s = sanitize(name or "")
            warn("[Config] Save called for:", tostring(s), "MEM_MODE=", tostring(MEM_MODE))
            if s == "" then return false end
            -- New payload shape: metadata + controls (easier for users to edit manually)
            local payload = { metadata = { name = s, lastModified = os.time(), autoLoad = false }, controls = {} }
            ...
            if not snapshot then
                ...
                for id, c in pairs(Library._controls) do
                    if c and c.Get then
                        ...
                        if not ok then
                            warn("[Config] _getSnapshot: Get failed for id=", tostring(id), "err=", tostring(val))
                        else
                            ...
                            if type(c) == 'table' and c._slots then
                                ...
                                warn("[Config] captured Colorpicker for saving:", tostring(id), "rainbow slots=", tostring(#rainbowFlags), "pulse slots=", tostring(#pulseFlags))
                            else
                                snapshot[id] = val
                            end
                        end
            ...
            for id, val in pairs(snapshot or {}) do
                ...
                if type(val) == 'table' and (val.__ctrl == 'Colorpicker' or (Library._controls[id] and Library._controls[id]._slots)) then
                    ...
                    payload.controls[id] = cp
                    warn("[Config] captured Colorpicker for saving:", tostring(id), "slots=", tostring(#(cp.slots or {})))
                else
                    local ok, ser = pcall(function() return serialize(val) end)
                    if ok then
                        payload.controls[id] = ser
                    else
                        warn("[Config] serialize failed for id=", tostring(id), "type=", typeof(val), "err=", tostring(ser))
                        local safeString = nil
                        pcall(function() safeString = tostring(val) end)
                        payload.controls[id] = safeString
                    end
                end
            end
            if MEM_MODE then
            ...
            -- Diagnostics: attempt to encode JSON
            local okj, json = pcall(function() return HttpService:JSONEncode(payload) end)
            if not okj or type(json) ~= "string" then
                warn("[Config] JSON encode failed for:", s, "okj:", tostring(okj), "jsonType:", type(json))
                return false
            end
            ...
            local function prettyJSON(str)
                ...
                for i = 1, #str do
                    local c = str:sub(i,i)
                    if c == '"' then
                        -- toggle string state if previous char isn't backslash
            ...
            -- ensure folders exist when possible
            if gif and gmf then pcall(ensureFolders) end
            warn("[Config] Save: attempting write to:", tostring(path))
            local okw, err = pcall(function() atomicWrite(path, json) end)
            if not okw then
                warn("[Config] primary atomicWrite failed:", tostring(err))
                -- fallback to root-style filename when we can't create folders
                local alt = sanitize(Base) .. "_" .. sanitize(s) .. ".json"
                warn("[Config] Save: attempting fallback write to:", tostring(alt))
                local altOk, altErr = pcall(function() atomicWrite(alt, json) end)
                if not altOk then
                    warn("[Config] fallback atomicWrite also failed:", tostring(altErr))
                    return false
                end
                -- record the alternative path in session index
                sessionAdd(s)
            ...
            sessionAdd(s)
            pcall(function() Config.RecordSave(s) end)
            warn("[Config] Save successful:", s, "->", tostring(path))
            return true
        end
        ...
        function Config.Load(name)
            ...
            if MEM_MODE then
                ...
                if type(t) == "table" and type(t.values) == "table" then
                    local out = {}
            ...
            if glf and gif and gif(configsFolder()) then
                ...
                if ok and type(files) == "table" then
                    for _, f in ipairs(files) do
                        ...
                        n = n:gsub("%.json$", "")
                        if n ~= "" and not n:match("^__") then table.insert(out, n) end
                    end
                end
                table.sort(out)
                return out
        ...
        function Config.Delete(name)
            ...
            local path = filePath(s)
            pcall(function() if gd then gd(path) end end)
            sessionRemove(s)
    ...
    Window.LoadConfig = function(name)
        ...
        warn("[Window] LoadConfig called for:", tostring(safe))
        local okLoad, data = pcall(function() return Config.Load(safe) end)
        if not okLoad then warn("[Window] Config.Load errored:", tostring(data)) return end
        if type(data) ~= 'table' then warn("[Window] Config.Load returned non-table") return end
        local applied = 0
        for id, value in pairs(data) do
            if type(id) == "string" and id:sub(1,2) == "__" then
                ...
                if not ctrl then
                    warn("[Window] LoadConfig: no control found for id:", tostring(id))
                else
                    if not ctrl.Set then
                        warn("[Window] LoadConfig: control found but has no Set method:", tostring(targetId))
                    else
                        ...
                        if type(value) == 'table' and (value.__ctrl == 'Colorpicker' or value.colors) then
                            ...
                            warn("[ConfigDebug] Entering Colorpicker load handler for", tostring(targetId))
                            local okOuter, outerErr = pcall(function()
                                warn("[ConfigDebug] Loading Colorpicker for", tostring(targetId))
                                -- dump incoming value for diagnostics
                                pcall(function()
                                    -- Build a JSON-safe copy (serialize Color3/Enum/UDim2 etc) so HttpService:JSONEncode doesn't emit null for userdata
                                    local function makeSerializable(x)
                                        local tx = typeof(x)
                                        if tx == 'Color3' or tx == 'EnumItem' or tx == 'UDim2' or tx == 'Vector2' or tx == 'Instance' then
                                            return serialize(x)
                                        elseif type(x) == 'table' then
                                            local out = {}
                                            for kk, vv in pairs(x) do out[kk] = makeSerializable(vv) end
                                            return out
                                        else
                                            return x
                                        end
                                    end
                                    local okv, sval = pcall(function() return HttpService:JSONEncode(makeSerializable(value)) end)
                                    if okv then warn("[ConfigDebug] incoming value JSON:", sval) else warn("[ConfigDebug] JSON dump failed:", tostring(sval)) end
                                end)
                                
                                local function applyOnce()
                                    local colors = value.colors or {}
                                    local decColors = {}
                                    for i=1, #colors do
                                        local okd, dc = pcall(function() return deserialize(colors[i]) end)
                                        if okd then decColors[i] = dc else decColors[i] = nil; warn("[ConfigDebug] deserialize failed for slot", i, tostring(dc)) end
                                    end
                                    warn("[ConfigDebug] decoded color slots:", tostring(#decColors))

                                    -- Apply colors silently first
                                    ...
                                    if ctrl and ctrl._slots then
                                        for i=1,#ctrl._slots do
                                            ...
                                            if type(value.rainbow) == 'table' and value.rainbow[i] == true and not slot.rainbowHook and rainbowAdd then
                                                ...
                                                warn("[ConfigDebug] fallback rainbowHook attached for", tostring(targetId), "slot", i)
                                            end
                                            if type(value.pulse) == 'table' and value.pulse[i] == true and not slot.pulseHook and rainbowAdd then
                                                ...
                                                warn("[ConfigDebug] fallback pulseHook attached for", tostring(targetId), "slot", i)
                                            end
                                        end
                                    end
                                    ...
                                    -- diagnostic dump
                                    pcall(function()
                                        warn("[ConfigDebug] Colorpicker state for", tostring(targetId))
                                        if ctrl and ctrl._slots then
                                            for i=1,#ctrl._slots do local slot = ctrl._slots[i]; warn(string.format("[ConfigDebug] slot %d: color=%s, rainbow=%s, rainbowHook=%s, _syncId=%s, pulse=%s, pulseHook=%s, _pulseSyncId=%s", i, tostring(slot and slot.color), tostring(slot and slot.rainbow), tostring(slot and slot.rainbowHook ~= nil), tostring(slot and slot._syncId), tostring(slot and slot.pulse), tostring(slot and slot.pulseHook ~= nil), tostring(slot and slot._pulseSyncId))) end
                                        else warn("[ConfigDebug] ctrl._slots missing for", tostring(targetId)) end
                                        warn("[ConfigDebug] global rainbow listeners:", tostring(#(Library._rainbowBus and Library._rainbowBus.listeners or {})), "globalRGBSync.active=", tostring(Library._globalRGBSync and Library._globalRGBSync.active))
                                        warn("[ConfigDebug] global pulse listeners:", tostring(#(Library._globalPulseSync and Library._globalPulseSync.listeners or {})), "globalPulseSync.active=", tostring(Library._globalPulseSync and Library._globalPulseSync.active))
                                    end)
                            ...
                            local ok, err = pcall(function() Library:_setValue(targetId, value, true) end)
                            if not ok then
                                warn("[Window] _setValue failed for id:", tostring(targetId), "err:", tostring(err))
                            else
                                applied = applied + 1
                            end
                        end
        ...
        warn("[Window] LoadConfig applied values:", applied)
    end
    ...
    local nameBox = group:AddTextbox({ Name = "Config Name", Placeholder = namePlaceholder, Default = "" })
        ...
        local createBtn = group:AddButton({ Name = "Create", Compact = true, Callback = function()
            ...
            warn("[ConfigUI] Create clicked, raw input:", tostring(raw))
            ...
            local low = safe:lower()
            if safe == "" or low == "__meta" or low == "__index" then return end
            warn("[ConfigUI] Create -> saving as:", tostring(safe))
        ...
        local saveBtn = group:AddButton({ Name = "Save", Compact = true, Callback = function()
            ...
            warn("[ConfigUI] Save clicked, raw selection:", tostring(raw))
            if type(raw) == "string" then
                local l = raw:lower()
                if l == "__meta" or l == "__index" then return end
            end
            if not raw or raw == "" then return end
            local safe = sanitize(raw)
            warn("[ConfigUI] Save -> saving as:", tostring(safe))
            local okSave, res = pcall(function() return Window.SaveConfig(safe) end)
            if okSave and res then
                refreshList()
                task.defer(function()
                ...
                if not okSave then warn("[ConfigUI] Save pcall error") end
            end
        end })
        local loadBtn = group:AddButton({ Name = "Load", Compact = true, Callback = function()
            ...
            pcall(function() raw = dd and dd.Get and dd:Get() end)
            if (not raw or raw == "") then pcall(function() raw = nameBox and nameBox.Get and nameBox:Get() end) end
            warn("[ConfigUI] Load clicked, raw selection:", tostring(raw))
            if type(raw) == "string" then
                local l = raw:lower()
                if l == "__meta" or l == "__index" then return end
            end
        ...
        local deleteBtn = group:AddButton({ Name = "Delete", Compact = true, Callback = function()
            ...
            pcall(function() raw = dd and dd.Get and dd:Get() end)
            if (not raw or raw == "") then pcall(function() raw = nameBox and nameBox.Get and nameBox:Get() end) end
            warn("[ConfigUI] Delete clicked, raw selection:", tostring(raw))
            if type(raw) == "string" then
                local l = raw:lower()
                if l == "__meta" or l == "__index" then return end
            end
